/**
 * @import { ComponentType, ElAttrs, ElDispatch, El, ElTopLevel, ElNode } from './type.ts'
 */
...
export const render = (function () {
   ...
   * @param {ElAttrs | null | undefined} attrs
   * @param {El[] | undefined} children
   */
  const wireElement = (element, attrs, children = []) => {
    if (attrs != null) {
      const { events = {}, styles = {}, dataset = {}, ...other } = attrs;
      ...
      if (!def) {
        ...
        try {
          if (typeof def[0] === 'string') {
            element = document.createElement(def[0]);
          } else {
            element = def[0];
          }
        } catch (e) {
        ...
        wireElement(element, def[1], def[2]);
...
export function updateElement(node, parent, vElement) {
  try {
    ...
    if (vElement[0] instanceof Function) {
      console.warn('updating web components not implemented');
      return false;
    }

    const attributes = vElement[1] ?? {};
    const children = vElement[2] ?? [];
    const domElement = /* @type {Element} */ (node);

    if (
        typeof vElement[0] === 'string' &&
        domElement.tagName.toLowerCase() !== vElement[0].toLowerCase()
    ) {
