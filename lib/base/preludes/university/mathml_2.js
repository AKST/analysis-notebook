...
export const set = makeHelper((attrs, body) => parensC(attrs)(
  ...seperateItems(body, [mo`,`, mspace({ space: 4 })]),
));

export const annotationUnder = makeHelper(({ label, ...attr }, body) => (
  munder(attr)(mrow(...body), munder(mo`⏟`, mrow(label)))
), { label: mrow() });

export const annotationOver = makeHelper(({ label, ...attr }, body) => (
  mover(attr)(mrow(...body), mover(mo`⏟`, mrow(label)))
), { label: mrow() });
...
export const sum = makeHelper(({ max, inc, ...attr }, items) => (
  mrow(attr)(
    munderover(mi`∑`, mrow(inc), mrow(max)),
    mspace({ space: 4 }),
    mrow(...items),
  )
), ({ max: mrow(), inc: mrow() }));
 ...
 * @returns {(...items: E.Item[]) => E.Item}
 */
const makeOperatorPA = (o, space=4) => (...items) => mrow(
 ...
 * @returns {(...items: E.Item[]) => E.Item}
 */
const makeOperatorPB = (o, space=4) => (...items) => mrow(
