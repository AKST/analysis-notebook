 ...
 */
import { makeTemplateHelper } from '../../render_app/helper/util.js';
import { helpers as doc } from '../../render_app/index.js';

/** @param {string} tagName @param {string} cls */
const makeClsTag = (tagName, cls) => makeTemplateHelper(({ className: classIn, ...attr }, items) => {
  const className = classIn == null
    ? cls
    : `${classIn} ${cls}`;
  return [tagName, { className, ...attr }, items];
});

export const text = {
  p: {
    m: makeClsTag('p', 'text-medium'),
    l: makeClsTag('p', 'text-large'),
  },
  li: {
    m: makeClsTag('li', 'text-medium'),
    l: makeClsTag('li', 'text-large'),
  },
};
...
export const note = (...content) => (
  ['p', {
    ...
    doc.small.of(doc.b.of('NOTE')), ulLightSm(content),
  ]]
);
...
export const noteOn = (topic) => (...content) => (
  ['p', {
    ...
    doc.small.of(doc.b`NOTE (on ${topic} )`), ulLightSm(content),
  ]]
);
 ...
 * @returns {E.Item}
 */
export const ulLightSm = items => doc.small.of(['ul', { className: 'no-item-padding' }, (
  items.map(item => ['li', {}, [item]])
)]);
