...
export const note = (...content) => (
  ['p', {
    ...
    doc.small`${['strong', ['NOTE']]}:`, ulLightSm(content),
  ]]
);
...
export const noteOn = (topic) => (...content) => (
  ['p', {
    ...
    doc.small`${['strong', ['NOTE (on ', topic, ')']]}:`, ulLightSm(content),
  ]]
);
 ...
 * @returns {E.Item}
 */
export const ulLightSm = items => doc.small`${['ul', { className: 'no-item-padding' }, (
  items.map(item => ['li', {}, [item]])
)]}`;
