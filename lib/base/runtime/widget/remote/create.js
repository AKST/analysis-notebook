 ...
 * @import { SendOrigin, RemoteConnection, ParentToChildMessage, RemoteWidget } from './type.ts';
 */
...
export class RemoteWidgetWidgetEngine {
   ...
   */
  render(config, state, event) {
    /** @type {SendOrigin} */
    let origin;

    if (event?.kind === 'config') {
      const source = (event?.source ?? []).join(':');
      origin = { kind: 'knob', source };
    } else {
      origin = { kind: 'unknown' };
    }
    ...
    this.#normalisedSend({ kind: 'push', state, config }, origin);
  }
   ...
   * @param {SendOrigin} origin
   */
  #normalisedSend(message, origin) {
    if (this.module.normaliseSend) {
      for (const prepared of this.module.normaliseSend(message, origin)) {
        this.#send(prepared);
      }
    }
  }
