...
import * as doc from '../../dsl_dom/helper/html.js';
import { render, renderOnto } from '../../dsl_dom/render.js';
import { insertAtStart } from '../../util/dom.js';
import { initialiseState, getFactory as __getFactory } from './util.js';
 ...
 */
export class MultiWidgetApplicationEngine {
  #state;
  #columns
  ...
  /**
   * @param {typeof __getFactory} getFactory
  ...
  installWidgets(viewportWidth) {
    ...
    renderOnto(this.container, [
      doc.hr.css({ width: '100%', borderColor: 'transparent' }).void({}),
    ])
  }
  ...
  installWidget(childModule, index, viewportWidth) {
    ...
    const widgetFactory = this.#getFactory(childModule);
    ...
    if (hud.header) {
      insertAtStart(widgetContainer, render(hud.header, undefined));
    }
