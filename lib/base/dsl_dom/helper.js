 ...
 */
import { node, AttributeContainer, RenderNode, FragmentDemarker } from './render.js';
...
class ElementBuilder {
  ...
  /**
   * @param {E.Attrs<Attrs>} meta
   */
  meta({ events, dataset, styles, element }) {
    return new ElementBuilder(
      this.#construct,
      this.#upsertAttrs(element),
      { ...this.styles, ...styles },
      { ...this.data, ...dataset },
      { ...this.events, ...events },
      this.children,
...
class VoidElementBuilder {
  ...
  attr(attrsIn) {
    ...
    const meta = new AttributeContainer({}, this.dataset, this.styles, attr);
  ...
  ({ of: [ns, t], create }) => {
    ...
    return new Proxy(helper, {
      apply(_target, _thisArg, args) {
        ...
        if (args[0] instanceof AttributeContainer) {
...
export const makeHelper = (
   ...
   * @param {{
   *   of: [NS, T],
   *   create?: (meta: E.Attrs<O>, items: I) => HelperNode<NS, T>,
   * }} cfg
  ...
  ({ of: [ns, t], create }) => {
    ...
    const initialise = () => {
      return new ElementBuilder(/** @type {any} */(f), defaultArgs, {}, {}, {});
    }
