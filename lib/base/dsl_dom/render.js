...
export const update = /** @type {any} */ ((
  ...
  if (typeof newSpec === 'string') {
    ...
    return render(newSpec, eventRegistry);
  }
  ...
  if (attributes) {
    ...
    if (attributes && attributes.events && eventRegistry) {
      // element.dataset.eventsource = eventRegistry.register(events);
      throw new Error('events not implemented');
    }
  }
  ...
  for (const [i, newChild] of flattenChildren(children)) {
    const currentChild = currentChildren[i];

    if (currentChild) {
      const replacement = update(currentChild, newChild, eventRegistry);
      if (replacement) domElement.replaceChild(replacement, currentChild);
    } else {
      domElement.appendChild(render(newChild, eventRegistry));
    }
  }
});
...
export function setAttribute(ns, tag, attr, element, valueIn) {
  try {
    ...
    switch (access) {
      case 'getAttribute':
        element.setAttribute(/** @type {any} - no sane way to type */ (attr), valueEncoded);
        break;
