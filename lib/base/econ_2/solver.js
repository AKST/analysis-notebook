...
/**
 * @param {Model.Config.T['market']} market
...
export function status(model) {
  ...
  switch (market.kind) {
    ...
    case 'exporting': {
      ...
      const dq = getQuantityAtPrice(shiftCurve(demand, demandWedge), effectivePrice);
    ...
    case 'importing': {
      ...
      const sq = getQuantityAtPrice(shiftCurve(supply, supplyWedge), effectivePrice);
  ...
  return {
    ...
    model: {
      ...
      demand: {
        ...
        market: marginalValue(demandState, shiftCurve(demand, demandShift), q0),
