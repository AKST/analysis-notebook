...
export function status(model) {
  ...
  switch (market.kind) {
    ...
    case 'importing:world': {
      ...
      demandState = market.supply === 'ceiling'
        ? ({ kind: 'dual-alloc', local: demandLocal, world: demandWorld })
        : ({ kind: 'mono-alloc', value: demandWorld });
...
export function taxRevenue(alloc, rawCurve, shift) {
  const [{ dir }] = curve.seperateCurveFromShift(rawCurve);
  if (shift * dir <= 0) {
    return { ok: true, space: { size: 0, geom: [] } };
  }
