/**
 * @import { Vec2 } from '../../geom-2d/type.ts';
...
import { v2 } from '../../geom-2d/index.js';
 ...
 * @param {number} m
 * @param {number} i
 * @returns {Curve.Continious}
 */
export const continious = (dir, i, m) => ({ kind: 'continious', dir, m, i });
...
export function equilibrium(demandIn, supplyIn) {
  ...
  if (supply.dir !== 1) throw new Error('incorrect curve direction');
  if (demand.dir !== -1) throw new Error('incorrect curve direction');
...
/**
 * @param {Curve.T} demand
 * @param {Curve.T} supply
 * @param {number} price
 * @returns {{ ok: true, q: number } | { ok: false, reason: string }}
 */
export function quantityGapAtPrice(demand, supply, price) {
  const demandQ = getQuantityAtPrice(demand, price);
  if (!demandQ.ok) return extendReason(demandQ, 'gap');

  const supplyQ = getQuantityAtPrice(supply, price);
  if (!supplyQ.ok) return extendReason(supplyQ, 'gap');

  return { ok: true, q: demandQ.q - supplyQ.q };
}

/**
 * Get the price at quota
 *
 * @param {Curve.T} demandIn
 * @param {Curve.T} supplyIn
 ...
 * @returns {EquilibriumResult}
 */
export function quotaPrice(demandIn, supplyIn, quantity) {
  if (quantity === 0) return equilibrium(demandIn, supplyIn);

  const [supply, supplyShift] = seperateCurveFromShift(supplyIn);
  if (supply.dir !== 1) throw new Error('incorrect curve');

  switch (supply.kind) {
    case 'continious': {
      const { i: si, m: spb } = supply;
      const spa = si + supplyShift;
      const sqa = ((-spa)/spb) + quantity;
      const sqb = 1/spb;
      const sa = (-sqa)/sqb;
      const sb = 1/sqb;

      /** @type {Curve.T} */
      const supplyShifted = { kind: 'continious', dir: 1, i: sa, m: sb };
      return equilibrium(demandIn, supplyShifted);
    }
  ...
  switch (a.kind) {
    case 'continious': switch (b.kind) {
      case 'continious': {
        ...
        const i = (-si) / sm;
        const m = 1 / sm;
        /** @type {Curve.Continious} */
        const curve = { kind: 'continious', i, m, dir };
        // @ts-ignore - dw
        return s === 0 ? curve : shift(curve, s);
      }
    ...
    case 'discrete': switch (b.kind) {
      ...
      case 'discrete': {
        ...
        /** @type {Curve.Discrete} */
        const curve = { kind: 'discrete', rate, dir };
        // @ts-ignore - dw
        return s === 0 ? curve : shift(curve, s);
      }
...
/**
 * @overload
 * @param {readonly Curve.Continious[]} curves
 * @returns {Curve.Continious}
 */
/**
