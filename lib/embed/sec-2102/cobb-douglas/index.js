...
async function createPipelines(device, format, scene, uniforms, buffers, textures) {
  const { curveShader, textShader } = await loadShader(device);
  ...
  const textPipeline = device.createRenderPipeline({
    ...
    vertex: {
      ...
      buffers: [
        ...
        { // Instance data: position (vec3), rotation (f32), scale (vec2)
          arrayStride: 6 * 4, // 3 + 1 + 2 floats = 24 bytes
          stepMode: 'instance',
          attributes: [
            { shaderLocation: 1, offset: 0, format: 'float32x3' },      // position
            { shaderLocation: 2, offset: 12, format: 'float32' },        // rotation
            { shaderLocation: 3, offset: 16, format: 'float32x2' },      // scale
          ],
        }
      ],
    },
...
function createUniforms(device) {
  ...
  return { meshGen, meshRender, text };
}
...
function createBuffers(device, scene) {
  ...
  return {
    meshVertices: scene.curveDomain.createVertixMesh(device),
