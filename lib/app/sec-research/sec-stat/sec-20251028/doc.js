  ...
  dashbox(
    ...
    text.ol(
      doc.li`
        ...
        value, instead ${text.b`return a PDF or distribution`} relating to the
        probablity of possible values?

        ${text.ul({ blockMargin: 1, itemSpace: 'sparse' }).c(
          doc.li`
      ...
      doc.li`
        ...
        ${text.ul(
          doc.li`Some kind of ${text.b`distribution algebra`}`,
          doc.li`
            For example a normal distribution can be passed around as a
            ${text.b`mean (𝜇)`} and a ${text.b`variance (𝜎)`}, the variance
...
export const someLeads = createDoc(() => container(
  ...
  dashbox(
    ...
    text.ol({ itemSpace: 'sparse' }).c(
      ...
      doc.li.c(doc.s`Quantile Regression`, doc.span`:
      ...
      `),
      doc.li`${text.b`Distributional Regression (GAMLSS)`}: Generalized Additive Models for Location`,
      doc.li`Kernel Density Methods`,
    ),
    doc.h4`Examples`,
    text.ul(
      doc.li.c(PDF_MODELS.rigby2005.t`GAMLSS from 2005`),
      doc.li.c(PDF_MODELS.fong2025.t`Temporal mixture density networks for enhanced investment modeling`),
      doc.li.c(PDF_MODELS.will2025.t`Exploring the Application of a Mixture Density Network to the Australian Housing Market`),
    ),
  ),
));
