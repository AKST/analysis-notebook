...
import { todo, infobox, defineTerm } from '../common/components.js';
import { container, twoThree, twoColumns } from '../common/layout.js';
...
const TERMS = {
  equilibrium: defineTerm({
    word: 'Equilibrium',
    source: 'https://en.wiktionary.org/wiki/equilibrium',
    meaning: [
...
/** @param {string} url @param {string} text @returns {E.Item} */
const makeLink = (url, text='Link') =>
  doc.frag('[', ['a', { href: url }, [text]], ']');

const LINKS = {
  privateGood: 'https://en.wikipedia.org/wiki/Private_good',
};
...
export const intro = {
  meta: { kind: 'document' },
  render: () => container(
    todo({ leadingText: 'TODO(Knob)' }, 'Absolute Price Control'),
    todo({ leadingText: 'TODO(Knob,Supply variant)' }, 'Make it so supply can be replace with a fixed price'),
    twoThree(
      container(
        ['h1', 'Equilibrium'],
        ['h4', 'Definition Lightning round'],
    ...
    ['div', [
      container(
        ...
        ['details', {}, [
          ...
          ['br'],
          IMAGES.paretoImprovingTransaction,
          container(doc.p(`
...
export const subjectPerfectCompetition = {
  ...
  render: () => container(
    twoThree(
      container(
        ...
        ['details', {}, [
          ...
          container(
            ...
            `, makeLink(LINKS.privateGood)),
          ),
        ]],
        ['h3', 'Devivations from Equilibrium'],
        doc.p(`
        ...
        `),
        doc.ul([
          ['A ', doc.b('Price ABOVE Equilbrium'), `,
            will result in `, doc.b('excessive SUPPLY'), `
            with an insufficent number of buyers.
          `],
          ['A ', doc.b('Price BELOW Equilbrium'), `,
            will result in `, doc.b('excessive DEMAND'), `
      ...
      container(
        ['h3', 'Perfect Competition Assumptions'],
        doc.dl([
...
export const introAggregateSupplyAndDemand = {
  ...
  render: (_ctx, state) => container(
    ...
    twoThree(
      container(
        ['h1', 'Aggregate Supply & Demand'],
        ['h2', 'Horizontal Summation'],
        doc.p(`
