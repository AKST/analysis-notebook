...
import { twoThree, fontsize, container } from '../common/layout.js';
...
export const elascity = {
  ...
  render: (_ctx, state, cfg) => common.dashbox(
    ...
    ['div', { className: 'c2' }, [
      ...
      container(
        doc.dl([
          ['Short run shut down condition', [doc.small`
            The Entrepreneur should shut down
            production if Profit < -FC
            (of Price < min AVC).
          `]],
          ['Long run shut down condition', [doc.small`
    ...
    ['div', { className: 'c2 twoThree' }, [
      container(
        ...
        doc.small`Here are some determinants of elasticity of supply`,
        doc.dl([
          ['Availablity of raw materials', [doc.small`
          ...
          `]],
          ['Inventory/Excess Capacity', [doc.small`
...
export const deriveAtcAndAvcFromMC = {
  meta: { kind: 'document' },
  render: (_ctx, state, cfg) => common.dashbox(
    ['h2', 'Partial ATC and AVC from MC'],
    twoThree(
      container(
        doc.small`
      ...
      common.infobox('Derive Parital ATC & AVC', [
