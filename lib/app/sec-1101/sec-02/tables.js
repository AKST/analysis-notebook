/**
 * @import { EconDiscrete, E } from '../../prelude-type.ts';
 */
...
/**
 * @param {EconDiscrete.FirmBehaviourTable} focus
 * @param {EconDiscrete.FirmBehaviourTable} other
  ...
  const getRows = function *() {;
    for (const [index, cost] of enumerate(focus.costTotal)) {
      ...
      yield [
        ...
        fmtNum(other.marginalBenefit[index] - focus.marginalBenefit[index]),
      ];
    }
  };
 ...
 * @param {EconDiscrete.FirmBehaviourTable} fTable
 * @returns {E.Item}
 */
...
export function shutdownConditions(fixedCost, fTable) {
  const shortRun = [false].concat(fTable.profit.map((profit) => {
    return !(profit < -fixedCost);
  }));

  const longRun = [false].concat(fTable.profit.map((profit) => {
    return !(profit < 0);
  }))
  ...
  const show = outcomes => {
    ...
    return outcomes.filter(it => it).join(', ');
  }
