import { mathml, mathml2 } from '../../prelude.js';
import { mathmlHelper2 } from '../prelude.js';

const {
  frac,
  space,
  table,
} = mathml;

const { mi, mo, mtext, mrow, mn, msup, msub, munder } = mathml2;

const { rows } = mathmlHelper2;

export function relativeValue() {
  const muai = msub(mi`MU`, mrow(mi`A`, space(2), mi`i`));
  const mubj = msub(mi`MU`, mrow(mi`B`, space(2), mi`j`));
  const pb = msub(mtext`p`, mi`B`);
  const ipb = msup(pb, mrow(mo`-`, mn(1)));

  return mathml2.math(
    rows(
      mrow(
        msub(mtext`Reserve Price`, mi`A`),
        space(4), mo`=`, space(4),
        frac([muai], [mubj, mo`×`, ipb]),
      ),
      mrow(),
      mrow(muai, mo`=`, mtext`Marginal Utility of A, at position i`),
      mrow(mubj, mo`=`, mtext`Marginal Utility of B, at position j`),
      mrow(pb, mo`=`, mtext`Price of B`),
    ),
  );
}

export function reservationPrice() {
  return mathml2.math(
    rows(
      mrow(
        mtext`Reserve Price`,
        space(4), mo`=`, space(4),
        frac([msub(mi`MU`, mi`i`)], [mi`λ`]),
      ),
      mrow(mi`λ`, mo`=`, mtext`Marginal Utility of Income`),
    ),
  );
}

export function reservationPriceCourse() {
  const indexA = mrow(
    space(4), mi`A`,
    space(4), mi`i`,
  );

  return mathml2.math(
    mrow(
      munder(
        msub(mtext`Reserve Price`, indexA),
        mrow(mi`i`, mo`>`, mn(0)),
      ),
      space(4), mo`=`, space(4),
      frac([
        msub(mi`MU`, indexA)
      ], [
        msup(mi`p`, mrow(mo`-`, mn(1))),
        space(4),
        msub(mi`MU`, mrow(mi`B`, space(2), mn(1))),
      ]),
    ),
  );
}

export function elasticity() {
  return mathml2.math(
    table([
      [mi`ε`, mo`=`, mrow(
        frac([mi`P`], [mi`Q`]), mo`×`,
        frac([mi`ΔQ`], [mi`ΔP`])
      )],
    ]),
  );
}
