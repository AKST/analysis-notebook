...
const { mi, mo, mtext, mrow, mover, msub, mspace, mfrac } = mathml2;
const { eq, minus, mul, matrix, rows, annotationOver, call } = mathmlHelper2;
  ...
  const topLine = (vName, desc, output) => mrow(
    mover(
      rows(
        ...
        eq(call.attr({ fn: mtext`OC` })(abcd), output),
      ),
