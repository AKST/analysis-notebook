...
const fn = {
  p: bindOf(call.attr({ fn: msub(mi`p`, mi`t`) })),
  q: bindOf(call.attr({ fn: msub(mi`q`, mi`t`) })),
  T: bindOf(call.attr({ fn: mi`T` })),
  s: bindOf(call.attr({ fn: msub(mi`s`, mi`h`) })),
  lambda: bindOf(call.attr({ fn: mi`𝜆` })),
  H: bindOf(call.attr({ fn: mi`H` })),
  u: bindOf(call.attr({ fn: mi`u` })),
  Omega: bindOf(call.attr({ fn: mi`Ω` })),
  log: bindOf(call.attr({ fn: mtext`log` })),
};
...
export const maximise = text.figure(
  mathml.math(
    mrow(
      mi`E`,
      sum({ inc: eq.sup(mi`t`, mn(0)), max: mi`∞` }).of(
        msup(beta, mi`t`),
        call.attr({ fn: mi`u` }).of(
          msub(mi`c`, mi`t`),
          msub(mi`h`, mi`t`),
          msub(mi`l`, mi`t`)
        ),
...
export const homeOwnerBudget = text.figure(
  mathml.math.attr({ className: 'wideMath' }).of(
    eq(
      add(
        msub(mi`c`, mi`t`),
        msub(mi`a`, add.sub(mi`t`, mn(1))),
...
export const individualStateValueFunction = text.figure(
  mathml.math(
    eq(
      call.attr({ fn: msub(mi`V`, mi`t`) }).of(Gamma),
      mul0(
        munder(
          mtext`max`,
          mrow(
        ...
        add(
          ...
          mul0(
            beta,
            msub(mi`E`, mi`t`),
            call.attr({ fn: msub(mi`V`, add.sub(mi`t`, mn(1))) }).of(
              msup(Gamma, mo`′`)
            )
          )
        ),
  ...
  mathml.math(
    rows(
      mrow(
        ...
        mo`=`,
        call.attr({ fn: msub(mi`H`, mi`i`) }).of(
          fn.p(msub(mi`h`, mi`i`))
        ),
        mo`,`
      ),
    ...
    eq(
      ...
      add.sub(
        ...
        mul0(
          munder(mi`∑`, Gamma),
          parensB(add.sub(
            call.attr({ fn: mi`κ` }).of(Gamma),
            call.attr({ fn: mi`di` }).of(Gamma)
          )),
          fn.lambda(Gamma)
        ),
        mul0(
...
export const populationDistribution = text.figure(
  mathml.math(
    rows(
      eq(
        call.attr({ fn: msup(mi`λ`, mo`′`) }).of(msup(Gamma, mo`′`)),
        mul0(
          munder(mi`∑`, Gamma),
          fn.Omega(mu, msup(mu, mo`′`)),
          call.attr({ fn: msub(mi`Ω`, mi`z`) }).of(mi`z`, msup(mi`z`, mo`′`)),
          call.attr({ fn: msub(mi`Ω`, mi`v`) }).of(msubsup(mi`i`, mrow(), mi`v`), msup(msubsup(mi`i`, mrow(), mi`v`), mo`′`)),
...
export const housingSupplyFunction = text.figure(
  mathml.math(
    eq(
      call.attr({ space: false, fn: mo`H` }).of(call.attr({ space: false, fn: mo`p` }).of(mi`h`)),
      mul0(
        msub(zeta, mn(2)),
