...
export const homeOwnerBudget = text.figure(
  mathml.math({ className: 'wideMath' }).c(
    eq(
      add(
        msub(mi`c`, mi`t`),
        msub(mi`a`, add.sub(mi`t`, mn(1))),
...
export const individualStateValueFunction = text.figure(
  mathml.math(
    eq(
      call({ fn: msub(mi`V`, mi`t`) }).c(Gamma),
      mul0(
        munder(
          mtext`max`,
          mrow(
        ...
        add(
          ...
          mul0(
            beta,
            msub(mi`E`, mi`t`),
            call({ fn: msub(mi`V`, add.sub(mi`t`, mn(1))) }).c(
              msup(Gamma, mo`′`)
            )
          )
        ),
    ...
    eq(
      ...
      add.sub(
        ...
        mul0(
          munder(mi`∑`, Gamma),
          parensB(add.sub(
            call({ fn: mi`κ` }).c(Gamma),
            call({ fn: mi`di` }).c(Gamma)
          )),
          fn.lambda(Gamma)
        ),
        mul0(
...
export const populationDistribution = text.figure(
  mathml.math(
    rows(
      eq(
        call({ fn: msup(mi`λ`, mo`′`) }).c(msup(Gamma, mo`′`)),
        mul0(
          munder(mi`∑`, Gamma),
          fn.Omega(mu, msup(mu, mo`′`)),
          call({ fn: msub(mi`Ω`, mi`z`) }).c(mi`z`, msup(mi`z`, mo`′`)),
          call({ fn: msub(mi`Ω`, mi`v`) }).c(msubsup(mi`i`, mrow(), mi`v`), msup(msubsup(mi`i`, mrow(), mi`v`), mo`′`)),
...
export const housingSupplyFunction = text.figure(
  mathml.math(
    eq(
      call({ space: false, fn: mo`H` }).c(call({ space: false, fn: mo`p` }).c(mi`h`)),
      mul0(
        msub(zeta, mn(2)),
