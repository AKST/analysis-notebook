...
const { multiscripts, call, rows, sum, abs, SPECIAL, parensA, table, inv, eqId, eq, add, div, minus, mul } = mathmlHelper_2;
...
export const normality = doc.figure(
  mathml.math.attr({ style: 'font-size: 16px' })(
    rows(
      mrow(
...
export const samplingDistribution = doc.figure(
  mathml.math.attr({ style: 'font-size: 14px' })(
    rows(
      mrow(
        ...
        munder(
          mrow(
            op.test === 'one-sided' ? mrow(
              multiscripts.attr({ sup: [mi`t`, undefined] })(param(mathml)),
...
export const fStatInTermsOfRSq = doc.figure(
  mathml.math.attr({ style: 'font-size: 10px' })(
    eq(
      eq(
        mi`F`,
        mfrac(
