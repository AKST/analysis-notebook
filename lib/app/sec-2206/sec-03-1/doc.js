...
export const fTest = createDoc(() => dashbox(
  ...
  container(
    ...
    doc.small`
      ...
      ${doc.b`F Statistic`} which can be broken down like so:${ulLight([
        doc.span`${doc.b`numerator`}: differences in residuals divided by the number of omitted.`,
        doc.span`${doc.b`demonator`}: unrestricted divided by normal degrees of freedom.`,
      ])}
    `,
    twoThree(
      doc.small`
  ...
  container(
    twoColumns(
      container(
        ...
        ulLightSm([
          doc.span`Do we have two seperate ways of testing hypothesis about a single coefficent?${ulLight([
            doc.span`
