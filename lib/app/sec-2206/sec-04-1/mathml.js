...
const { mi, mo, mtext, mrow, mover, mn, msup, msub, msubsup, msqrt, munder } = mathml2;
...
export const consistency = {
  ...
  probabilityLimits: doc.figure(
    mathml2.math(
      mathml.table([
        [
          mrow(mi`plim`, space(4), msub(SPECIAL.greek.circumflex.beta, mn(1))),
          mo`=`,
          add(
            msub(SPECIAL.greek.beta, mn(1)),
            divP(
              mathml.call('Cov', mrow(msub(mi`x`, mn(1)), mo`,`, space(4), mi`u`)),
  ...
  strongerMLR4_1: doc.figure(
    mathml2.math(
      mrow(
        ...
        space(16),
        eq(
          mathml.call('Cov', msub(mi`x`, mi`j`), mo`,`, space(4), mi`u`),
          mn(0)
        ),
        mo`,`,
        space(4),
