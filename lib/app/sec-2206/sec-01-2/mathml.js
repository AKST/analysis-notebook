...
export const jointProbabilityDistributionOfXAndYInPop = {
  p2_12: mathml([eq(E(minus(vars.y, vars.b0, mul(vars.b1, vars.x))), mn(0))]),
  p2_13: mathml([eq(
    mrow(
      mi`E`,
...
export const sumOfSquares = {
  sst: mathml([eqId(mi`SST`, summaise(msup(minusPA(vars.yi, vars.ysm), mn(2))))]),
  sse: mathml([eqId(mi`SSE`, summaise(msup(minusPA(vars.yEstI, vars.ysm), mn(2))))]),
  ssr: mathml([eqId(mi`SSR`, summaise(msubsup(vars.uEst, mi`i`, mn(2))))]),
  relation: mathml([eq(mi`SST`, add(mi`SSE`, mi`SSR`))]),
};
...
export const slrAssumptions = {
  ...
  slr4: mathml([eq(E(cond(vars.u, vars.x)), mn(0))]),
