...
const {
  eqId, eq, add, mul0: mul, mul0PB: mulPB, rows, parensA, parensC,
  minus, minusPA, minusPB, sum, inv, table, SPECIAL,
} = mathmlHelper;
...
export const populationRegressionFunction = mathml([
  eq(E(vars.y, o('|'), vars.x), add(vars.b0, mul(vars.b1, vars.x))),
]);
...
export const indexSyntax = mathml([
  row([
    text('Let'), space(8),
    parensC.of(
      parensA.of(vars.xi, o(','), vars.yi), o(':'), space(8),
      n(1), o(','),
  ...
  varB1EstC: mathml([
    table({ columnalign: 'right center left center left' }).of(
      [
        call('Var', vars.b1Est),
        o('='), mul(
          sup(parensA.of(inv.of(sub(i('SST'), i('x')))), n(2)),
          call('Var', summaise(mul(vars.ui, minusPA(vars.xi, vars.xsm)))),
        ),
        o('='), mul(
          sup(parensA.of(inv.of(sub(i('SST'), i('x')))), n(2)),
          mul(
      ...
      [
        space(0),
        o('='), mul(
          sup(parensA.of(inv.of(sub(i('SST'), vars.x))), n(2)),
          summaise(
            sup(sigma, n(2)),
            sup(minusPA(vars.xi, vars.xsm), n(2)),
          ),
