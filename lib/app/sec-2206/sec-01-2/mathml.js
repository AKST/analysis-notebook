...
export const simpleLinearRegresion = mathml2.math(
  eq(vars.y, add(vars.b0, frag([vars.b1, vars.x]), vars.u)),
);

export const simpleLinearRegresionSample = mathml2.math(
  eq(vars.yi, add(vars.b0, mul(vars.b1, vars.xi), vars.ui)),
);

export const xyFunctionalRelationship = mathml2.math(
  eq(frag([Delta, vars.y]), frag([vars.b1, Delta, vars.x])),
  mspace(16), mtext`if`, mspace(16),
  eq(frag([Delta, vars.u]), mn(0)),
);

export const assumeExpectedUToBe0 = mathml2.math(
  eq(E(vars.u), mn(0)),
);

export const assumeExpectedUDoesNotDependOnX = mathml2.math(
  eq(E(cond(vars.u, vars.x)), E(vars.u)),
);

export const assumeZeroConditionalMean = mathml2.math(
  eq(E(cond(vars.u, vars.x)), mn(0)),
);

export const populationRegressionFunction = mathml2.math(
  eq(E(cond(vars.y, vars.x)), add(vars.b0, mul(vars.b1, vars.x))),
);

export const populationRegressionFunctionSimple = mathml2.math(
  eq(vars.yi, add(vars.b0, mul(vars.b1, vars.xi), vars.ui)),
);

export const indexSyntax = mathml2.math(
  mrow(
    mtext`Let`, mspace(8),
    parensC(
...
export const idealProperitesOfUnobserved = {
  p2_10: mathml2.math(eq(E(mi`u`), mn(0))),
  p2_11: mathml2.math(
...
export const jointProbabilityDistributionOfXAndYInPop = {
  p2_12: mathml2.math(eq(E(minus(vars.y, vars.b0, mul(vars.b1, vars.x))), mn(0))),
  p2_13: mathml2.math(eq(
    mrow(
      mi`E`,
...
export const sumOfSquares = {
  sst: mathml2.math(eqId(mi`SST`, summaise(msup(minusPA(vars.yi, vars.ysm), mn(2))))),
  sse: mathml2.math(eqId(mi`SSE`, summaise(msup(minusPA(vars.yEstI, vars.ysm), mn(2))))),
  ssr: mathml2.math(eqId(mi`SSR`, summaise(msubsup(vars.uEst, mi`i`, mn(2))))),
  relation: mathml2.math(eq(mi`SST`, add(mi`SSE`, mi`SSR`))),
};

export const rSquared = mathml2.math(
...
export const rSquareFull = mathml2.math(
  eq(
    msup(mi`R`, mn(2)),
    mfrac(
      msup(
...
export const whatIsOrdinaryLeastSquares = mathml2.math(
  eq(
    mrow(
      mtext`min`,
      mspace(4),
...
export const slrAssumptions = {
  slr3: mathml2.math(Range(parensC(
    vars.xi, mo`:`, mspace(4),
    eq(mi`i`, frag([mn(1), SPECIAL.ellipse.h, mi`n`])),
  )), mspace(4), mo`>`, mspace(4), mn(0)),
  slr4: mathml2.math(eq(E(cond(vars.u, vars.x)), mn(0))),
  slr5: mathml2.math(eq(Var(cond(vars.u, vars.x)), vars.x)),
};

export const simpleLinearRegressionAnalysis = {
  varB1EstA: mathml2.math(
    eq(
      Var(vars.b1Est),
