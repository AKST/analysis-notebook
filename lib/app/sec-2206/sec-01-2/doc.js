...
const { text, infobox_2, dashbox, note, todo } = prelude.components;
...
const INFOBOX = {
  populationParamaters: infobox_2({ title: 'Population Parameters' })(
    doc.p`
  ...
  noteOnIndexSyntax: infobox_2({ title: 'Note on Index Syntax' })(
    doc.p`
  ...
  propertiesOfSummation: infobox_2({ title: 'Properties of Summation' })(
    todo({}, 'Write me later'),
  ),
  betaOneIsARandomVariable: infobox_2({ title: 'β̂₁ is a random variable' })(
    doc.p`
...
export const linearRegression = createDoc(() => dashbox(
  ...
  twoThree(
    container(
      doc.figure(mathml.simpleLinearRegresion, doc.figcaption`Simple Linear regression`),
      doc.p`${doc.b`u`}
  ...
  twoColumns(
    container(
      ...
      doc.figure(mathml.xyFunctionalRelationship, doc.figcaption`Linear effect requirement`),
      doc.p`
    ...
    container(
      ...
      doc.figure(mathml.assumeExpectedUToBe0, doc.figcaption`Average u in the population given β₀`),
      doc.p`
  ...
  container(
    ...
    doc.figure(mathml.assumeExpectedUDoesNotDependOnX, doc.figcaption`Assume U does not depend on X`),
    doc.p`
    ...
    `,
    doc.figure(mathml.assumeZeroConditionalMean, doc.figcaption`Zero Conditional mean assumption`),
  ),
  ['hr'],
  container(
    doc.h4`Population regression function (PRF)`,
    responsiveGridAutoRow([
      doc.figure(mathml.populationRegressionFunction, doc.figcaption`The PRF`),
      doc.figure(mathml.populationRegressionFunctionSimple, doc.figcaption`Also the PRF`),
...
export const ordinaryLeastSquaresEstimate = createDoc(() => dashbox(
  ...
  twoThree(
    container(
      ...
      `,
      doc.figure(mathml.simpleLinearRegresionSample, doc.figcaption`Sample linear regression`),
      doc.p`
    ...
    container(
      ...
      doc.figure(mathml.idealProperitesOfUnobserved.p2_10, doc.figcaption`mean unobserved is zero`),
      doc.figure(mathml.idealProperitesOfUnobserved.p2_11, doc.figcaption`covariance between u & x is zero`),
    ),
  ),
  ...
  container(
    ...
    twoColumns(
      doc.figure(mathml.betaOneEstimation.rearrangingAbove, doc.figcaption`
        Start by plugging β̂₀ into${['br']}
        the Sum of 1st Order Condition
      `),
      doc.figure(mathml.betaOneEstimation.alsoNotingThat, doc.figcaption`
    ...
    `,
    responsiveGridAutoRow([
      doc.figure(mathml.betaOneEstimation.assumption, doc.figcaption`Given`),
      doc.figure(mathml.betaOneEstimation.betaOneOLS, doc.figcaption`Estimating β̂₁, the OLS slope`),
      doc.figure(mathml.betaOneEstimation.betaOneOLSSimplied, doc.figcaption`Simpified β̂₁`),
      doc.figure(mathml.betaOneEstimation.populationBetaOneOLSSimplied, doc.figcaption`Population β₁`),
    ], {
      columns: {
  ...
  container(
    ...
    `,
    responsiveGridAutoRow([
      doc.figure(mathml.fittedValueOfYi, doc.figcaption`Fitted value of ŷᵢ`),
      doc.figure(mathml.fittedValueResiduals, doc.figcaption`Computing residual ûᵢ`),
      doc.figure(mathml.sampleRegressionFunction, doc.figcaption`Sample Regression Function`),
    ], {
      columns: {
  ...
  container(
    ...
    `,
    responsiveGridAutoRow([
      doc.figure(mathml.sumOfSquares.sst, doc.figcaption`Total Sum of Squares`),
      doc.figure(mathml.sumOfSquares.sse, doc.figcaption`Explained Sum of Squares`),
      doc.figure(mathml.sumOfSquares.ssr, doc.figcaption`Residual Sum of Squares`),
    ], {
      columns: {
    ...
    `,
    responsiveGridAutoRow([
      doc.figure(mathml.sumOfSquares.relation, doc.figcaption`Sum of Squares constraint`),
      doc.figure(mathml.rSquared, doc.figcaption`R Squared`),
    ], {
      columns: {
        desktop: 'auto auto',
      },
    }),
    doc.figure(mathml.rSquareFull, doc.figcaption`R Square Expanded`),
  ),
  container(
...
export const assumptionsOfUnbiasednessInOLS = createDoc(() => dashbox(
  ...
  twoColumns(
    container(
      ...
      doc.figure(mathml.slrAssumptions.slr3, doc.figcaption`SLR 3`),
    ),
    container(
      doc.h4`SLR 4 - Zero Conditional Mean`,
      doc.p`The error has an expected value of zero for any given explanatory variable`,
      doc.figure(mathml.slrAssumptions.slr4, doc.figcaption`SLR 4`),
    ),
  ),
  container(
    ...
    doc.p`
      ...
      error rate.
    `,
    doc.figure(mathml.slrAssumptions.slr5, doc.figcaption`SLR 5`),
  ),
  container(
  ...
  container(
    ...
    twoColumns(
      doc.figure(mathml.simpleLinearRegressionAnalysis.varB1EstA, doc.figcaption`Constraint A, Sampling variance of estimators`),
      doc.figure(mathml.simpleLinearRegressionAnalysis.varB1EstB, doc.figcaption`Constraint B, Sampling variance of estimators`),
    ),
    doc.figure(
