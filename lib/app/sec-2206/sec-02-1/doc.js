...
const LINKS = {
  ...
  papers: {
    randomEffectsModels: doc.a({ href: 'https://www.jstor.org/stable/2529876' }),
  },
  yt: {
    mixedEffects: doc.a({ href: 'https://www.youtube.com/watch?v=MOyjGnjAols' }),
    logitModels: doc.a({ href: 'https://www.youtube.com/watch?v=iQNRzHKjFw0' }),
  },
};
...
export const expectedOLSEstimatorsVariance = createDoc(() => container(
  infobox('What is J?', [container(
    ...
    ulLight([
      doc.span`N denotes the size of a sample or population`,
      doc.span`K denotes the number of variables in a dataset or model`,
      doc.span`I subscript denotes observation, e.g. observation № I out N`,
      doc.span`J subscript denotes variables, e.g. variable № J out K`,
    ]),
  )]),
  dashbox(
    ...
    container(
      ...
      twoColumns(
        container(
          ...
          doc.small`
            From the above equation ${doc.b`a large 𝜎²`} means ${doc.b`larger sampling variances`}
            for the OLS Estimators. Which ${doc.b`put another way means more noise`}.
          `,
          doc.small`
            ${doc.b`Note`}, because 𝜎² is a population feature it has nothing to do with
      ...
      container(
        ...
        twoThree(
          container(
            ulLightSm([
              doc.span`Obtaining ${vars.RJ}, ${ulLight([
                doc.span`Given a model explain y from K independent variables`,
                doc.span`Create a new model to explain the Jth independent variable using the remaining variables`,
                doc.span`Compute the ${vars.R2} for that model`,
                doc.span`${vars.RJ} does not appear in SLR, due to need for additional variables`,
              ])}`,
              doc.span`Understanding ${vars.RJ}, ${ulLight([
                doc.span`Given a model explain y from 2 independent variables`,
                doc.span`Compute the R₂² (${vars.RJ} of x₂), with this regression:${ulLight([
                  doc.span`x₂ = 𝛽₀ + 𝛽₁x₁ + u`,
                ])}`,
                doc.span`A value of R₂² close to 1 means:${ulLight([
                  doc.span`Most of the variance in x₂ is explained by x₁`,
                  doc.span`x₂ and x₁ are highly correlated.`,
                ])}`,
                doc.span`As R₂² gets closer to 1 Var(𝛽̂₂) gets larger:${ulLight([
                  doc.span`This suggests a high degree of linearity between x₁ and x₂`,
                  doc.span`This describes multicollinearity`,
                ])}`,
              ])}`,
            ]),
            doc.small`
        ...
        dashbox(doc.details.of(
          ...
          container(
            doc.small`
              ...
              relationship, as you'd be:${ulLight([
                doc.span`estimating (predicted y) values`,
                doc.span`based on an estimate (coefficent on the residual)`,
                doc.span`derived from an estimation (xⱼ residual)`,
                doc.span`derived from estimations (estimated xⱼ)`,
                doc.span`from an estimation (estimated coefficent for the model to explain xⱼ with the other independent variables)`,
              ])}
            `,
            doc.small`
...
export const varianceInMisspecifedModels = createDoc(() => dashbox(
  container(
    ...
    ulLightSm([
      doc.span`When 𝛽₂ = 0, 𝛽̃₁ and 𝛽̂₁ are both is unbiased and Var(𝛽̃₁) < Var(𝛽̂₁) ${ulLight([
        doc.span`In this case 𝛽̃₁ is preferable`,
      ])}`,
      doc.span`When 𝛽₂ ≠ 0, 𝛽̃₁ is biased, 𝛽̂₁ is unbiased and Var(𝛽̃₁) < Var(𝛽̂₁) ${ulLight([
        doc.span`Resolving this scenario is less obvious`,
        doc.span`However as the sample size grows ${ulLight([
          doc.span`The bias in 𝛽̃₁ does not shrink, nor does it really follow any pattern`,
          doc.span`However both Var(𝛽̃₁) and Var(𝛽̂₁) will converge to zero`,
          doc.span`The size of error term will increase`,
        ])}`,
      ])}`,
    ]),
  ),
...
export const estimatingStandardErrorOfOLSEstimators = createDoc(() => dashbox(
  container(
    ref({ page: 94 }, doc.h3`Estimating 𝜎²: Standard Errors of the OLS Estimators`),
    responsiveGridAutoRow([
...
export const beyondTheScope = createDoc(() => dashbox(
  ...
  columns('even-2', [
    doc.h3`Logit Models`,
    doc.small`You can ${LINKS.yt.logitModels`watch a video on this here`}.`,
    ...
    doc.small`
      You can ${LINKS.yt.mixedEffects`watch a video on this here`}, and read
      a paper on its applications ${LINKS.papers.randomEffectsModels`here`}.
    `,
    doc.h4`Why its useful`,
    doc.small`
