  ...
  twoThree(
    ...
    infobox('Resources', [
      text.ul.compact.of(
        ...
        doc.li.of(LINKS.gaussMarkovTheorem`Gauss Marko Theorem`),
        doc.li.of(LINKS.frischWaughLovell`Frisch–Waugh–Lovell theorem`),
      ),
    ]),
  ),
));
...
export const mechanicsOfOLS = createDoc(() => dashbox(
  ...
  twoColumns(
    ...
    container(
      ...
      `,
      text.ul.compact.of(
        doc.li`If the partial effect on ŷ is 0, then 𝛽̂₂ is 0`,
        doc.li`If x₁ and x₂ are uncorrelated then 𝛿̂₁ is 0`,
      ),
      doc.quote`
...
export const expectedOLSEstimatorsIrrelevantVars = createDoc(() => dashbox(
  ...
  container(
    ...
    text.ul.compact.of(
      doc.li`Say you make a model ${text.ul.compact.of(
        doc.li.of(doc.b`Y = θ0 + θ1*X1 + θ2*X2 + θ3*X3 + V`),
      )}`,
      doc.li`Say the True model is ${text.ul.compact.of(
        doc.li.of(doc.b`Y = B0 + B1*X1 + B2*X2 + U`),
      )}`,
      doc.li`At the very least there won't be a bias${text.ul.compact.of(
    ...
    container(
      ...
      container(
        ...
        twoThree(
          container(
            text.ul.compact.of(
              doc.li`Obtaining ${vars.RJ}, ${text.ul.compact.of(
              ...
              doc.li`Understanding ${vars.RJ}, ${text.ul.compact.of(
                doc.li`Given a model explain y from 2 independent variables`,
                doc.li`Compute the R₂² (${vars.RJ} of x₂), with this regression:${text.ul.compact.of(
                  doc.li`x₂ = 𝛽₀ + 𝛽₁x₁ + u`,
                )}`,
                doc.li`A value of R₂² close to 1 means:${text.ul.compact.of(
                  doc.li`Most of the variance in x₂ is explained by x₁`,
                  doc.li`x₂ and x₁ are highly correlated.`,
                )}`,
                doc.li`As R₂² gets closer to 1 Var(𝛽̂₂) gets larger:${text.ul.compact.of(
...
export const varianceInMisspecifedModels = createDoc(() => dashbox(
  container(
    ...
    text.ul.compact.of(
      doc.li`When 𝛽₂ = 0, 𝛽̃₁ and 𝛽̂₁ are both is unbiased and Var(𝛽̃₁) < Var(𝛽̂₁) ${text.ul.compact.of(
        doc.li`In this case 𝛽̃₁ is preferable`,
      )}`,
      doc.li`When 𝛽₂ ≠ 0, 𝛽̃₁ is biased, 𝛽̂₁ is unbiased and Var(𝛽̃₁) < Var(𝛽̂₁) ${text.ul.compact.of(
        doc.li`Resolving this scenario is less obvious`,
        doc.li`However as the sample size grows ${text.ul.compact.of(
          doc.li`The bias in 𝛽̃₁ does not shrink, nor does it really follow any pattern`,
          doc.li`However both Var(𝛽̃₁) and Var(𝛽̂₁) will converge to zero`,
...
export const beyondTheScope = createDoc(() => dashbox(
  ...
  columns('even-2', [
    doc.h3`Logit Models`,
    doc.p`You can ${LINKS.yt.logitModels`watch a video on this here`}.`,
