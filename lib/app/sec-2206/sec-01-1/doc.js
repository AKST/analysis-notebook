...
const LINKS = {
  welcomeToStata: (
    ['a', { href: 'https://www.princeton.edu/~otorres/Stata/' }, ['Welcome to Stata']]
  ),
};
  ...
  twoThree(
    ...
    infobox_2({ title: 'Resources' })(
      ...
      text.ul(
        ...
        doc.li.of(LINKS.welcomeToStata),
      ),
    ),
  ),
));
...
export const about = createDoc(() => dashbox(
  ...
  doc.h3`Type of data`,
  text.ul(
    doc.li`${doc.b`Nonexperimental data`}:
    ...
    `,
    doc.li`${doc.b`Experimental data`}: collected under laboratory conditions,
  ...
  text.ul(
    ...
    doc.li`Other times it may mean constructing a formal ${doc.b`Economic model`}.`,
  ),
...
const crossSectionalData = container(
  ...
  doc.p`A ${doc.b`Cross sectional`} dataset consists
