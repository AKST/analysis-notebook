  ...
  twoThree(
    ...
    infobox({ title: 'Resources' }).of(
      ...
      text.ul(
        ...
        doc.li.of(LINKS.laboursDecliningShareOfIncome`Labor's Declining Share of Income`),
        doc.li.of(LINKS.wiki.solowSwan`Wikipedia Solow Swan`),
        doc.li.of(LINKS.data.worldBank`World Bank Data`),
        doc.li.of(LINKS.data.pennWorldTable`Penn World Table`, ': data for s̄, d̄, Ā, ᾱ'),
      ),
    ),
  ),
));
...
export const solvingTheModel = createDoc(() => dashbox(
  ...
  twoThree(
    ...
    container(
      ...
      infobox({ title: 'Growth Transition Dynamics' }).of(
        doc.p`
    ...
    twoColumns(
      container(
        doc.p`
          ...
          or Output from either ${doc.i`(even if capital in an
        ...
        `,
        doc.p`
          ${doc.b`What you can do however`}, is use a method called
          the ${LINKS.wiki.ralphNewton`Newton Ralphson method`}.
          Which involves a bit of calculus. ${doc.b`Assuming you already
        ...
        text.ol(
          ...
          doc.li`Solve for the value that converges to ${doc.b`0`}.`,
        ),
      ),
      container(
