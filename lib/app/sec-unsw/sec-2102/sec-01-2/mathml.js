...
export const newtonRalphMethod = text.figure(
  math({ style: '--fontsize-math-m: 14px' }).c(
    table({ columnalign: 'right center left' }).c(
      ...
      eq.array(
        call({ fn: msup(mi`f`, mo`′`) }).c(msub(mi`K`, mi`t`)),
        minus(
          mn(1),
          mul0(
            mul0(mi`s̄`, mi`Ā`),
      ...
      eq.array(
        ...
        minus(
          ...
          mfrac(
            call({ fn: mi`f` }).c(msub(mi`K`, op.comma(mi`t`, mi`n`))),
            call({ fn: msup(mi`f`, mo`′`) }).c(msub(mi`K`, op.comma(mi`t`, mi`n`)))
          )
        )
      ),
