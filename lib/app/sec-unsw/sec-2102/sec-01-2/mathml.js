...
export const newtonRalphMethod = doc.figure(
  math({ style: '--fontsize-math-m: 14px' })(
    table.attr({ columnalign: 'right center left' })(
      ...
      eq.array(
        call.attr({ fn: mi`f` })(msub(mi`K`, mi`t`)),
        minus(
          msub(mi`K`, mi`t`),
          mul0(
            msub(mi`K`, minus.sub(mi`t`, mn(1))),
      ...
      eq.array(
        call.attr({ fn: msup(mi`f`, mo`′`) })(msub(mi`K`, mi`t`)),
        minus(
          mn(1),
          mul0(
            mul0(mi`s̄`, mi`Ā`),
      ...
      eq.array(
        ...
        minus(
          ...
          mfrac(
            call.attr({ fn: mi`f` })(msub(mi`K`, op.comma(mi`t`, mi`n`))),
            call.attr({ fn: msup(mi`f`, mo`′`) })(msub(mi`K`, op.comma(mi`t`, mi`n`)))
          )
        )
      ),
      ...
      [mrow(), mrow(), lt(
        abs(minus(
          msubsup(mi`K`, mi`t`, add.sup(mi`n`, mn(1))),
          msubsup(mi`K`, mi`t`, mi`n`)
        )),
        mi`ε`
      )],
      ...
      eq.array(
        mi`ε`,
        mtext`tolerance for sufficiently small difference`
      ),
    )
  ),
