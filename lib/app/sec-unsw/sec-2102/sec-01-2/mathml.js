...
export const newtonRalphMethod = text.figure(
  math({ style: '--fontsize-math-m: 14px' }).c(
    table.attr({ columnalign: 'right center left' }).c(
      ...
      eq.array(
        call.attr({ fn: mi`f` }).c(msub(mi`K`, mi`t`)),
        minus(
          msub(mi`K`, mi`t`),
          mul0(
            msub(mi`K`, minus.sub(mi`t`, mn(1))),
      ...
      eq.array(
        call.attr({ fn: msup(mi`f`, mo`′`) }).c(msub(mi`K`, mi`t`)),
        minus(
          mn(1),
          mul0(
            mul0(mi`s̄`, mi`Ā`),
      ...
      eq.array(
        ...
        minus(
          ...
          mfrac(
            call.attr({ fn: mi`f` }).c(msub(mi`K`, op.comma(mi`t`, mi`n`))),
            call.attr({ fn: msup(mi`f`, mo`′`) }).c(msub(mi`K`, op.comma(mi`t`, mi`n`)))
          )
        )
      ),
