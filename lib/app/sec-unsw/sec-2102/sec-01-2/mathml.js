...
import { SPECIAL, neg, rows, parensA, call, op, table, abs } from '@prelude-uni/mathml.js';

const { math, mi, mo, mtext, mrow, mover, mn, msub, msup, msubsup, munder, mspace, mfrac } = mathml;
const { eq, eqId, imply, add, mul0, minus, mul, lt } = op;
...
const K = bindOf(call.attr({ fn: mi`K` }));
const ratio = mfrac(
  mul0(vars.iFactor, vars.tech, msup(vars.labour, alpha)),
  vars.depreciation,
);
...
export const continiousK = mathml.math(
  eq(
    ...
    msup(
      add.square(
        mul(
          minus.paren(msubsup(mi`K`, mn(0), alpha), ratio),
          msup(mi`e`, mul.sup(neg(alpha), vars.depreciation, mi`t`)),
        ),
        ratio,
      ),
      mfrac(mn(1), alpha),
...
export const newtonRalphMethod = text.figure(
  math({ style: '--fontsize-math-m: 14px' }).of(
    table.attr({ columnalign: 'right center left' }).of(
      eq.array(
        ...
        add(
          mul0(
            msub(mi`K`, minus.sub(mi`t`, mn(1))),
            minus.paren(mn(1), mi`d̄`)
          ),
          mul0(mi`s̄`, msub(mi`Y`, mi`t`))
        )
      ),
      ...
      eq.array(
        ...
        add(
          ...
          mul0(
            mul0(mi`s̄`, mi`Ā`),
            msubsup(mi`K`, mi`t`, minus.paren(mn(1), alpha)),
            msup(mi`L̄`, alpha)
          )
        )
      ),
      ...
      eq.array(
        call.attr({ fn: mi`f` }).of(msub(mi`K`, mi`t`)),
        minus(
          msub(mi`K`, mi`t`),
          mul0(
            msub(mi`K`, minus.sub(mi`t`, mn(1))),
      ...
      eq.array(
        call.attr({ fn: msup(mi`f`, mo`′`) }).of(msub(mi`K`, mi`t`)),
        minus(
          mn(1),
          mul0(
            mul0(mi`s̄`, mi`Ā`),
            minus.paren(mn(1), alpha),
            msubsup(mi`K`, mi`t`, neg(alpha)),
            msup(mi`L̄`, alpha)
          ),
        )
      ),
      ...
      eq.array(
        msub(mi`K`, op.comma(mi`t`, add.sup(mi`n`, mn(1)))),
        minus(
          msub(mi`K`, op.comma(mi`t`, mi`n`)),
          mfrac(
            call.attr({ fn: mi`f` }).of(msub(mi`K`, op.comma(mi`t`, mi`n`))),
            call.attr({ fn: msup(mi`f`, mo`′`) }).of(msub(mi`K`, op.comma(mi`t`, mi`n`)))
          )
        )
      ),
      ...
      eq.array(mi`ε`, mtext`tolerance for sufficiently small difference`),
      eq.array(mi`n`, mtext`Newton ralph Iteration`),
    )
  ),
  doc.figcaption`Solving Kₜ using the Ralph Newton method`,
);
