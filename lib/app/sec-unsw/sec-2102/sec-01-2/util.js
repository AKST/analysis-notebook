 ...
 */
import { Unreachable, v2, math } from '@app/prelude.js';
...
function calibrateDynamics(variant, config, now) {
  ...
  const periods = timeTillK(model, capitalZero + (steadyState - capitalZero) * 0.9999)

  /** @type {t.TrajectoryMap} */
  const trajectory = {
...
/**
 * @param {t.TrajectoryMap} map
 * @param {t.CameraMap} cameraMap
 * @returns {t.RenderedTrajectoryMap}
 */
function projectMap(map, cameraMap) {
  /** @param {t.CameraTransform} camera */
  ...
  return {
    ...
    outputByK: map.output.map(projectPoint(cameraMap.capital)),
  };
}

/**
 * @param {t.TrajectoryMap} map
 * @param {t.TrajectoryMap | undefined} mmap
 */
...
export function getCamera(map, mmap) {
  /** @param {Vec2[]} vectors @returns {t.CameraTransform} */
  function getCameraForVectors(vectors) {
    ...
    const bounds = math.el.mul(v2(xMax, yRng), v2(1.2, 1.05));
...
export function solveCapitalAtT(model, t) {
  ...
  // K(t) = [(K0^⍺ - ((s̄ĀL̄^𝛼)/d̄))e^(-⍺d̄t) + ((s̄ĀL̄^𝛼)/d̄)]^(1/𝛼)
  const yish = (s * A * (L**a))/d;
  const k0 = model.capitalZero;
  return ((((k0**a) - yish) * Math.exp(-a * d * t)) + yish)**(1/a);
}
