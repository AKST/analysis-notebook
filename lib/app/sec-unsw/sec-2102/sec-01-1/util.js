...
export function getConfig() {
  return {
    production: {
      ...
      group: {
        technology: { kind: 'number', range: [0, 10000], label: 'Technology (A)' },
        capital: { kind: 'number', range: [0, 10**6], label: 'Capital (K)' },
        labour: { kind: 'number', range: [0, 10**6], label: 'Labour (L)' },
        alpha: { kind: 'number', range: [0, 1], label: 'Alpha (α)' },
      },
    },
    ces: {
      kind: 'group',
      ...
      group: {
        rho: { kind: 'number', range: [-12, 1], label: 'Rho (ρ)' },
        xi: { kind: 'number', range: [-1, 10], label: 'Xi (ξ)' },
      },
    },
  }
}
