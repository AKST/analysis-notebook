...
export const modelOfProduction = createDoc(() => dashbox(
  ...
  doc.p`
    ...
    to ${doc.i`model`} macro economic problems. This model
    will help us address questions like, why are Americans
    X times richer than ${doc.i`[insert other nation]`}.
  `,
...
export const cesProductionFunction = createDoc(() => dashbox(
  ...
  doc.p`
    ...
    states as well, which tends to vary with ${doc.b`ρ`}.
  `,
  text.ul(
    doc.li`${doc.b`ρ = 1`}: it resembles a flat plane, α rotates it`,
    doc.li`${doc.b`ρ > 0`}: it Begins to flatten as it approaches 1`,
    doc.li`${doc.b`ρ = 0`}: it acts like Cobb Douglas`,
    doc.li`${doc.b`ρ → -∞`}: it forms a Leontief production function, as it approach negative inifinity`,
  ),
));
