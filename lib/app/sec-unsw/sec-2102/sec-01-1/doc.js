...
const LINKS = {
  tradingEconomics: text.a({ href: 'https://tradingeconomics.com' }).t`Trading Economics`,
  tutorialNotes: text.a({ href: 'https://www.desmos.com/calculator/epl08s8qwm' }).t`Desmos`,
  fred: text.a({ href: 'https://fred.stlouisfed.org' }).t`FRED`,
  wiki: {
    cobbDouglas: text.a({ href: 'https://en.wikipedia.org/wiki/Cobb–Douglas_production_function' }),
    leontief: text.a({ href: 'https://en.wikipedia.org/wiki/Leontief_production_function' }),
    ces: text.a({ href: 'https://en.wikipedia.org/wiki/Constant_elasticity_of_substitution' }),
  },
};

const INFOBOXES = {
  returnsToScale: infobox({ title: 'Returns To Scale' }).c(
    doc.p`${text.b`Constant returns to scale`}:
    ...
    `,
    doc.p`${text.b`Increasing returns to scale`}:
      ...
      more than double.
    `,
    doc.p`${text.b`Decreasing returns to scale`}:
      This is when the sum of the exponent coeffients
  ...
  twoThree(
    container(
      doc.h1`
        Macroeconomics 2
        ${doc.br()}
        ${doc.small({ style: 'color: #6666ff' }).t`Aggregate Production`}
        ${doc.br()}
        ${doc.small({ style: 'color: #aaaaff' }).t`ECON2102, W1, Lecture 1`}
      `,
    ),
...
export const modelOfProduction = createDoc(() => dashbox(
  ...
  doc.p`
    ...
    revision on the ${LINKS.wiki.cobbDouglas.t`Cobb Douglas`}
    production function, in addition to a model we can use
    to ${text.i`model`} macro economic problems. This model
    will help us address questions like, why are Americans
    X times richer than ${text.i`[insert other nation]`}.
  `,
  doc.h3`Production Function`,
  mathml.production.generalForm,
  doc.p`This is the general form of the production function.`,
  text.ul(
    doc.li`${text.b`Y`}: output or ${text.b`GDP`}`,
    doc.li`${text.b`K`}: phyisical capital, machines, equipment, structures, tools`,
    doc.li`${text.b`L`}: labour (number of workers or hours or work)`,
    doc.li`${text.b`A`}: technology (a scale of factor)`,
  ),
  doc.h4`Cobb Douglas`,
  mathml.production.cobbDouglas,
  doc.blockquote`
    The Australia Government has a special name for Ā,
    being ${text.b`total factor productivity`}.
  `,
  doc.p`
...
export const deminishingReturns = createDoc(() => container(
  twoThree(
    dashbox(
...
export const marginalProducts = createDoc(() => dashbox(
  ...
  text.ul(
    doc.li`${text.b`MPK`}: Marginal Product of Capital`,
    doc.li`${text.b`MPL`}: Marginal Product of Labour`,
  ),
...
export const modelOfFirmsBehaviour = createDoc(() => dashbox(
  ...
  text.ul(
    doc.li`${text.b`w`}: real wage paid to labour`,
    doc.li`${text.b`r`}: real rental paid to capital`,
    doc.li`${text.b`Π`}: real profits of a typical firm`,
  ),
  doc.h4`Profits for typical firm`,
  mathml.firms.profits,
  text.ul(
    doc.li`${text.b`Assume`} firms will profit maximise`,
    doc.li`${text.b`Assume`} firms act in competitive manner, take w & r as given.`,
    doc.li`${text.b`Assume`} firms are perfectly competitive.`,
    doc.li`
  ...
  twoColumns(
    container(
      ...
      text.ul(
        doc.li`Has a cost of ${text.b`w`}.`,
        doc.li`Has a benefit of ${text.b`MPL`}.`,
      ),
    ),
    container(
      doc.p`Adding a unit of Capital`,
      text.ul(
        doc.li`Has a cost of ${text.b`r`}.`,
        doc.li`Has a benefit of ${text.b`MPK`}.`,
      ),
    ),
  ),
  doc.p`Therefore...`,
...
export const solvingModelEquilibrium = createDoc(() => dashbox(
  ...
  twoColumns(
    container(
      ...
      text.ul(
        doc.li`Production Function Y = Ā * K^(1 - α) * L^α`,
        doc.li`Rule for hiring labour α * (Y/L) = w`,
        doc.li`Rule for hiring capital (1-α) * (Y/K) = r`,
    ...
    container(
      ...
      text.ul(
        doc.li`Labour L${doc.sup`*`} = L̄`,
        doc.li`Capital K${doc.sup`*`} = K̄`,
        doc.li`Output Y${doc.sup`*`} = Ā K̄${doc.sup`1 - α`} L̄${doc.sup`α`}`,
        doc.li`Real Wage w${doc.sup`*`} = α(Y${doc.sup`*`}/L${doc.sup`*`})`,
        doc.li`Real rental rate r${doc.sup`*`} = (1-α)(Y${doc.sup`*`}/K${doc.sup`*`})`,
      ),
      doc.blockquote`Would be nice if the world was this simple`,
    ),
  ),
...
export const regressingCobbDouglas = createDoc(() => dashbox(
  doc.h2`Extra: Regressing Cobb Douglas`,
  mathml.regressionOfCobbDouglas,
  doc.p`
    It may not be immediately obvious how you might assemble
    a regression model for the Cobb Douglas given each term
    is a multiple of each other. However it becomes apparent
    when you simply log the body of the function. Below is
    an example of such.

    the constraint holds suggesting if you were to
    create a regression of the cobb douglas it would have a
    log log functional form.
  `,
));

export const cesProductionFunction = createDoc(() => dashbox(
  doc.h2`CES Aggregate Production Function`,
  twoThree(
    mathml.ces,
    infobox({ title: 'Functions' }).c(
      text.ul(
        doc.li`${LINKS.wiki.cobbDouglas.t`Cobb Douglas`}`,
        doc.li`${LINKS.wiki.ces.t`Constant elasticity of substitution`}`,
        doc.li`${LINKS.wiki.leontief.t`Leontief production function`}`,
      ),
    ),
  ),
  doc.p`
    The ${LINKS.wiki.cobbDouglas.t`Cobb Douglas`} production function
    is not the only aggregate production function. Another such
    function includes the ${LINKS.wiki.ces.t`Constant elasticity of
    substitution`} (or CES function). It's actuall a more general
    function than the Cobb Douglas and is inclusive of it and other
    states as well, which tends to vary with ${text.b`ρ`}.
  `,
  text.ul(
    doc.li`${text.b`ρ = 1`}: it resembles a flat plane, α rotates it`,
    doc.li`${text.b`ρ > 0`}: it Begins to flatten as it approaches 1`,
    doc.li`${text.b`ρ = 0`}: it acts like Cobb Douglas`,
    doc.li`${text.b`ρ → -∞`}: it forms a Leontief production function, as it approach negative inifinity`,
  ),
));
