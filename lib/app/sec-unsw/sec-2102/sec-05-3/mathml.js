  ...
  identity: text.figure(
    mathml.math(
      eqId(
        ...
        mfrac(
          minus.paren(
            msub(mi`P`, minus.attr({ subsup: true }).c(mi`t`, mn(1))),
            msub(mi`P`, mi`t`),
          ),
          msub(mi`P`, mi`t`),
        ),
  ...
  decomposition: text.figure(
    mathml.math(
      eq(
        msub(pi, mi`t`),
        add(
          annotationUnder.attr({ label: mtext`expected inflation` }).c(
            msubsup(pi, mi`t`, mi`e`),
          ),
          annotationUnder.attr({ label: mtext`demand conditions` }).c(
