  ...
  identity: text.figure(
    mathml.math(
      eqId(
        ...
        mfrac(
          minus.paren(
            msub(mi`P`, minus.attr({ subsup: true }).of(mi`t`, mn(1))),
            msub(mi`P`, mi`t`),
          ),
          msub(mi`P`, mi`t`),
        ),
  ...
  decomposition: text.figure(
    mathml.math(
      eq(
        msub(pi, mi`t`),
        add(
          annotationUnder.attr({ label: mtext`expected inflation` }).of(
            msubsup(pi, mi`t`, mi`e`),
          ),
          annotationUnder.attr({ label: mtext`demand conditions` }).of(
