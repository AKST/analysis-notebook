...
const vars = {
  ...
  v: text.b`ν̄`,
  o: text.b`ο̄`,
  inExT: text.b`π${doc.sup`e`}${doc.sub`t`}`,
  Ms: text.b`M${doc.sup`S`}`,
  Md: text.b`M${doc.sup`d`}`,
  M: text.b`M${doc.sup`*`}`,
};
...
export const header = createDoc(() => container(
  twoThree(
    container(
      doc.h1.c('MP Curve', doc.br(), doc.small.attr({ style: 'color: #aaaaff' }).t`
    ...
    infobox({ title: 'Resources' }).c(
      text.ul(
        ...
        doc.li.c(LINKS.wiki.arbitrage.t`Arbitrage`),
        doc.li.c(LINKS.wiki.classicalDichotomy.t`Classical Dichotomy`),
      ),
    ),
  ),
));
...
export const intro = createDoc(() => dashbox(
  ...
  doc.p`
    The ${text.b`IS Curve`} and the ${text.b`MP Curve`} are
    two halves of the same whole. While the IS Curve stands
    for ${text.b`investment`} and ${text.b`saving`}, the MP
    in MP curve stands for ${text.b`Monetrary Policy`}.
  `,
));
...
export const theMPCurve1 = createDoc(() => container(
  ...
  dashbox(
    ...
    twoColumns(
      container(
        doc.p`
          ...
          inflation based on the ${text.b`Fisher Equation`}.
          ${text.b.c(text.b`
    ...
    doc.p`
      ${text.b`Sticky Inflation Assumption`} is where we assume that
      the rate of inflation exhbits ${text.b`inertia`} or ${text.b`stickiness`},
    ...
    text.ul(
      doc.li`${text.b`Exante`}, is the real inflation investors expect to prevail.`,
      doc.li`${text.b`Expost`}, is the real inflation investors that actually occured.`,
    ),
  ),
));
...
export const theMPCurve2 = createDoc(() => container(
  dashbox(
    ...
    doc.p`
      ...
      you get the ${text.b`IS-MP Diagram`}
    `,
  ),
  dashbox(
...
export const philipCurve = createDoc(() => container(
  ...
  dashbox(
    ...
    doc.p`
      ...
      The difference in price can be observed by the ${text.b`Inflation
      Identity`} below. We can break this down further into subcomponents:
    `,
    text.ol.attr({ itemSpace: 'sparse' }).c(
      doc.li.c(
        doc.p`One component is the expectations we just discussed ${vars.inExT}.`,
        text.ul.attr({ blockMargin: 1 }).c(
          doc.li`This is called ${text.b`expected inflation`}.`,
          doc.li`
      ...
      doc.li.c(
        ...
        `,
        text.ul.attr({ blockMargin: 1 }).c(
          doc.li`This is called the ${text.b`demand condition`}.`,
          doc.li`
    ...
    twoThree(
      doc.p`
        ...
        called ${text.b`adaptive expceations`}, which is when firms
  ...
  dashbox(
    ...
    doc.p`
      ...
      being shocks. Such as an oil price shock (${text.i`denoted by
      ${vars.o}`}).
    `,
    text.ul(
    ...
    doc.p`
      ...
      tickets, home heating, car fuel, etc. ${text.b`For producers`},
  ...
  dashbox(
    ...
    doc.p`
      ...
      generally called ${text.b`cost-push inflation`} given they
      push up the inflation rate. The term ${vars.v}${vars.YtFluc}
      is called ${text.b`demand-pull inflation`}, incases in
...
export const usingShortRunModel = createDoc(() => container(
  ...
  dashbox(
    ...
    doc.p`
      In 1979 ${LINKS.wiki.paulVolcker.t`Paul Volcker`} was appointed
...
export const microFoundations1 = createDoc(() => container(
  ...
  dashbox(
    ...
    text.ul({ itemSpace: 'sparse' }).c(
      doc.li`
        ${text.b`sticky inflation`} is an essential assumption
        underlying the short-run model.
      `,
      doc.li`
        The ${text.b`MP Curve`} has the ${doc.u`sticky inflation`} assumption
      ...
      doc.li`
        ...
        to the forumation of the ${text.b`Philips Curve`}.
      `,
    ),
    twoThree(
      container(
        doc.p`
          The ${text.b`Classical Dichotomy`} states that ${doc.u`the real
          side of the economy is determined solely by real forces`}.
        `,
        doc.p`
          In order for monetary policy to affect real variables, it must be
          the case that the ${text.b`fails to hold`}. At least so in the
          short-run.
        `,
        doc.p`
  ...
  dashbox(
    ...
    twoThree(
      container(
        doc.p`
          ...
          time, ${text.i`all prices in the economy, including
          nominal wages ${text.b`must`} ajudst in the same
          proportion immediattely`}. This of course will not
          happen, at least not in reality (such a feat would
          require no ${LINKS.wiki.informationAsymmetry.t`information asymmetry
          or imperfect information`}).
        `,
        doc.p`
          Additionally there's a level of ${LINKS.papers.costlyComputation.t`Costly
    ...
    doc.p`
      ...
      ${text.b`contracts set prices and wages in nominal`}
    ...
    doc.p`
      ...
      ${LINKS.wiki.moneyIllusion.t`Money Illusion`} which have
...
export const microFoundations2 = createDoc(() => container(
  ...
  dashbox(
    doc.blockquote`
      The ${text.b`nominal interest rate`} is the
      ${text.b`opportunity cost`} of holding money.
    `,
    twoThree(
      doc.p`
        How does the bank set the nominal interest rate? Well the central
        bank ${text.i.c(doc.u`sets it by supplying whatever money is
        demanded at that interest rate`)}. Given ${doc.u`the nominal
        interest rate is the opportunity cost of holding money`} the
        rate reflects ${doc.u`the amount you ${text.i`give up by
        ${text.b`holding money`}`} instead of ${text.i`keeping it in a
        ${text.b`savings account`}`}`}.
      `,
      infobox({ title: 'Definitions' }).c(
        text.ul(
    ...
    twoThree(
      container(
        ...
        text.ul(
          doc.li`${vars.i}: On the vertical axis we have ${text.b`Nominal Interest`}.`,
          doc.li`${vars.M}: On the horizontal axis we have the ${text.b`Money Supply`}.`,
        ),
      ),
      infobox({ title: 'Nominal Interest in the short run' }).c(
        doc.p`
          With ${LINKS.wiki.nominalRigidity.t`stick Inflation`} we
          ...
          changes to the ${LINKS.wiki.velocityOfMoney.t`velocity of money`}.
          But wait you say, doesn't the ${LINKS.wiki.quantityTheoryOfMoney.t`quantity
  ...
  dashbox(
    ...
    text.ul(
      doc.li`${text.b`How does the bank increase the interest rate`}? It decreases the money supply`,
      doc.li`${text.b`How does the bank decrease the interest rate`}? It increases the money supply`,
    ),
  ),
  dashbox(
    ...
    text.ul(
      doc.li`${text.b`Monetary Expansion`}: a loosening of monetary policy.`,
      doc.li`${text.b`Monetary Contraction`}: a tighening of monetary policy.`,
    ),
  ),
));
...
export const insideTheFed = createDoc(() => container(
  ...
  dashbox(
    ...
    text.ol.attr({ itemSpace: 'sparse' }).c(
      doc.li.c(
        doc.p`
          ${text.b`Reserve Requirements`}: this is where banks are
          required to hold a certian fraction of their deposits in
          special accounts called "${text.b`in reserve`}",
        `,
        text.ul(
      ...
      doc.li.c(
        doc.p`
          ${text.b`Discount Rate`}: this is the interest rate charged
