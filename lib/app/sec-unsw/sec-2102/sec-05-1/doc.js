...
import { clsRef, infobox, text, dashbox, note, todo } from '@prelude-uni/components.js';
import { createDoc } from '@prelude-uni/util.js';
import * as mathml from './mathml.js';
import * as tables from './tables.js';

const LINKS = {
  potenialOutput: text.a({ href: 'https://en.wikipedia.org/wiki/Potential_output' }),
};
...
const vars = {
  potentialOutput: text.b`Ȳ${doc.sub`t`}`,
  currentOutput: text.b`Y${doc.sub`t`}`,
  shortrunFlux: text.b`Ỹ${doc.sub`t`}`,
  longrunIR: text.b`π̄`,
  currentIR: text.b`π`,
  changeInIR: text.b`∆π`,
};
...
export const header = createDoc(() => container(
  twoThree(
    container(
      doc.h1.c(
        'The Short run', doc.br(),
        doc.small({ style: 'color: #aaaaff' }).t`ECON2102, W5, Lecture 1`,
      ),
    ),
    infobox({ title: 'Resources' }).c(
      text.ul(
        doc.li`Chapter 11 of the textbook`,
        doc.li.c(LINKS.potenialOutput.t`Potential Output`),
      ),
    ),
  ),
));
  ...
  container(
    twoThree(
      ...
      doc.quote.c(
        ...
        `,
        doc.p({ style: 'text-align: right' }).t`— John Maynard Keynes`,
      ),
    ),
  ),
));
...
export const theLongrunTheShortrun = createDoc(() => container(
  ...
  dashbox(
    ...
    doc.ul.c(
      doc.li`
        ${text.b`Potenial Output`}, is the amount the economy would
        produce if all inputs were utlized at their long-run sustainable
        levels.
      `,
      doc.li`
        ${text.b`Actual Output`}, can deviate from this in that it
      ...
      `,
      doc.li`
        The ${text.b`short run`} is generally the period in which these
    ...
    doc.p`
      ...
      a short run model, such as ${text.b`potenial output`} and the
      ${text.b`long-run inflation rate`} which are taken as
      ${text.b`exogenous`} variables in the short run model. Our model
      also has 2 ${text.b`endogenous`} variables, the ${text.b`current
      level of output`} and the ${text.b`current level of inflation`}.
    `,
    doc.h4`Notation`,
    text.ul(
      doc.li`
        ${text.b`Potential output`} is expressed as ${vars.potentialOutput},
        and is generally determined using a combined Solow-Romer model.
      `,
      doc.li`
        ${text.b`Long run inflation`} is expressed as ${vars.longrunIR}, for now
        it will be expressed without of time, hence no time subscript.
      `,
      doc.li`
        ${text.b`Current output`} (or current level of output) is expressed
        in terms of ${vars.currentOutput}.
      `,
      doc.li`
        ${text.b`Current inflation`} (or the current rate of inflation) is
    ...
    container(
      ...
      twoColumns(
        container(
          doc.p`
            ...
            also referred to as ${text.b`detrended output`} or ${text.b`short-run
            output`}).
          `,
          mathml.actualOutput,
        ),
      ...
      doc.p`
        ...
        defines a ${text.b`recession`} as the point at which acutal output
    ...
    container(
      ...
      doc.p`
        ...
        ${text.b`Congresssional Budget Office`} uses a version of the
...
export const theShortrunModel = createDoc(() => container(
  ...
  dashbox(
    ...
    doc.ul.c(
      doc.li`
        ${text.i`The economy is constantly being hit by shocks`}. These are
        ${text.b`Economic shocks`}, which is inclusive of changes in resource
      ...
      `,
      doc.li`
        ${text.i`Monetary and fiscal policy affect output`}. The Government has
      ...
      `,
      doc.li`
        ${text.i`There is a dynamic trade-off between output and inflation`}.
  ...
  dashbox(
    doc.ul.c(
      ...
      doc.li`
        ...
        is meant by ${text.b`Potential output`}. It actually
        ...
        same (${text.b`I need to fact check this`}).
      `,
      doc.li.c(
        doc.p`In a ${text.b`booming`} economy`,
        text.ul(
          doc.li.c(doc.p`Say the prevailing inflation rate is 5%`),
          doc.li.c(doc.p`All firms are raising their prices by 7%`),
        ),
      ),
      doc.li.c(
        doc.p`In a ${text.b`contracting`} economy`,
        text.ul(
          doc.li.c(doc.p`Say the prevailing inflation rate is 5%`),
          doc.li.c(doc.p`All firms are raising their prices by 3%`),
          doc.li.c(doc.p.c(text.i`
  ...
  dashbox(
    ...
    text.ul(
      ...
      doc.li`
        ...
        ${doc.font({ color: 'red' }).t`TODO: insert meaning`}.
      `,
    ),
  ),
));
