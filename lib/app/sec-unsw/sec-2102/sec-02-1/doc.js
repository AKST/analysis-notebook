
/**
 * @import { E, DocumentWidget, Widget } from '@app/prelude-type.ts';
 * @import { Config, State } from './type.ts';
 */
import { doc } from '@app/prelude.js';
import { responsiveGridAutoRow, twoColumns, twoThree, container } from '@prelude-uni/layout.js';
import { text, noteOn, infobox, dashbox, note, todo } from '@prelude-uni/components.js';
import { createDoc } from '@prelude-uni/util.js';
import * as mathml from './mathml.js';
import * as tables from './tables.js';

const LINKS = {
  romerInterview: text.a({ href: 'https://www.youtube.com/watch?v=iCdr86rkVBc' }),
  nobelPrize: text.a({ href: 'https://www.youtube.com/watch?v=vZmgZGIZtiM' }),
  romer: text.a({ href: 'https://www.jstor.org/stable/2937632' }),
  base44: text.a({ href: 'https://base44.com' }),
};

/**
 * @param {string} label
 * @returns {Widget<any, State, Config>}
 */
export const createPlaceholder = (label) => (
  createDoc(() => container(
    todo({}, `for "${label.trim()}"`),
  ))
);

export const intro = createDoc(() => container(
  twoThree(
    container(
      doc.h1.c(
        'Macroeconomics 2', doc.br(),
        doc.small({ style: 'color: #aaaaff' }).t`ECON2102, W2, Lecture 1`,
      ),
      doc.p`
        This idea isn't commonly used however its easy
        to teach, and we'll use it as a stepping stone
        to more complicated ideas.
      `,
    ),
    infobox({ title: 'Resources' }).c(
      doc.p`See here for resources relating to the lesson`,
      text.ul(
        doc.li.c(LINKS.romer.t`Romer Paper`),
        doc.li.c(LINKS.romerInterview.t`Romer Interview`),
        doc.li.c(LINKS.nobelPrize.t`Nobel Prize`),
        doc.li.c(LINKS.base44.t`AI app platform`, ': mentioned by lecturer.'),
      ),
    ),
  ),
));

export const economicsOfIdeas = createDoc(() => dashbox(
  doc.h2`The Economics of Ideas`,
  mathml.ideaDiagram,
  twoColumns(
    container(
      doc.h4`Non-Rival`,
      doc.p`
        The extent which something is "Rival" describes the extent
        to which ones persons use prevents other peoples use. You
        can't use a fork if someone else is using it, if the apple
        store runs out of iPhones they won't sell you the right to
        use someone elses iPhone, you have to wait till there's
        more in stock.
      `,
    ),
    container(
      doc.h4`Excludability`,
      doc.p`
        It's important not to confuse rivary and excludability.
        An idea however can be used by many people simultaneously,
        but it can still exhibt qualities of exclusicivity. In
        practice this is often the result of something like a
        patent of Intellectual Property rights.
      `,
    ),
  ),
));

export const increasingReturnsToScale = createDoc(() => dashbox(
  doc.h2`Increasing returns to scale`,
  note(
    doc.li`Non-rival ideas leads to increasing retursn to production`,
  ),
  todo({}, 'revisit slides, here lecture skipped over this'),
  twoColumns(
    container(
      noteOn('CRS')(
        doc.li`CRS means Constant returns to scale`,
        doc.li`if CRS just means X, as Y/X = A is constant`,
        doc.li`As in as output increases and you divide by X A remains the same`,
        doc.li`Double the inputs, output doubles`,
      ),
    ),
    container(
      noteOn('IRS')(
        doc.li`IRS means Increasing returns to scale`,
        doc.li`if IRS means A & X, as Y/X = A is increasing`,
        doc.li`As in as output increases and you divide by X, A increases`,
        doc.li`Double the inputs, output more than doubles`,
      ),
    ),
  ),
));

export const problemsWithPerfectCompetition = createDoc(() => dashbox(
  doc.h2`Problems with Perfect Competition`,
  noteOn('perfect competition')(
    doc.li`IRS is at odds with perfect competition`,
    doc.li`firm behaviour${text.ul(
      doc.li`perfect competition => Price = Marginal Cost`,
      doc.li`This conditions is unlikely hold`,
    )}`,
  ),
  todo({}, 'what is a comenstsar, 10:19'),
  noteOn('in relation to ideas')(
    doc.li`Ideas also have fixed costs${text.ul(
      doc.li`Cost of developing drugs`,
      doc.li`A startups seed funding`,
    )}`,
  ),
));

export const romerGrowthModel = createDoc(() => dashbox(
  doc.h2`Romer Growth Model`,

  responsiveGridAutoRow([
    text.figure(mathml.romerLabour(), doc.figcaption`Romer Labour`),
    text.figure(mathml.romerLabourResearch(), doc.figcaption`Research Labour`),
    text.figure(mathml.romerLabourOutput(), doc.figcaption`Output Labour`),
    text.figure(mathml.romerGrowthRateOfKnowledge(), doc.figcaption`Growth of Knowledge`),
  ], {
    columns: {
      desktop: 'auto auto auto auto',
    },
  }),
  responsiveGridAutoRow([
    text.figure(mathml.romerModelProductionOfNewIdeas(), doc.figcaption`Production of new Ideas`),
    text.figure(mathml.romerGrowthModel(), doc.figcaption`Romer Output`),
    text.figure(mathml.stockOfKnowledge(), doc.figcaption`Stock of Knowledge`),
  ], {
    columns: {
      desktop: 'auto auto auto',
    },
  }),
  text.figure(mathml.perCapitaStockOfKnowledge(), doc.figcaption`Output Per Capita`),
  twoColumns(
    note(
      doc.li`we need distinguish between goods and ideas`,
      doc.li`In the previous Solow-Swan idea we saw stagnation`,
      doc.li`This idea introduces ideas into the model`,
      doc.li`Capital becomes fixed, e.g. K = 1`,
    ),
    note(
      doc.li`z̄ is probably something small like 0.0005`,
      doc.li`𝓁 is similarly small`,
      doc.li`Some people have repurposed this to resemble the education sector`,
    ),
  ),
  noteOn('Labour Supply and Allocation of Labour Across Sectors')(
    doc.li`Labour is not non-rival; either in one sector or the other and seubject to availabilty of labour supply`,
  ),
));

export const solvingRomerModel = createDoc(() => dashbox(
  doc.h2`Solving the Model`,
  mathml.romerSolvingAssumptions(),
  note(
  ),
));

export const balancedGrowthPath = createDoc(() => dashbox(
  doc.h2`Balanced Growth Path`,
  doc.p`
    Given the Romer model exhibts sustained growth it doesn't
    have a steady state in the same way the Solow Swan does.
    The growth rate is constant and is expressed as:
  `,
  text.figure(mathml.constantGrowth, doc.figcaption`Constant Growth Parameters`),
));

export const growthAccounting = createDoc(() => dashbox(
  doc.h2`Growth Accounting`,
  text.figure(mathml.romerGrowthAccounting(), doc.figcaption`Growth Accounting`),
  text.figure(mathml.romerGrowthAccounting2(), doc.figcaption`???`),
));

export const summary = createDoc(() => dashbox(
  doc.h2`Summary`,
  note(
  ),
  todo({}, 'Combine swan-solow and rommer model'),
));

export const continiousRomerSolow = createDoc(() => dashbox(
  doc.h2`Extra — Continious`,
  twoColumns(
    text.figure(mathml.continiousA(), doc.figcaption`Continious A`),
    text.figure(mathml.continiousY(), doc.figcaption`Continious Y`),
  ),
  text.figure(mathml.continiousK(), doc.figcaption`Continious K`),
));
