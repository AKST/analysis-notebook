 ...
 */
import { doc, frag, mathml } from '@app/prelude.js';
import { SPECIAL, rows, parensA, call, op } from '@prelude-uni/mathml.js';
...
export const romerGrowthModel = () => mathml.math(
  eq(vars.outputT, mul(vars.techT, vars.labourYT)),
);
...
export const romerModelProductionOfNewIdeas = () => mathml.math(
  eq(frag(Delta, vars.techTNext), mul(vars.researchProductivity, vars.techT, vars.labourAT)),
);
...
export const romerLabourResearch = () => mathml.math(
  eq(vars.labourAT, mul(vars.researchSector, vars.labour)),
);
...
export const romerLabourOutput = () => mathml.math(
  eq(vars.labourYT, mul(minus.paren(mn(1), vars.researchSector), vars.labour)),
);
...
export const romerGrowthRateOfKnowledge = () => mathml.math(
  eq(vars.knowledgeGrowthRate, mul(
...
export const perCapitaStockOfKnowledge = () => mathml.math(
  eq(vars.outputTPerCapita, mul(
    vars.tech0,
    minus.paren(mn(1), vars.researchSector),
    msup(parensA(add(mn(1), vars.knowledgeGrowthRate)), mi`t`),
  )),
);
...
export const romerSolvingAssumptions = () => (
  doc.div({ style: 'background: red; color: white' }).t`knowledge stock assumption`
);
...
export const continiousA = () => mathml.math(
  eq(call.attr({ fn: mi`A` }).of(mi`t`), frag(
...
export const continiousY = () => mathml.math(
  eq(call.attr({ fn: mi`Y` }).of(mi`t`), mul(
    call.attr({ fn: mi`A` }).of(mi`t`),
    msup(mrow(call.attr({ fn: mi`K` }).of(mi`t`)), mrow(mn(1), mo`-`, alpha)),
    msup(mrow(call.attr({ fn: mtext`Ly` }).of(mi`t`)), alpha),
  )),
);
...
export const continiousK = () => mathml.math(
  eq(call.attr({ fn: mi`K` }).of(mi`t`), msup(mrow(mo`[`, mrow(
    add(
      mul(
      ...
      frag(
        mfrac(
          mul(
            mul(alpha, vars.iFactor),
            mul(vars.tech0, msup(vars.labourY, alpha)),
          ),
          add(mi`g`, mul(alpha, vars.depreciation))
        ),
        minus.paren(
