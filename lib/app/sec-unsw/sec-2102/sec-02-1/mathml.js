...
export const continiousY = () => mathml.math(
  eq(call({ fn: mi`Y` }).c(mi`t`), mul(
    call({ fn: mi`A` }).c(mi`t`),
    msup(mrow(call({ fn: mi`K` }).c(mi`t`)), mrow(mn(1), mo`-`, alpha)),
    msup(mrow(call({ fn: mtext`Ly` }).c(mi`t`)), alpha),
  )),
);
...
export const continiousK = () => mathml.math(
  eq(call({ fn: mi`K` }).c(mi`t`), msup(mrow(mo`[`, mrow(
    add(
      mul(
