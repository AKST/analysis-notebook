...
export const continiousA = () => mathml.math(
  eq(call.attr({ fn: mi`A` }).c(mi`t`), frag(
...
export const continiousY = () => mathml.math(
  eq(call.attr({ fn: mi`Y` }).c(mi`t`), mul(
    call.attr({ fn: mi`A` }).c(mi`t`),
    msup(mrow(call.attr({ fn: mi`K` }).c(mi`t`)), mrow(mn(1), mo`-`, alpha)),
    msup(mrow(call.attr({ fn: mtext`Ly` }).c(mi`t`)), alpha),
  )),
);
...
export const continiousK = () => mathml.math(
  eq(call.attr({ fn: mi`K` }).c(mi`t`), msup(mrow(mo`[`, mrow(
    add(
      mul(
