  ...
  math(
    table.attr({ columnalign: 'right center left' }).of(
      ...
      eq.array(
        ...
        annotationUnder.attr({ label: mtext`total wealth` }).of(
          add(
...
export const lifetimeUtilityFunction = text.figure(
  math(
    eq(
      mi`U`,
      add(
        call({ fn: mi`u` }).of(msub(mi`c`, mtext`today`)),
        mul0(
          greek.beta,
          call({ fn: mi`u` }).of(msub(mi`c`, mtext`future`))
        )
      )
    )
  ),
...
export const indifferenceCondition = text.figure(
  math(
    eq(
      call({ fn: msup(mi`u`, mo`′`) }).of(msub(mi`c`, mtext`today`)),
      mul0(
        greek.beta,
        add.paren(mn(1), mi`R`),
        call({ fn: msup(mi`u`, mo`′`) }).of(msub(mi`c`, mtext`future`))
      )
    )
  ),
...
export const solvingConsumptionWhenBetaIs1 = text.figure(
  math({ style: '--fontsize-math-m: 14px' }).of(
    table.attr({ columnalign: 'right right center left' }).of(
      [
...
export const consumptionProportionalToWealth = text.figure(
  math(
    eq(
      ...
      add(
        msub(mi`f`, mtext`today`),
        annotationUnder({ label: mtext`PDV of income` }).of(
          add(
            msub(mi`y`, mtext`today`),
            minus.square(
  ...
  math(
    table({ columnalign: 'left right center left' }).of(
      [mtext`General`, ...eq.array(
        call({ fn: mtext`𝔼` }).of(mi`C`),
        mul0(
          inv(mi`n`),
          parensA(sum({ max: mi`n`, inc: add.sub(mi`i`, mn(1)) }).of(msub(mi`c`, mi`i`))),
        ),
      )],
      [mrow()],
      [mtext`Two periods`, ...eq.array(
        call({ fn: mtext`𝔼` }).of(msub(mi`c`, mn(1)), msub(mi`c`, mn(2))),
...
const taxHl = mi({ style: 'font-weight: 900; text-shadow: var(--txt-shadow-blue)' })`τ`;
...
export const lagrangian = {
  l: math(
    mtable(
      mtr(mtd({ columnalign: 'left', columnspan: 3 }).of(
        mtext`Lagrangian`
      )),
      eq.mtr({ pos: ['right', 'left'] }).of(
        call({ fn: mi`𝔏` }).of(vars.c1, vars.c2, mi`λ`),
        minus(
          add(utility(vars.c1), mul0(mi`β`, utility(vars.c2))),
          mul0(
            mi`λ`,
            minus.square(
  ...
  functions: math(
    mtable(
      ...
      eq.mtr({ pos: ['right', 'left'] }).of(
        ...
        add(utility(vars.c1), mul(mi`β`, utility(vars.c2))),
      ),
    ),
  ),
