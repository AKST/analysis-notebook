...
import { text, noteOn, clsRef, infobox, dashbox, note, todo } from '@prelude-uni/components.js';
...
const LINKS = {
  ytMonsterOfInflation: text.a.attr({ href: 'https://www.youtube.com/watch?v=pbgY0dYoBvA' }),
  ytMoneyInTheEconomy: text.a.attr({ href: 'https://www.rba.gov.au/publications/bulletin/2018/sep/money-in-the-australian-economy.html' }),
  wikiVelocityOfMoney: text.a.attr({ target: '_blank', href: 'https://en.wikipedia.org/wiki/Velocity_of_money' }),
};
...
export const header = createDoc(() => container(
  twoThree(
    container(
      doc.h1.c('Inflation', doc.br(),
        doc.small({ style: 'color: #aaaaff' }).t`ECON2102, W4, Lecture 1`
      ),
    ),
    infobox({ title: 'Resources' }).c(
      text.ul(
        doc.li`Chapter 8 of the text book`,
        doc.li.c(LINKS.ytMonsterOfInflation.t`The monster of Inflation`),
        doc.li.c(LINKS.ytMoneyInTheEconomy.t`Money in the Economy`),
      ),
    ),
  ),
));
...
const PYt = frag(text.b`P${doc.sub`t`}`, text.b`Y${doc.sub`t`}`);
...
export const quantityTheoryOfMoney = createDoc(() => container(
  ...
  dashbox(
    ...
    note(
      ...
      doc.li`Money has value because we expect others will value it`,
  ...
  dashbox(
    ...
    container(
      twoThree(
        container(
          ...
          `,
          text.ul(
            doc.li`${text.b`M${doc.sub`t`}`}: the supply of money`,
            doc.li`${text.b`P${doc.sub`t`}`}: price level`,
            doc.li`${text.b`Y${doc.sub`t`}`}: real GDP`,
            doc.li`${text.b`V${doc.sub`t`}`}: the velocity of money`,
          ),
        ),
    ...
    twoColumns(
      container(
        ...
        doc.p`
          This equation largely states that ${text.b`in the
      ...
      container(
        ...
        doc.blockquote`Read ${
          LINKS.wikiVelocityOfMoney.t`here about the velocity of money`
        }`,
        doc.p`
          ...
          apparently in reality the ${text.b`velocity of M2`}
    ...
    doc.p`
      The essence of the quantity theory of money is: ${text.b`in the long run`}
...
export const theoryOfInflation = createDoc(() => dashbox(
  ...
  text.ul(
    doc.li`${text.b`inflation`}: When money growth is faster than the growth of real GDP`,
    doc.li`${text.b`deflation`}: When money growth is slower than the growth of real GDP`,
  ),
  ...
  twoColumns(
    container(
      ...
      doc.p`
        ...
        the ${text.b`nominal interest rate`}.,
      `,
    ),
  ...
  dashbox(
    twoColumns(
      ...
      container(
        ...
        text.ul(
          doc.li`${text.b`G`}: Govt use of money`,
          doc.li`${text.b`T`}: Tax revenue`,
          doc.li`${text.b`∆B`}: change in outstanding debt`,
          doc.li`${text.b`∆M`}: change in money supply`,
        ),
      ),
    ),
    ...
    doc.p`
      ...
      (∆M) is called ${text.b`seignorage`} or the
      ${text.b`inflation tax`}. Inflation tax is paid by who
