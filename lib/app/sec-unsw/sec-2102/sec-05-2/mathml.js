...
export const outputDecomposition = {
  ...
  d: text.figure(
    mathml.math(
      eq(
        minus(
          annotationUnder.attr({ label: mi`Ỹₜ` }).of(
            mfrac(msub(mi`Y`, mi`t`), msub(mi`Ȳ`, mi`t`)),
          ),
          mn(1)
        ),
        mrow(
          annotationUnder.attr({ label: mi`ā` }).of(
            add(
              msub(mi`ā`, mi`c`),
              msub(mi`ā`, mi`i`),
              msub(mi`ā`, mi`g`),
...
export const ISCurve = text.figure(
  mathml.math(
    eq(
      msub(mi`Ỹ`, mi`t`),
      mul0(
        annotationUnder.attr({ label: mtext`multiplier` }).of(
          mfrac(mn(1), minus(mn(1), mi`x̄`)),
        ),
        annotationUnder.attr({ label: mtext`original IS curve` }).of(
          minus.square(
            mi`ā`,
            mul0(mi`b̄`, minus.paren(msub(mi`R`, mi`t`), mi`r̄`))
          ),
