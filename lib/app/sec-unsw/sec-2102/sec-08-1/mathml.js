...
export const govtBudgetConstraint = text.figure(
  math(
    eq(
      annotationUnder.attr({ label: mtext`uses` }).of(
        add(
          msub(mi`G`, mi`t`),
      ...
      annotationUnder.attr({ label: mtext`sources` }).of(
        add(
          msub(mi`T`, mi`t`),
          add(
  ...
  math(
    eq(
      ...
      add(
        ...
        annotationUnder.attr({ label: mtext`deficit` }).of(
          minus(
            msub(mi`G`, mi`t`),
            msub(mi`T`, mi`t`)
          )
...
export const intertemporalBudgetConstraint = text.figure(
  math(
    eq(
      add(
        annotationUnder.attr({ label: mtext`pdv of spending` }).of(
          add(
            msub(mi`G`, mn(1)),
            mfrac(
              msub(mi`G`, mn(2)),
        ...
        annotationUnder.attr({ label: mtext`initial debt` }).of(
          mul0(add.paren(mn(1), mi`i`), msub(mi`B`, mn(1)))
        )
      ),
      add(
        msub(mi`T`, mn(1)),
        annotationUnder.attr({ label: mtext`pdv of taxes` }).of(
          mfrac(
            msub(mi`T`, mn(2)),
            add.paren(mn(1), mi`i`)
          )
  ...
  math(
    eq(
      ...
      add(
        ...
        sum({ inc: eq.sub(mi`p`, mn(1)), max: mi`t` }).of(
          parensC(
            add.square(
              mul0(
                munderover(
  ...
  math(
    eq(
      ...
      minus(
        ...
        annotationUnder({ label: mtext`Primary Budget Balance` }).of(
          minus.square(
            msub(mi`T`, mi`t`),
            msub(mi`G`, mi`t`),
            msub(mi`TR`, mi`t`),
...
export const bondValue = text.figure(
  math(
    eq(
      call({ fn: mtext`BondValue` }).of(
        mtext`Coupon`,
        mtext`FaceValue`,
        mi`N`,
      ),
      add(
        sum({ max: mi`N`, inc: eq.sub(mi`t`, mn(1)) }).of(
