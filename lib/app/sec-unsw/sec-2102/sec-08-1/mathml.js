...
export const govtBudgetConstraint = text.figure(
  math(
    eq(
      annotationUnder.attr({ label: mtext`uses` }).c(
        add(
          msub(mi`G`, mi`t`),
      ...
      annotationUnder.attr({ label: mtext`sources` }).c(
        add(
          msub(mi`T`, mi`t`),
          add(
  ...
  math(
    eq(
      ...
      add(
        ...
        annotationUnder.attr({ label: mtext`deficit` }).c(
          minus(
            msub(mi`G`, mi`t`),
            msub(mi`T`, mi`t`)
          )
...
export const intertemporalBudgetConstraint = text.figure(
  math(
    eq(
      add(
        annotationUnder.attr({ label: mtext`pdv of spending` }).c(
          add(
            msub(mi`G`, mn(1)),
            mfrac(
              msub(mi`G`, mn(2)),
        ...
        annotationUnder.attr({ label: mtext`initial debt` }).c(
          mul0(add.paren(mn(1), mi`i`), msub(mi`B`, mn(1)))
        )
      ),
      add(
        msub(mi`T`, mn(1)),
        annotationUnder.attr({ label: mtext`pdv of taxes` }).c(
          mfrac(
            msub(mi`T`, mn(2)),
            add.paren(mn(1), mi`i`)
          )
...
export const savingIdentity = text.figure(
  math(
    eq(
      add(
        annotationUnder.attr({ label: mtext`private saving` }).c(
          add.paren(mi`Y`, op.minus(mi`T`), op.minus(mi`C`))
        ),
        annotationUnder.attr({ label: mtext`government saving` }).c(
          add.paren(mi`T`, op.minus(mi`G`))
        ),
        annotationUnder.attr({ label: mtext`foreign saving` }).c(
          add.paren(mi`IM`, op.minus(mi`EX`))
        )
      ),
      mi`I`
  ...
  math(
    eq(
      ...
      add(
        ...
        sum({ inc: eq.sub(mi`p`, mn(1)), max: mi`t` }).c(
          parensC(
            add.square(
              mul0(
                munderover(
  ...
  math(
    eq(
      ...
      minus(
        ...
        annotationUnder({ label: mtext`Primary Budget Balance` }).c(
          minus.square(
            msub(mi`T`, mi`t`),
            msub(mi`G`, mi`t`),
            msub(mi`TR`, mi`t`),
...
export const bondValue = text.figure(
  math(
    eq(
      call({ fn: mtext`BondValue` }).c(
        mtext`Coupon`,
        mtext`FaceValue`,
        mi`N`,
      ),
      add(
        sum({ max: mi`N`, inc: eq.sub(mi`t`, mn(1)) }).c(
