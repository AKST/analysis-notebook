...
export const govtBudgetConstraint = text.figure(
  math(
    eq(
      annotationUnder({ label: mtext`uses` }).c(
        add(
          msub(mi`G`, mi`t`),
      ...
      annotationUnder({ label: mtext`sources` }).c(
        add(
          msub(mi`T`, mi`t`),
          add(
  ...
  math(
    eq(
      ...
      add(
        ...
        annotationUnder({ label: mtext`deficit` }).c(
          minus(
            msub(mi`G`, mi`t`),
            msub(mi`T`, mi`t`)
          )
...
export const intertemporalBudgetConstraint = text.figure(
  math(
    eq(
      add(
        annotationUnder({ label: mtext`pdv of spending` }).c(
          add(
            msub(mi`G`, mn(1)),
            mfrac(
              msub(mi`G`, mn(2)),
        ...
        annotationUnder({ label: mtext`initial debt` }).c(
          mul0(add.paren(mn(1), mi`i`), msub(mi`B`, mn(1)))
        )
      ),
      add(
        msub(mi`T`, mn(1)),
        annotationUnder({ label: mtext`pdv of taxes` }).c(
          mfrac(
            msub(mi`T`, mn(2)),
            add.paren(mn(1), mi`i`)
          )
...
export const savingIdentity = text.figure(
  math(
    eq(
      add(
        annotationUnder({ label: mtext`private saving` }).c(
          add.paren(mi`Y`, op.minus(mi`T`), op.minus(mi`C`))
        ),
        annotationUnder({ label: mtext`government saving` }).c(
          add.paren(mi`T`, op.minus(mi`G`))
        ),
        annotationUnder({ label: mtext`foreign saving` }).c(
          add.paren(mi`IM`, op.minus(mi`EX`))
        )
      ),
      mi`I`
