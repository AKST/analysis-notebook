...
export const firmTable = (firm, fixedCost) => {
  if (firm.price.some(isNullish)) {
    return doc.font({color: 'red'}).t`You may need to adjust quantity per worker`;
  }
  const rows = Array.from(econFirm.firmBehaviourTableIter(firm), row => doc.tr(
    doc.td.c(intstr(row.workers)),
    doc.td.c(ns(fixedCost)),
    doc.td.c(ns(row.costVar)),
    doc.td.c(ns(row.costTotal)),
    doc.td.c(ns(row.marginalCost)),
    doc.td.c(ns(row.avgCostTotal)),
    doc.td.c(ns(row.avgCostVar)),
    doc.td.c(intstr(row.units)),
    doc.td.c(ns(row.price)),
    doc.td.c(ns(row.revenue)),
    doc.td.c(ns(row.marginalRevenue)),
    doc.td.c(ns(row.profit)),
  ));
  return doc.table(
    doc.thead(doc.tr(
      doc.th`W`,
...
export const exampleTwoAreas = (state) => {
  ...
  return doc.table(
    ...
    doc.tbody(
      doc.tr(
        doc.th`Demand`,
        doc.td.c(ns(a.demand.size)),
        doc.td.c(ns(a.demand.size)),
        doc.td.c(ns(c.demand.size)),
        doc.td`0`,
      ),
      doc.tr(
        doc.th`Supply`,
        doc.td.c(ns(a.supply.size)),
        doc.td.c(ns(b.demand.size)),
        doc.td.c(ns(c.supply.size)),
        doc.td.c(ns(totalC)),
      ),
      doc.tr(
        doc.th`Total`,
      ...
      doc.tr(
        doc.th`Total (DWL)`,
        doc.td.c(ns(a.dwl.size)),
        doc.td.c(ns(b.dwl.size)),
        doc.td`0`,
        doc.td`0`,
      ),
    ),
...
export const exampleThreeMarketSegments = (state) => {
  ...
  for (const [k, v] of Object.entries(segments)) {
    items.push(doc.h4`Market Segment ${k}`);
