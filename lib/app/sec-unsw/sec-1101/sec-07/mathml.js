...
import { rows, sum, call, parensA, SPECIAL } from '@prelude-uni/mathml.js';

const { mi, mo, mtext, mrow, mover, mn, msub, msup, msqrt, munder, mspace, mfrac } = mathml;

const { greek: { alpha, beta, Delta }, operation } = SPECIAL;

export const marginalRevenue = () => mathml.math(
  mrow(
...
export const calcMarginalRevenue = () => mathml.math(
  rows(
...
export const profitMaximisingQuantity = () => mathml.math(
  rows(
...
export const monopolistPrice = () => mathml.math(
  mrow(msub(mi`P`, mi`m`), mo`=`, sa, mo`+`, sb, qm),
);

export const averageCostPricing = () => mathml.math(
  mover(mrow(
    mi`Profit`, mo`=`, mi`Q`,
...
export const firstDegreePriceDiscriminationTR = () => mathml.math(
  munder(mover(rows(
    mrow(
      mi`TR`, mo`=`,
...
export const firstDegreePriceDiscriminationMR = () => mathml.math(
