 ...
 * @returns {E.Item}
 */
const ul = (...items) => doc.ul({ className: 'no-item-padding' }).css({
  paddingLeft: '16px',
}).c(...items.map(p => text.li.m.c(p)));
  ...
  render: () => container(
    dashbox(
      ...
      doc.p`
        ...
        no longer "${text.b`Free entry and exit`}" which
    ...
    dashbox(
      ...
      twoColumns(
        doc.dl(
          ...
          doc.dd.c(doc.span`When a firm has the ability to set its own prices`),
          doc.dt`Market Power`,
          doc.dd.c(doc.span`When a firm has the ability to be a price-setter`),
          doc.dt`Monopoly`,
          doc.dd.c(doc.span`
        ...
        doc.dl(
          ...
          doc.dd.c(doc.span`
            ...
            slightly differentiated goods. Such as ${text.b`restraurants`},
            and ${text.b`clothing brands`}.
          `),
          doc.dt`Oligopolistic Competition`,
          doc.dd.c(doc.span`
            A market with a small number of firms selling goods
            that are close substitutes. Think ${text.b`internet/cell
            service providers`}, ${text.b`Australian grocery stores`}.
          `),
        ),
      ),
      twoColumns(
        container(
          ...
          doc.p`
             ...
             100 units each. Think ${text.b`mining`},
             ${text.b`infrastructure`}, ${text.b`pharmaceuticals`}.
          `,
        ),
        container(
...
export const originsOfMonopolies = {
  ...
  render: () => dashbox(
    ...
    doc.p`
      ...
      some kind of ${text.b`Barrier to entry`}
      for producers to partipate in the market.
    `,
    twoColumns(
      doc.dl(
        doc.dt`Control over scarce resource`,
        doc.dd.c(doc.span`oil, diamonds, trade routes`),
        doc.dt`Government created barriers to entry`,
        doc.dd.c(doc.span`parents, copyright`),
      ),
      doc.dl(
        doc.dt`Increasing returns to scale`,
        doc.dd.c(doc.span`economies of scale, such as utilties`),
        doc.dt`Network economies`,
        doc.dd.c(doc.span`iMessage, Facebook`),
      ),
    )
  ),
};
...
export const costBenefitPrinciple = {
  ...
  render: () => dashbox(
    ...
    doc.p`
      ...
      text.b`Market Price ≥ Marginal Cost`} where
      as monopolist will produce each unit up to the
      point at which ${
      text.b`Marginal Revenue ≥ Marginal Cost`}.
    `,
  ),
};
  ...
  render: () => container(
    twoThree(
      dashbox(
        ...
        `,
        ul(
          doc.p`Produce ${text.b`Qm where MR = MC`}`,
          doc.p`Price ${text.b`Pm along the demand curve at Qm`}`,
        ),
        doc.p`
      ...
      infobox({ title: 'Profit Maximisation' }).c(
        container(
          text.p.m`Solve for marginal revenue`,
          doc.div({ className: 'container' }).css({ fontSize: '14px' }).c(
  ...
  render: (ctx, state) => container(
    ...
    dashbox(
      ...
      doc.p`
        ...
        A big part of this is due to a ${text.b`Revenue Trade off`}.
...
export const monopolyRegulation = {
  ...
  render: () => dashbox(
    ...
    doc.p`
      ...
      ${text.b`Average Cost Pricing`} will make more
...
export const priceDiscrimination = {
  ...
  render: () => dashbox(
    ...
    `,
    ul(
      frag(text.b`Quantity`, ' — think bulk discounts, buy one get one deals'),
      frag(text.b`Quality`, ' — think economy vs business class, iPhone 10 vs iPhone SE'),
    ),
