...
export const intro = {
    ...
    render: (_ctx, _state) => container(
      doc.h1`Supply Curve`,
      doc.div({ className: 'c2 twoThree' }).of(
        container(
...
export const example = {
  ...
  render: (_ctx, state) => {
    ...
    return container(
      ...
      `,
      doc.div({ className: 'c2 center' }).of(
      ...
      doc.div({ className: 'relevance-disclaimer' }).of(
        doc.div({ className: 'relevance-disclaimer-2' }).of(
          doc.p`${text.b`Note`}:
...
export const kindsOfCosts = {
  ...
  render: (_ctx, state, cfg) => container(
    doc.div({ className: 'c2 twoThree' }).of(
      ...
      container(
        infobox({ title: 'Cost Benefit Principle' }).of(
          text.p.m`
          ...
          `,
          doc.br(),
          doc.span({ className: 'cbp' }).of(
            mathml.costBenefitPrinciple(),
          ),
        ),
        infobox({ title: 'Law of Supply' }).of(
          text.p.m`
        ...
        infobox({ title: 'Economic Surplus' }).of(
          text.p.m`
...
export const elascity = {
  ...
  render: (_ctx, state, cfg) => dashbox(
    ...
    doc.div({ className: 'c2 twoThree' }).of(
      ...
      container(
        infobox({ title: 'Elasticity Formular' }).of(
          container(
            text.p.m`
            ...
            doc.div({ className: 'elasticity' }).of(
              mathml.elasticity()
            ),
            text.p.m`This is point elasticity`,
          ),
        ),
        doc.span({ className: 'elasticity-table' }).of(
...
export const deriveAtcAndAvcFromMC = {
  ...
  render: (_ctx, state, cfg) => dashbox(
    ...
    twoThree(
      ...
      infobox({ title: 'Derive Parital ATC & AVC' }).of(
        doc.div({ style: 'font-size: 14px' }).of(
