...
import { SPECIAL, abs, index, iv, set, matrix, rows, sum, parensA, table, call } from '@prelude-uni/mathml.js';
...
export function vectorDefs() {
  return mathml.math(
    table.attr({ className: 'mathtable' }).c(
      [mi`C`, mo`=`, mover(
...
export function costBenefitPrinciple() {
  return mathml.math(
    mrow(
      call.attr({ fn: mtext`MB` }).c(mi`Price`),
      rel.lteq, mi`MC`,
      mspace(8), rel.imply,
