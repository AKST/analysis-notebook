...
export function margins(
  ...
  const getRows = function *() {
    for (const [index, cost] of enumerate(focus.costTotal)) {
      yield doc.tr(
        doc.td.c(index+1),
        doc.td.c(fmtNum(cost)),
        doc.td.c(fmtNum(focus.marginalCost[index])),
        doc.td.c(fmtNum(focus.revenue[index])),
        doc.td.c(fmtNum(focus.marginalBenefit[index])),
        doc.td.c(fmtNum(other.price[0] * focus.marginalCost[index])),
      );
    }
  };
...
export function firmSupplyCurve(fixedCost, fTable) {
  ...
  const t1 = doc.table(
    ...
    doc.tbody(...rowSet1.map(row => doc.tr(...row.map(c => doc.td.c(c))))),
  );
  const t2 = doc.table(
    doc.thead(doc.tr(doc.th`W`, doc.th`AVC`, doc.th`ATC`, doc.th`MC`, doc.th`Feasible`, doc.th`П`)),
    doc.tbody(...rowSet2.map(row => doc.tr(...row.map(c => doc.td.c(c))))),
  );

  return frag(
    doc.div({ className: 'c2 supplycurvetable' }).c(t1, t2),
    doc.h4`Legend`,
    doc.dl({ className: 'legendInfo' }).c(
      doc.dt`P`, doc.dd`Price`,
      doc.dt`W`, doc.dd`Workers`,
      doc.dt`Q`, doc.dd`Quantity`,
      doc.dt`FC`, doc.dd`Fixed Cost`,
      doc.dt`VC`, doc.dd`Variable Cost`,
      doc.dt`TC`, doc.dd`Total Cost`,
      doc.dt`AVC`, doc.dd`Average VC`,
      doc.dt`ATC`, doc.dd`Average  TC`,
      doc.dt`MC`, doc.dd`Marginal Cost`,
      doc.dt`П`, doc.dd`Profit`,
    ),
  );
}
...
export function shutdownConditions(fixedCost, fTable) {
  ...
  return doc.table(
    doc.caption({ className: 'footerCap' }).c(text.p.l`* Output within time frame`),
    doc.thead(doc.tr(doc.th`Condition`, doc.th`Operate?`, doc.th`When W = ?`)),
    doc.tbody(
      doc.tr(doc.th`Short Run`, doc.td.c(cond(shortRun)), doc.td.c(show(shortRun))),
      doc.tr(doc.th`Long Run`, doc.td.c(cond(longRun)), doc.td.c(show(longRun))),
    ),
  );
}
  ...
  return doc.table(
    ...
    doc.tbody(
      ...
      ...rows.map((row, i) => {
        ...
        return doc.tr(
          doc.td.c(r(row.units)),
          doc.td.c(r(lastUnits)),
          doc.td.c(r(deltaUnits)),
          doc.td.c(r(row.marginalCost)),
          doc.td.c(r(lastMCost)),
          doc.td.c(r(deltaMCost)),
          doc.td.c(r(i === 0 ? undefined : elasicity[i-1])),
        );
      }),
    ),
  );
