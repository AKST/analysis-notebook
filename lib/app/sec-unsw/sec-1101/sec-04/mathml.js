    ...
    rows(
      ...
      mover(mrow(
        mrow(
          mi`α`,
          mo`=`,
          sum.attr({ max: mi`n`, inc: mrow(mi`i`, mo`=`, mn(1)) }).c(),
          parensA(
            msub(mi`a`, mi`i`),
            mspace(4),
            inv(msub(mi`b`, mi`i`)),
...
export function supplyDemandCurvesContinuious() {
  /** @param {E.Item} qs */
  const ps = qs => call.attr({ fn: msub(mi`P`, mi`S`) }).c(qs);
  /** @param {E.Item} qd */
  const pd = qd => call.attr({ fn: msub(mi`P`, mi`D`) }).c(qd);

  return mathml.math(
    rows(
      mrow(
        mover(mrow.c(ps(mi`q`)), mtext`Supply Price`),
        mo`=`, mspace(8),
        msub(greek.alpha, mi`S`), mo`+`,
        msub(greek.beta, mi`S`), mo`·`, mi`q`,
      ),
      mrow(
        mover(mrow.c(pd(mi`q`)), mtext`Demand Price`),
        mo`=`, mspace(8),
...
export function equilibriumDiscrete() {
  ...
  /** @param {E.Item} id */
  const diff = id => call.attr({ fn: mi`D` }).c(msub(mi`q`, id));

  return mathml.math(
    rows(
      mover(mrow(
        call.attr({ fn: mi`D` }).c(mi`q`),
        mo`=`, mspace(4),
        call.attr({ fn: mtext`sign` }).c(mrow(ps(mi`q`), mo`-`, pd(mi`q`))),
