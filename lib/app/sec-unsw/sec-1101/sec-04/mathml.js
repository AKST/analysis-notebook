    ...
    rows(
      ...
      mover(mrow(
        mrow(
          mi`α`,
          mo`=`,
          sum.void({ max: mi`n`, inc: mrow(mi`i`, mo`=`, mn(1)) }),
          parensA(
            msub(mi`a`, mi`i`),
            mspace(4),
            inv(msub(mi`b`, mi`i`)),
...
export function supplyDemandCurvesContinuious() {
  /** @param {E.Item} qs */
  const ps = qs => call({ fn: msub(mi`P`, mi`S`) }).c(qs);
  /** @param {E.Item} qd */
  const pd = qd => call({ fn: msub(mi`P`, mi`D`) }).c(qd);
...
export function equilibriumDiscrete() {
  ...
  /** @param {E.Item} id */
  const diff = id => call({ fn: mi`D` }).c(msub(mi`q`, id));
  ...
  return mathml.math(
    rows(
      mover(mrow(
        call({ fn: mi`D` }).c(mi`q`),
        mo`=`, mspace(4),
        call({ fn: mtext`sign` }).c(mrow(ps(mi`q`), mo`-`, pd(mi`q`))),
