    ...
    rows(
      ...
      mover(mrow(
        mrow(
          mi`α`,
          mo`=`,
          sum.attr({ max: mi`n`, inc: mrow(mi`i`, mo`=`, mn(1)) }).of(),
          parensA(
            msub(mi`a`, mi`i`),
            mspace(4),
            inv(msub(mi`b`, mi`i`)),
      ...
      mover(
        mrow(
          call.attr({ fn: mi`ℋ` }).of(cursive.F),
          mspace(4),
          mo`=`,
          mspace(4),
          msup(mi`f`, mo`*`), mo`=`,
...
export function supplyDemandCurvesContinuious() {
  /** @param {E.Item} qs */
  const ps = qs => call.attr({ fn: msub(mi`P`, mi`S`) }).of(qs);
  /** @param {E.Item} qd */
  const pd = qd => call.attr({ fn: msub(mi`P`, mi`D`) }).of(qd);
...
export function equilibriumDiscrete() {
  ...
  /** @param {E.Item} id */
  const diff = id => call.attr({ fn: mi`D` }).of(msub(mi`q`, id));

  return mathml.math(
    rows(
      mover(mrow(
        call.attr({ fn: mi`D` }).of(mi`q`),
        mo`=`, mspace(4),
        call.attr({ fn: mtext`sign` }).of(mrow(ps(mi`q`), mo`-`, pd(mi`q`))),
