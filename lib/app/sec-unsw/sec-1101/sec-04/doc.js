...
const IMAGES = {
  pareto: doc.img.css({
    float: 'right',
    marginLeft: '8px',
  }).void({ src: './assets/econ/pareto.png', height: 144 }),
  paretoImprovingTransaction: doc.img.css({
    float: 'left',
    marginRight: '8px',
  }).void({ src: './assets/econ/pareto-improving-transaction.png', height: 120 }),
  smith: doc.img({
    src: './assets/econ/adam-smith.png',
    height: 138,
    className: 'adamSmith',
  }),
  rationing: doc.img.css({
    float: 'left',
    marginRight: '8px',
  }).void({ src: './assets/econ/rationing-2.png', height: 130, title: 'Child collecting WW2 rations' }),
  competition: doc.img.css({
    float: 'left',
    marginRight: '8px',
  }).void({ src: './assets/econ/perfect-competition.png', height: 107, title: 'Joey Chestnut 17 times winning hot dog eatting world champion' }),
  privateGoods: doc.img.css({
    float: 'left',
    marginRight: '8px',
  }).void({ src: './assets/econ/private-goods.png', height: 107, title: 'Cheese' }),
};

const INFOBOXES = {
  pareto: infobox({ title: 'Paret-who?' }).c(
    doc.div.c(
      IMAGES.pareto,
      text.p.m`
  ...
  render: () => container(
    twoThree(
      container(
        ...
        doc.div({ classList: ['container', 'dashbox'] }).c(
          doc.ul({ className: 'list-with-padded-items' }).css({
            paddingLeft: '16px',
          }).c(
            text.li.m`
...
export const introAggregateSupplyAndDemand = {
  ...
  render: (_ctx, state) => container(
    ...
    twoThree(
      ...
      infobox({ title: "Horizontal Summation" }).c(
        doc.div({ className: 'horizontal-summary-math' }).c(
