...
const IMAGES = {
  supermarketEnquiry: doc.img.css({
    float: 'left',
    marginRight: '8px',
  }).void({
    src: import.meta.resolve('@assets/econ/supermarket-enquiry.png'),
    height: 100,
  }),
}
...
const TERMS = {
  oligopoly: defineTerm({
    word: 'Oligpoly',
    meaning: [
      doc.span`An economic condition in which a small number of sellers exert control over the market of a commodity.`,
    ],
...
const INFOBOXES = {
  supermarketEnquiry: infobox({ title: '2024-25 Supermarket Enquiry' }).c(
    text.p.m`${IMAGES.supermarketEnquiry}
      ...
      barriers to entry for new partipants. [${
        doc.a({ href: LINKS.supermarketEnquiry }).c(
          'read more here'
        )
      }]`,
    text.p.m`
      ...
      applications to build a grocery store get ${
        doc.a({ href: LINKS.woolworthsButtsHeadWithCouncil }).c(
          'challenged'
        )
