...
const IMAGES = {
  losersFromTrade: doc.figure.css({
    width: '199px',
    margin: '0',
  }).c(
    doc.img.css({
      ...
      src: import.meta.resolve('@assets/econ/trade-losers.gif'),
      height: 188,
    }),
 ...
 * @returns {E.Item}
 */
const ul = (...items) => doc.ul({ className: 'list-with-padded-items' }).css({
  paddingLeft: '16px',
}).c(...items.map(p => text.li.l.c(p)));
...
export const header = {
  ...
  render: (_ctx) => container(
    twoThree(
      ...
      container(
        infobox({ title: 'Legend' }).c(container(
          text.p.m`
            Here are some symbols used through out this topic.
          `,
          doc.span.css({ fontSize: '18px' }).c(mathml.definitions()),
        )),
        infobox({ title: 'Market Conditions' }).c(container(
          text.p.m`${doc.span.css({ fontSize: '18px' }).c(
...
export const onTrade = {
  ...
  render: (_ctx) => container(
    dashbox(
      doc.div({ className: 'tradeLosers' }).c(
        container(
