 ...
 */
import { alternating, dashbox, infobox, crossSectionTable, defineTerm, readmore, todo, text } from '@prelude-uni/components.js';
...
const INFOBOX = {
  enclosure: infobox({ title: 'Enclosure' }).of(
    text.p.m`
  ...
  verticalSummation: infobox({ title: 'Vertical Summation' }).of(
    text.p.m`
      ...
      or service.
    `,
    doc.span({ style: 'font-size: 16px' }).of(
      mathml.verticalSummation()
    ),
  ),
  lindhalPrice: infobox({ title: 'Lindhal Price' }).of(
    text.p.m`
...
const OTHER_TABLES = {
  typesOfGoods: crossSectionTable({
    ...
    outcomes: {
      yes: {
        yes: alternating(['Private Good', frag(doc.br(), 'T-Shirts')]),
        no: alternating(['Common Pool Goods', frag(doc.br(), 'Natural Resources')]),
      },
      no: {
        yes: alternating(['Club Goods', frag(doc.br(), 'Cable TV')]),
        no: alternating(['Pure Public', frag(doc.br(), 'Public Radio')]),
      },
    },
    ...
    twoThree(
      dashbox(
        ...
        doc.p`
          ...
          text.b`vertically rather than horizontally`}
          (by units) to calculate aggregate demand.
        `,
        doc.p`
          The aggregated curve (or ${text.b`marginal social
  ...
  render: () => container(
    dashbox(
      twoThree(
        ...
        infobox({ title: 'Examples of Public Goods' }).of(
          text.p.m`
      ...
      twoThree(
        container(
          ...
          doc.p`
            ...
            funding them a number of ways ${text.i`(which are better
        ...
        infobox({ title: 'Fairness in Taxation' }).of(
          container(
            text.p.m`
