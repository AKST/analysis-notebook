...
const OTHER_TABLES = {
  typesOfGoods: crossSectionTable({
    ...
    outcomes: {
      yes: {
        yes: alternating(['Private Good', frag('br', 'T-Shirts')]),
        no: alternating(['Common Pool Goods', frag('br', 'Natural Resources')]),
      },
      no: {
        yes: alternating(['Club Goods', frag(['br'], 'Cable TV')]),
        no: alternating(['Pure Public', frag(['br'], 'Public Radio')]),
      },
    },
