...
export const curveInformation = ({
  ...
  const aggregateRow = doc.tr(
    doc.th`Aggregate`,
    doc.td.c(econCurve.toString(aggregateDemand)),
    doc.td`/`,
    doc.td.c(ns((aggregateDemand.i - marginalCost) * produced.contribute * 0.5)),
    doc.td`/`,
  );
  ...
  const rows = Object.entries(consumers).map(([name, v]) => {
    ...
    return doc.tr(
      doc.th.c(name),
      doc.td.c(econCurve.toString(v.curve)),
      doc.td.c(colC),
      doc.td.c(colD),
      doc.td.c(colE),
    );
  ...
  return frag(
    ...
    doc.table(
      ...
      doc.tbody(doc.tr(
        doc.td.c(ns(produced.contribute)),
        doc.td.c(ns(produced.freeRide)),
        doc.td.c(ns(
