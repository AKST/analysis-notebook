...
export function choiceOfGood({ donuts, muffins }) {
  ...
  return doc.table(
    doc.thead(doc.tr(doc.th`Action`, ...headers.map(h => doc.th.c(h)))),
    doc.tbody(
      doc.tr(doc.th`Muffins`, ...muffinRow.map(v => doc.td.c(v))),
      doc.tr(doc.th`Donuts`, ...donutRow.map(v => doc.td.c(v))),
      doc.tr(doc.th`$`, ...spendRow.map(v => doc.td.c('$'+v.toFixed(2)))),
      doc.tr(doc.th`Util Σ`, ...tUtilRow.map(v => doc.td.c(str(v)))),
      doc.tr(doc.th`Util Δ`, ...mUtilRow.map(v => doc.td.c(str(v)))),
    ),
  );
}
...
export function elasicityTable({
  ...
  return doc.table(
    ...
    doc.tbody(...m.total.map((_, i) => doc.tr(...Array.from(getRow(i), v => doc.td.c(v))))),
  );
}
