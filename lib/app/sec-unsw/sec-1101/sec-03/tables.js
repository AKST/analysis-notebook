...
function utilityTable(good, name, cfg) {
  ...
  return doc.table(
    doc.caption({ className: 'headingCap' }).c(name),
    doc.thead(doc.tr(...headers.map(h => doc.th.c(h)))),
    doc.tbody(...quantity.map(q => doc.tr(...Array.from(getRow(q), v => doc.td.c(v))))),
    doc.caption({ className: 'footerCap' }).c(caption.join(' ')),
  );
}
...
export function choiceOfGood({ donuts, muffins }) {
   ...
   * @returns {E.Item}
   */
  const goodCount = (curr, prev) => doc.span({
  ...
  return doc.table(
    doc.thead(doc.tr.cAppend(doc.th`Action`).from(headers.map(doc.th.unit))),
    doc.tbody(
      doc.tr.cAppend(doc.th`Muffins`).from(muffinRow.map(doc.td.unit)),
      doc.tr.cAppend(doc.th`Donuts`).from(donutRow.map(doc.td.unit)),
      doc.tr.cAppend(doc.th`$`).from(spendRow.map(v => doc.td.c('$'+v.toFixed(2)))),
      doc.tr.cAppend(doc.th`Util Σ`).from(tUtilRow.map(doc.td.unit)),
      doc.tr.cAppend(doc.th`Util Δ`).from(mUtilRow.map(doc.td.unit)),
    ),
  );
}
...
export function elasicityTable({
  ...
  return doc.table(
    ...
    doc.tbody.from(m.total.map((_, i) => doc.tr.from(Array.from(getRow(i), v => doc.td.c(v))))),
  );
}
