...
export const curveShifts = {
  ...
  render: () => container(
    ...
    doc.ul({ className: 'c2' }).of(
      ...
      doc.li`${doc.b`Inferior good`}: an decrease in the income.`,
      doc.li`${doc.b`Normal good`}: an increase in the income.`,
...
export const elascity = {
  ...
  render: (_ctx, state) => container(
    ...
    doc.ul({ className: 'c2' }).of(
      ...
      `,
      doc.li`${doc.b`Perfect inelastic`}:
        When the demand curve is represented by a
