...
export const ocMatrix = () => {
  ...
  const topLine = (vName, desc, output) => mrow(
    mover(
      rows(
        eq(msub(mtext.c(vName), mrow(mi`i`, mi`j`)), annotationOver(abcd)),
        eq(call.attr({ fn: mtext`OC` }).c(abcd), output),
      ),
      mrow(rows(...desc.map(l => mrow(mtext.c(l))))),
    ),
  )
...
export const ppc = () => mathml.math(
  rows(
    ...
    annotationOver.attr({ label: mi`Production Possibility Curve` }).c(
