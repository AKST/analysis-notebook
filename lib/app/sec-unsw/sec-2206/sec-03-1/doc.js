...
export const typeErrors = createDoc(() => dashbox(
  ...
  twoColumns(
    infobox({ title: 'Type 1 Error' }).of(
      text.ul(
        doc.li.of(text.i`False negative`),
        doc.li.of(text.i`Rejecting H₀ when H₀ is true`),
      ),
    ),
    infobox({ title: 'Type 2 Error' }).of(
      text.ul(
        doc.li.of(text.i`False Positive.`),
        doc.li.of(text.i`Not Rejecting H₀ when H₀ is false`),
      ),
    ),
  ),
));
