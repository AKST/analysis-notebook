...
export const vars = {
  ...
  est: {
    ...
    bj: doc.b`β̂${doc.sub`j`}`,
    rj: doc.b`r̂${doc.sub`i1`}`,
  },
  stddev: doc.b`σ${doc.sup`2`}`,
};
...
export const samplingDistribution = createDoc(() => dashbox(
  ...
  columns('2/3', [
    ...
    infobox({ title: 'First 2 Moments?' }).of(todo({}, 'describe what that means')),
  ]),
  columns('2/3', [
    doc.p`
    ...
    infobox({ title: 'Normality Assumption' }).of(
      todo({}, 'Do research on why this is acceptable and not some appeal to concequences'),
    ),
  ]),
  container(
    w3Ref({ page: 118 }, infobox({ title: 'MLR 6, Normality' }).of(
      doc.p`
...
export const hypothesisTestingTStat = createDoc(() => dashbox(
  ...
  doc.p`
    ...
    "${doc.b`How far is β̂ⱼ from 0`}?" Consider the following:
  `,
  text.ul(
    doc.li`A sample value of β̂ⱼ provides evidence against H₀`,
    doc.li`We much recognise there is a ${doc.b`sampling error`} in our estimate of β̂ⱼ`,
  ...
  doc.p`
    In order to reject H₀ we need values of ${doc.b`t${doc.sub`β̂ⱼ`}`}
...
export const hypothesisTestingOneSided = createDoc(() => dashbox(
  twoThree(
    container(
      ...
      doc.quote`Example of One Sided Alternative Hypothesis ${doc.b`H₁:  βⱼ > 0`}`,
      doc.p`
...
export const significaneLevelTable = createDoc(() => container(
  doc.details.of(
    doc.summary({ className: 'grid-layout pink' }).of(
...
export const pValues = createDoc(() => dashbox(
  ...
  twoThree(
    doc.p`
      ...
      ${doc.b`H₀: βⱼ = 0`} and is telling the smallest significance
...
export const confindenceIntervals = createDoc(() => dashbox(
  ...
  columns('even-2', [
    ...
    doc.p`
      ...
      is something like "${doc.b`H₀: βⱼ = 0`}, then using
  ...
  container(
    ...
    twoThree(
      doc.p`
        ...
        unrestricted models.
      `,
      infobox({ title: 'What is q?' }).of(doc.p`This is the number of exclusion restrictions`),
    ),
  ),
  container(
    ...
    twoThree(
      ...
      `,
      infobox({ title: 'When to use this?' }).of(
        doc.p`
