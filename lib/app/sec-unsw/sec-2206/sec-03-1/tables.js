...
export const stataOutputExample = text.figure(
  doc.table.attr({ style: 'width: 100%' }).c(
    doc.thead(doc.tr(
      doc.th``,
      doc.th`Coefficent`,
      doc.th`Std. Err.`,
...
export const significaneLevelTable = text.figure(
...
export const significanceTable = text.figure(
  doc.table.attr({ style: 'width: 100%' }).c(
    doc.thead(
      doc.tr(doc.th({ colSpan: 6 }).t`2-tailed α: .20  .10  .05  .02  .01`),
      doc.tr(...criticalLevelHeaders.map(h => doc.th.c(h))),
    ),
    doc.tbody(...criticalLevelRows.map(([first, ...rest]) => doc.tr(
      doc.th.c(first),
      ...rest.map(v => doc.td.c(v)),
    ))),
  ),
  doc.figcaption`Significance level table`
);

const dDf = doc.th({ className: 'denominator-df', rowSpan: 35 }).c(
  doc.span`Degrees of Freedom (Denominator)`
);

export const criticalValuesFDistribution10p = text.figure(
  doc.table.attr({ className: 'fDistributionTable' }).c(
    doc.thead(
      doc.tr(
        doc.th`df₂\\df₁`,
        doc.th({ colSpan: 11 }).t`Degrees of Freedom (Numerator)`,
      ),
      doc.tr(
        doc.th({ colSpan: 2 }).t``,
        doc.th`1`,
...
export const criticalValuesFDistribution5p = text.figure(
  doc.table.attr({ className: 'fDistributionTable' }).c(
    doc.thead(
      doc.tr(
        doc.th`df₂\\df₁`,
        doc.th({ colSpan: 11 }).t`Numerator Degrees of Freedom`,
      ),
      doc.tr(
        doc.th({ colSpan: 2 }).t``,
        doc.th`1`,
...
export const criticalValuesFDistribution1p = text.figure(
  doc.table.attr({ className: 'fDistributionTable' }).c(
    doc.thead(
      doc.tr(
        doc.th`df₂\\df₁`,
