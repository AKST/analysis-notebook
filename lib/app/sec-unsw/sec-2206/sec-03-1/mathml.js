...
export const normality = text.figure(
  mathml.math.attr({ style: 'font-size: 16px' }).c(
    rows(
      mrow(
...
export const samplingDistribution = text.figure(
  mathml.math.attr({ style: 'font-size: 14px' }).c(
    rows(
      mrow(
...
export const degreesOfFreedom = text.figure(
  ...
  doc.figcaption.attr({ style: 'font-size: 8px' }).c(
...
export const hypothSet = (param, cons, thres, title, op) => text.figure(
  mathml.math(
    rows(
      munder(
        table.attr({ columnalign: 'right left' }).c(
          [mrow(msub(mi`H`, mn(0)), mo`:`), mrow(param(mathml), mo.c(cons.null), thres(mathml))],
          [mrow(msub(mi`H`, mn(1)), mo`:`), mrow(param(mathml), mo.c(cons.alt), thres(mathml))],
        ),
        ...
        munder(
          mrow(
            op.test === 'one-sided' ? mrow(
              multiscripts.attr({ sup: [mi`t`, undefined] }).c(param(mathml)),
              mspace(4), mo.c(cons.alt),
              mspace(4), mi`c`,
            ) : mrow(
              mo`|`, mi`c`, mo`|`,
              mspace(4), mo`<`, mspace(4),
...
export const exampleOfMultiParaHypothesis = {
  ...
  matrixP2: text.figure(
    mathml.math(
      table.attr({ columnalign: 'right center left center left' }).c(
        [
          mi`V`,
          mo`=`,
          Var(SPECIAL.greek.circumflex.beta),
...
export const fStatInTermsOfRSq = text.figure(
  mathml.math.attr({ style: 'font-size: 10px' }).c(
    eq(
      eq(
        mi`F`,
        mfrac(
