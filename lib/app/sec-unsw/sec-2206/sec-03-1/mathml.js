...
const summaise = sum.void({ max: mi`n`, inc: mrow(mi`i`, mo`=`, mn(0)) });
...
export const normality = text.figure(
  mathml.math({ style: 'font-size: 16px' }).c(
    rows(
      mrow(
...
export const samplingDistribution = text.figure(
  mathml.math({ style: 'font-size: 14px' }).c(
    rows(
      mrow(
...
export const degreesOfFreedom = text.figure(
  ...
  doc.figcaption({ style: 'font-size: 8px' }).c(
        ...
        munder(
          mrow(
            op.test === 'one-sided' ? mrow(
              multiscripts({ sup: [mi`t`, undefined] }).c(param(mathml)),
...
export const fStatInTermsOfRSq = text.figure(
  mathml.math({ style: 'font-size: 10px' }).c(
    eq(
      eq(
        mi`F`,
        mfrac(
