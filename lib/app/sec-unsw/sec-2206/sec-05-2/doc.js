          ...
          text.ol(
            ...
            doc.li`Either${text.ol(
              ...
              doc.li`LM statistic${text.ul(
                doc.li`Get P value using 𝜒² dist`,
              )}`,
            )}`,
          ),
        ),
...
export const weightedLeastSquares = createDoc(() => container(
  ...
  dashbox(
    twoThree(
      container(
        ...
        doc.p`
          ...
          written like ${doc.b`𝜎² · income`}. In practise that means:
        `,
        text.ul(
        ...
        doc.p`
          ...
          are the best linear unbiased estimators of the 𝛽ⱼ and
      ...
      container(
        ...
        infobox({ title: "WLS, Weighted by what?" }).of(
          doc.p`
            The name comes from the the fact that 𝛽ⱼ*
        ...
        infobox({ title: "Functional form misspecification" }).of(
          doc.p`
            For WLS to be consistent for the 𝛽ⱼ, it is not
    ...
    container(
      ...
      `,
      doc.p`
        If the parameters 𝛿ⱼ were known, then we would just apply WLS, as in
        ...
        estimates to construct weights. To estimate 𝛿ⱼ, we will transform this
      ...
      doc.p`
        ...
        from 𝛿₀ (this is not important in implementing WLS). The dependent
        variable is the log of the squared error, and because it satisifies
        the Gauss-Markov assumptions, we can get unbiased estimators of 𝛿ⱼ
        using OLS.
      `,
      twoColumns(
        doc.p`
      ...
      doc.p`
        ...
        Var(y, x) = 𝜎²h(x), are no longer valid, even in large samples.
