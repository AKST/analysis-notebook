...
const sumConst = sum({ inc: eq.sub(mi`i`, mn(1)), max: mi`n` });
const h = call({ fn: mtext`h` });
const SE = call({ fn: mtext`SE` });
const SD = call({ fn: mtext`StdDev` });
const Expect = call({ fn: mtext`E` });
const ExpectS = call({ fn: mtext`E`, paren: '[]' });
...
export const wls = {
  variance: {
      ...
      math(
        mtable(
          eq.mtr({ pos: ['right', 'left'] })(
            ...
            mul0(msup(mi`𝜎`, mn(2)), h(mi`x`)),
          ),
          op.gt.mtr({ pos: ['right', 'left'] })(h(mi`x`), mn(0)),
        ),
      ),
      doc.figcaption`Var(u) under Heteroskedacity`,
    ),
    populationRandomDraw: doc.figure(
      math(
        eq(
          msubsup(greek.sigma, mi`i`, mn(2)),
    ...
    solveForResidualVariance: doc.figure(
      math(
        eq(
          SD(cond(msub(mi`u`, mi`i`), msub(mi`x`, mi`i`))),
          mul0(mi`𝜎`, msqrt(msub(mi`x`, mi`i`))),
        ),
      ),
        ...
        mtable(
          eq.mtr({ pos: ['right', 'left'] })(
            ...
            eq(
              ...
              mul0(msup(mi`𝜎`, mn(2)), msub(mi`h`, mi`i`)),
            ),
          ),
          eq.mtr({ pos: ['right', 'left'] })(
            ...
            eq(
              ...
              hFrac(mul0.paren(msup(mi`𝜎`, mn(2)), msub(mi`h`, mi`i`))),
              msup(mi`𝜎`, mn(2)),
            ),
          ),
          eq.mtr({ pos: ['right', 'left'] })(
            ...
            add(
              hSqrtFrac(msub(mi`𝛽`, mn(0))),
              mul0(msub(mi`𝛽`, mn(1)), hSqrtFrac(msub(mi`x`, mrow(mi`i`, mspace(1), mn(1))))),
              mul0(msub(mi`𝛽`, mn(2)), hSqrtFrac(msub(mi`x`, mrow(mi`i`, mspace(1), mn(2))))),
              mo`⋯`,
              mul0(msub(mi`𝛽`, mi`k`), hSqrtFrac(msub(mi`x`, mrow(mi`i`, mspace(1), mi`k`)))),
          ...
          eq.mtr({ pos: ['right', 'left'] })(
            ...
            add(
              mul0(msub(mi`𝛽`, mn(0)), msubsup(mi`x`, mrow(mi`i`, mn(0)), mo`*`)),
              mul0(msub(mi`𝛽`, mn(1)), msubsup(mi`x`, mrow(mi`i`, mn(1)), mo`*`)),
              mul0(msub(mi`𝛽`, mn(2)), msubsup(mi`x`, mrow(mi`i`, mn(2)), mo`*`)),
              mo`⋯`,
              mul0(msub(mi`𝛽`, mi`k`), msubsup(mi`x`, mrow(mi`i`, mi`k`), mo`*`)),
...
export const fgls = {
  ...
  linearForm: doc.figure(
    math(
      mtable(
        ...
        eq.mtr({ pos: ['right', 'left'] })(
          call({ fn: mtext`log` })(msup(mi`u`, mn(2))),
          add(
  ...
  estimate: doc.figure(
    math(
      eq(
        msub(mi`ĥ`, mi`i`),
        call({ fn: mtext`exp` })(msub(mi`ĝ`, mi`i`)),
      ),
    ),
    doc.figcaption`Estimated HetroSkedascity`,
  ),
  alternative: doc.figure(
    math(
      eq(
        call({ fn: mtext`log` })(msup(mi`û`, mn(2))),
        add(
          msub(greek.alpha, mn(0)),
