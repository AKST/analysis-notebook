...
export const unbiasedEstimatorUnderMlr5 = text.figure(
  math(
    eq(
      call.attr({ fn: mover(mtext`Var`, mo`^`) }).of(
        msub(mi`β̂`, mn(1))
      ),
      mfrac(
        msup(mi`σ̂`, mn(2)),
...
export const testingForHeteroskedasticity = {
  ...
  hypothesis: {
    null: text.figure(
      math(
        table({ columnalign: 'right left' }).of(
          [
    ...
    residual: text.figure(
      math(
        mtable(
          ...
          op.gt.mtr({ pos: ['right', 'left'] }).of(h(mi`x`), mn(0)),
        ),
      ),
...
export const fgls = {
  ...
  linearForm: text.figure(
    math(
      mtable(
        ...
        eq.mtr({ pos: ['right', 'left'] }).of(
          call({ fn: mtext`log` }).of(msup(mi`u`, mn(2))),
          add(
  ...
  estimate: text.figure(
    math(
      eq(
        msub(mi`ĥ`, mi`i`),
        call({ fn: mtext`exp` }).of(msub(mi`ĝ`, mi`i`)),
      ),
    ),
  ...
  alternative: text.figure(
    math(
      eq(
        call({ fn: mtext`log` }).of(msup(mi`û`, mn(2))),
        add(
          msub(greek.alpha, mn(0)),
