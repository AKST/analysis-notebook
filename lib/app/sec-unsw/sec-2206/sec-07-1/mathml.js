    ...
    resetVariant: text.figure(
      math(
        eq(mi`y`, add(
          ...
          mo`⋯`,
          mul0(msub(mi`β̂`, mi`k`), msub(mi`x`, mi`k`)),
          mul0(msub(mi`δ̂`, mn(1)), msup(mi`ŷ`, mn({ style: 'color: #ff0099' }).c(2))),
          mul0(msub(mi`δ̂`, mn(2)), msup(mi`ŷ`, mn({ style: 'color: #ff0099' }).c(3))),
        )),
      ),
...
export const nonnested = {
  example: text.figure(
    math(
      mtable(
        eq.mtr({ pos: ['right', 'left'] }).c(mi`y`, add(
          msub(mi`β`, mn(0)),
  ...
  mizon: text.figure(
    math(
      rows(
        ...
        mrow(
          mrow({ style: 'padding: 2px; border: 0.5px solid var(--fg-white)' }).c(
...
export const proxy = {
  ...
  assumption: text.figure(
    math(
      eq(
        call({ fn: mtext`E` }).c(op.cond(msubsup(mi`x`, mn(3), mo`*`), mrow(msub(mi`x`, mn(1)), op.comma(msub(mi`x`, mn(2)), msub(mi`x`, mn(3)))))),
        call({ fn: mtext`E` }).c(op.cond(msubsup(mi`x`, mn(3), mo`*`), msub(mi`x`, mn(3)))),
        add(
          msub(mi`δ`, mn(0)),
          mul0(msub(mi`δ`, mn(3)), msub(mi`x`, mn(3)))
        )
...
export const potentialOutcomes = {
  ...
  conditionalExpectations: text.figure(
    math(
      mtable(
        eq.mtr({ pos: ['right', 'left'] }).c(
          call({ fn: mtext`E` }).c(op.cond(mrow(mi`v`, add.paren(mn(0))), mrow(mi`w`, op.comma(mi`x`)))),
          eq(
            call({ fn: mtext`E` }).c(op.cond(mrow(mi`v`, add.paren(mn(0))), mi`x`)),
            mul0(minus.paren(mi`x`, call({ fn: mtext`E` }).c(mi`x`)), msub(mi`β`, mn(0)))
          ),
        ),
        eq.mtr({ pos: ['right', 'left'] }).c(
          call({ fn: mtext`E` }).c(op.cond(mrow(mi`v`, add.paren(mn(1))), mrow(mi`w`, op.comma(mi`x`)))),
          eq(
            call({ fn: mtext`E` }).c(op.cond(mrow(mi`v`, add.paren(mn(1))), mi`x`)),
            mul0(minus.paren(mi`x`, call({ fn: mtext`E` }).c(mi`x`)), msub(mi`β`, mn(1)))
          ),
        )
      )
    ),
...
export const measurementError = {
  ...
  assumption: {
    depVarErrorUnrelatedToExplanatoryVar: text.figure(
      math(
        op.gt(
          eq(
            call({ fn: mtext`Var` }).c(add(mi`u`, msub(mi`e`, mn(0)))),
            add(
    ...
    expErrCoVarA: text.figure(
      math(eq(call({ fn: mtext`CoVar` }).c(msub(mi`e`, mi`j`), msub(mi`x`, mi`j`)), mn(0))),
      doc.figcaption`E covariance with observed`,
    ),
    expErrCoVarB: text.figure(
      math(eq(call({ fn: mtext`CoVar` }).c(msub(mi`e`, mi`j`), msubsup(mi`x`, mi`j`, mo`*`)), mn(0))),
    ...
    expErrCoVarAImplication: text.figure(
      math({ style: '--fontsize-math-m: 14px' }).c(
        table({ columnalign: 'left right center left' }).c(
          [mo`∵`, ...eq.array(
            call({ fn: mtext`CoVar` }).c(msub(mi`e`, mi`j`), msub(mi`x`, mi`j`)),
            mn(0),
          )],
          [mo`⇒`, ...op.gt.array(
            abs(call({ fn: mtext`CoVar` }).c(msub(mi`e`, mi`j`), msubsup(mi`x`, mi`j`, mo`*`))),
            mn(0),
          )],
        ),
        table({ columnalign: 'left right center left' }).c(
    ...
    expErrCoVarB2: text.figure(
      math(
        op.imply(
          eq(call({ fn: mtext`CoVar` }).c(msub(mi`e`, mi`j`), msubsup(mi`x`, mi`j`, mo`*`)), mn(0)),
  ...
  cev: {
    covariance: text.figure(
      math(
        eq(
          call({ fn: mtext`Cov` }).c(msub(mi`x`, mi`j`), minus(mi`u`, mul0(msub(mi`β`, mi`j`), msub(mi`e`, mi`j`)))),
          mul0(neg(msub(mi`β`, mi`j`)), call({ fn: mtext`Cov` }).c(msub(mi`x`, mi`j`), msub(mi`e`, mi`j`))),
        ...
        mtable(
          eq.mtr({ pos: ['right', 'left'] }).c(
            ...
            add(
              ...
              mfrac(
                call({ fn: mtext`Cov` }).c(msub(mi`x`, mi`j`), minus(mi`u`, mul0(msub(mi`β`, mi`j`), msub(mi`e`, mi`j`)))),
                call({ fn: mtext`Var` }).c(msub(mi`x`, mi`j`))
              )
            )
          ),
          eq.mtr({ pos: ['right', 'left'] }).c(
    ...
    r: math(
      mtable(
        mtr(
          mtd({ columnalign: 'right' }).c(mtext`where`),
          mtd({ columnalign: 'right' }).c(mi`r`),
          mtd({ columnalign: 'center' }).c(mo`=`),
          mtd({ columnalign: 'left' }).c(mtext`the population error ...`),
        ),
        mtr(
          mtd({ columnalign: 'right' }).c(mtext`in`),
          mtd({ columnalign: 'right' }).c(msubsup(mi`x`, mi`j`, mi`*`)),
          mtd({ columnalign: 'center' }).c(mo`=`),
          mtd({ columnalign: 'left' }).c(
            add(
              msub(mi`α`, mn(0)),
  ...
  lad: text.figure(
    math(
      mrow(
        ...
        mspace(4),
        sum({ max: mi`n`, inc: eq.sub(mi`i`, mn(1)) }).c(
          abs(
            rows(
              minus(
                msub(mi`y`, mi`i`),
  ...
  implication: text.figure(
    math({ style: '--fontsize-math-m: 14px' }).c(
      table({ columnalign: 'left right center left' }).c(
        ...
        [
          mo`∧`,
          call({ fn: mtext`Med` }).c(op.cond(mi`u`, mi`x`)),
          mo`=`,
          mn(0)
        ],
        [
          mo`⇒`,
          call({ fn: mtext`Med`, paren: '[]' }).c(
            op.cond(call({ fn: mtext`log` }).c(mi`y`), mi`x`),
          ),
          mo`=`,
          add(
            msub(mi`β`, mn(0)),
        ...
        [
          mo`∧`,
          call({ fn: mtext`Med` }).c(op.cond(mi`y`, mi`x`)),
          mo`=`,
          call({ fn: mtext`exp` }).c(
            add(
              msub(mi`β`, mn(0)),
