  ...
  twoThree(
    container(
      doc.h1.c(
        ...
        doc.br(),
        doc.small({ style: 'color: #aaaaff' }).t`ECON2206, W7, Lecture 1`,
      ),
    ),
    infobox({ title: 'Resources' }).c(
      doc.p`See here for resources relating to the lesson`,
      text.ul(
        doc.li.c(LINKS.paper.ramsey1969.t`Ramsey 1969, specification errors`),
      ),
    ),
  ),
));
  ...
  dashbox(
    twoThree(
      container(
        ...
        doc.p`
          An ${text.b`F Test`} is already one such example we have to
          testing mispecified founctional form, in that it allows us
          to test for ${text.b`join exclusion restrictions`}. However
        ...
        doc.p`
          ...
          can be difficult. The ${text.b`RESET test`} is one such tool at our
          disposal in approaching this problem. ${text.b`The general idea
          behind RESET is`}:
        `,
        text.ul(
          doc.li.c(text.i`
        ...
        doc.p`
          ...
          trick to the one seen in the simplified ${text.b`white test`}
        ...
        text.ul(
          doc.li`
            An F Statistic (with a df of ${text.b`n - k - 3`}),
            where a significant F statistic suggests some
            sort of functional problem.
          `,
          doc.li`
            You can alo test with an LM statistic using a χ²
      ...
      container(
        ...
        infobox({ title: "Adding Quadratics" }).c(
          doc.p`
        ...
        doc.dl(
          doc.dt`RESET`,
          doc.dd`
            RESET stands for ${text.b`Regression Specification
            Error Test`}, and it was originally designed by
            ${LINKS.paper.ramsey1969.t`Ramsey`} published in his
            1969 paper.
          `,
        ),
        infobox({ title: "ŷ² and ŷ³ as nonlinear functions" }).c(
    ...
    container(
      ...
      doc.p`
        One method is the one divised by ${LINKS.paper.mizon1986.t`Mizon
      ...
      doc.p`
        ...
        answer to that is the ${LINKS.paper.davidson1981.t`davidson
      ...
      `,
      doc.p`
        To test we just check if θ₁ has a singificant t statistic
        (two sided test) to reject the model in which the fitted
        value was added. ${text.b`It's worthwhile doing both`}, as
        it's possible no clear winner emerges with both model
        rejected or neither models are rejected. ${text.b`If neither
...
export const usingProxyVariables = createDoc(() => container(
  ...
  dashbox(
    twoThree(
      container(
        ...
        text.ol(
          ...
          `,
          doc.li`
            ${text.b`v`} in the hypothetical model for deriving
            ${text.b`x₃*`} (shown below0 is not correlated with any of
    ...
    twoColumns(
      ...
      doc.div({ style: '--fontsize-math-m: 12px' }).c(mathml.proxy.solve),
    ),
    container(
      ...
      doc.p`
        ...
        all other variables.
      `,
      doc.h4`Assumption Summary`,
      mathml.proxy.summary,
      doc.p`
    ...
    container(
      ...
      doc.blockquote`Using a lagged dependent variable in a cross-sectional equation increases the data requirements`,
      doc.p`
        ...
        account for in other ways. ${text.b`Inertial effects`} are also
  ...
  dashbox(
    container(
      doc.p`
        ...
        grocery spending).
      `,
      text.ul({ className: 'c2', style: 'grid-gap: 32px' }).c(
        doc.li`
      ...
      doc.p`
        ...
        consider the origin of these errors, even so — ${text.i`perhaps there's a temptation to
        consider them close enough to each other that they can be treated
        identically`}?
      `,
      doc.p`
        This would be a mistake, ${text.b.c(text.b`its worth stressing
        ${text.i`two`} ways in which how the nature of these two things are
        fundermentally different`)}:
      `,
      text.ol({ className: 'c2', style: 'grid-gap: 32px' }).c(
        doc.li`
          With ${text.b.c(text.b`Proxy Variables`)}, we are looking for a
          variable that is somehow associated with the unobserved variable.
          With ${text.b.c(text.i`Measurement Error`)}, the variable that we
        ...
        `,
        doc.li`
          With ${text.b.c(text.i`Measurement Error`)} the primary thing of
          interst is  mismeasured independent variable. However with
          ${text.b.c(text.b`Proxy Variables`)}, the partial effect of the
          omitted variable is rarely of central interest, as we are
          usually more concerned with the effects of the other independent
          variables. The goal of a ${text.b.c(text.b`Proxy Variables`)} is
          typically to control for the effect of the unobserved variable.
        `,
      ),
      doc.p`
        In summary, ${text.b.c(text.i`one is a subsitute for what we like to
        measure`, ' ', text.b`the other is a poor measurment of what we would
        like to measure`)}.
      `,
    ),
    ...
    twoThree(
      container(
        doc.blockquote`y* is not measured instead we have y`,
        ...
        doc.p`
          So one assumptions we would like to make is the ${text.b`measurement
          ...
          estimator of the intercept (β₀). However assumption being incorrect
      ...
      container(
        ...
        infobox({ title: 'On error zero mean assumption' }).c(
          doc.p`
        ...
        doc.dl(
          doc.dt`Composite Error`,
          doc.dd`
            In measurement error, composite error
            is a combination of the residual (${text.b`u`})
            the error term (${text.b`eⱼ`}) and the coefficent
            (${text.b`βⱼ`}) of the observed model as:
            ${doc.br()}
            ${doc.br()}
            ${text.b`u - βⱼeⱼ`}
          `,
    ...
    twoThree(
      container(
        doc.blockquote`x* is not measured instead we have x`,
        doc.p`
        ...
        doc.p`
          ...
          ${text.b`plantonic variable`} and the ${text.b`measured variable`}.
        ...
        text.ol(
          doc.li`
            measurement error of zero correlates ${text.b`observed`} value
          `,
          doc.li`
            measurement error of zero correlates ${text.b`unobserved`} value
          `,
        ),
        twoColumns(
      ...
      container(
        ...
        doc.dl(
          ...
          doc.dt`CEV`,
          doc.dd`
            ${text.b`classical errors-in-variables`}, which is
    ...
    container(
      ...
      twoColumns(
        ...
        doc.div({ style: '--fontsize-math-m: 14px' }).c(
      ...
      doc.p`
        ...
        a variance in β̂ⱼ than what we would otherwise get when
    ...
    container(
      ...
      twoColumns(
        doc.p`
          Given the above β̂ⱼ is always closer to zero than βⱼ which
          gives us an ${text.b`attenuation bias`} due to ${text.b`CEV`}.
          So on average estimated OLS effect will be attenuated. So
          if βⱼ is positive, β̂ⱼ will tend to underestimate βⱼ.
          Things get more complicated when we add more explanatory
          variables, ${text.b`all our estimators will end up with
          bias not just β̂ⱼ`}.
        `,
        container(
    ...
    twoThree(
      container(
        ...
        doc.p`
          ...
          ${text.b`Z${doc.sub`ik`}`}, is defined to be ${text.b`x${doc.sub`ik`}`}
          when ${text.b`x${doc.sub`ik`}`} is observed, and zero otherwise.
          The second variable is a “missing data indicator,” say
          ${text.b`m${doc.sub`ik`}`}, which equals one when
          ${text.b`x${doc.sub`ik`}`} is missing and equals zero when
          ${text.b`x${doc.sub`ik`}`} is observed. It looks something like this:
        `,
        ...
        doc.p`
          ...
          always led any kind of statistical problem, ${text.b`exogenous
        ...
        doc.p`
          ...
          ${text.b`E(y|x₁,⋯,xₖ)`} any subset of the population described by
        ...
        doc.p`
          ...
          ${text.b`endogenous sample selection`}. If the sample is
...
export const leastAbsoluteDeviations = createDoc(() => container(
  ...
  dashbox(
    doc.blockquote`LAD estimators are justified only in large samples.`,
    twoThree(
      container(
        mathml.lad.lad,
        doc.p`
    ...
    container(
      ...
      doc.p`
        ...
        estimate the parameters of the ${text.b`conditional median`} of
      ...
      doc.p`
        ...
        conditional mean function ${text.i`(Given the fact it is based on
      ...
      twoThree(
        container(
          ...
          text.ul(
            doc.li`
              ${text.b`robust regression`}: In the statistics literature, a
              robust regression estimator is relatively insensitive to extreme
              observations.
            `,
            doc.li`
              ${text.b`quantile regression`}: which is used to estimate the
