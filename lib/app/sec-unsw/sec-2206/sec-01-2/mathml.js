...
import { frag, mathml } from '@app/prelude.js';
import { text } from '@prelude-uni/components.js';
import { rows, call, sum, inv, table, SPECIAL, parensA, parensC, op } from '@prelude-uni/mathml.js';
...
const { eqId, eq, add, cond, mul0: mul, minus, gt } = op;
...
/** @param {...E.Item} body */
const E = (...body) => call({ fn: mi`E` }).c(frag(...body))

const Covar = bindOf(call({ fn: mtext`Covar` }));
const Var = bindOf(call({ fn: mtext`Var` }))
const Range = bindOf(call({ fn: mtext`Range` }))

/** @param {...E.Item} items */
const summaise = (...items) => sum({
  max: mi`n`,
...
export const jointProbabilityDistributionOfXAndYInPop = {
  ...
  p2_13: mathml.math(eq(
    mrow(
      mi`E`,
      mul.square(vars.x, minus.paren(minus(vars.y, vars.b0), mul(vars.b1, vars.x))),
    ),
    mn(0),
  )),
};
...
export const sumOfFirstOrderConditions = {
  a: mathml.math(eq(
    frag(
      ...
      summaise(
        minus.paren(minus(vars.yi, vars.b0Est), frag(vars.b1Est, vars.xi)),
      ),
    ),
    mn(0),
  )),
  b: mathml.math(eq(
    mul(
      ...
      summaise(
        vars.xi,
        minus.paren(minus(vars.yi, vars.b0Est), mul(vars.b1Est, vars.xi)),
      ),
    ),
    mn(0),
  )),
...
export const betaOneEstimation = {
  assumption: mathml.math(
    summaise(
      gt(msup(minus.paren(vars.xi, vars.xsm), mn(2)), mn(0)),
    ),
  ),
  rearrangingAbove: mathml.math(
    rows(
      eq(
        summaise(
          mul(
            vars.xi,
            minus.square(
              minus(vars.yi, minus.paren(vars.ysm, mul(vars.b1Est, vars.xsm))),
      ...
      eq(
        summaise(mul(vars.xi, minus.paren(vars.yi, vars.ysm))),
        mul(vars.b1Est, summaise(mul(vars.xi, minus.paren(vars.xi, vars.xsm)))),
      ),
    )
  ),
  alsoNotingThat: mathml.math(
    rows(
      eq(
        summaise(mul(vars.xi, minus.paren(vars.xi, vars.xsm))),
        summaise(msup(minus.paren(vars.xi, vars.xsm), mn(2))),
      ),
      eq(
        summaise(mul(vars.xi, minus.paren(vars.yi, vars.ysm))),
        summaise(mul(minus.paren(vars.xi, vars.xsm), minus.paren(vars.yi, vars.ysm))),
      ),
    ),
  ),
  betaOneOLS: mathml.math(
    eq(vars.b1Est, mfrac(
      summaise(mul(minus.paren(vars.xi, vars.xsm), minus.paren(vars.yi, vars.ysm))),
      msup(summaise(minus.paren(vars.xi, vars.xsm)), mn(2)),
    )),
  ),
...
export const sumOfSquares = {
  sst: mathml.math(eqId(mi`SST`, summaise(msup(minus.paren(vars.yi, vars.ysm), mn(2))))),
  sse: mathml.math(eqId(mi`SSE`, summaise(msup(minus.paren(vars.yEstI, vars.ysm), mn(2))))),
  ...
  eq(
    ...
    mfrac(
      msup(
        parensA(
          summaise(
            minus.paren(msub(mi`y`, mi`i`), mi`ȳ`),
            minus.paren(msub(mi`ŷ`, mi`i`), mi`ŷ̄`)
          ),
        ),
        mn(2)
      ),
      mul(
        parensA(
          summaise(msup(minus.paren(msub(mi`y`, mi`i`), mi`ȳ`), mn(2))),
        ),
        parensA(
          summaise(msup(minus.paren(msub(mi`ŷ`, mi`i`), mi`ŷ̄`), mn(2))),
        ),
      )
    )
  )
...
export const whatIsOrdinaryLeastSquares = mathml.math(
  eq(
    ...
    mtext`min`,
    summaise(
      msup(minus.square(vars.yi, add(vars.b0, mul(vars.b1, vars.xi))), mn(2)),
    ),
  ),
);
...
export const simpleLinearRegressionAnalysis = {
  varB1EstA: mathml.math(
    eq(
      Var(vars.b1Est),
      mfrac(msup(sigma, mn(2)), summaise(msup(minus.paren(vars.xi, vars.xsm), mn(2)))),
  ...
  varB1EstC: mathml.math(
    table({ columnalign: 'right center left center left' }).c(
      [
        Var(vars.b1Est),
        mo`=`, mul(
          msup(parensA(inv(msub(mi`SST`, mi`x`))), mn(2)),
          Var(summaise(mul(vars.ui, minus.paren(vars.xi, vars.xsm)))),
        ),
        mo`=`, mul(
          msup(parensA(inv(msub(mi`SST`, mi`x`))), mn(2)),
          mul(
      ...
      [
        ...
        mo`=`, mul(
          ...
          summaise(
            msup(sigma, mn(2)),
            msup(minus.paren(vars.xi, vars.xsm), mn(2)),
          ),
        ),
        mo`=`, mrow(),
      ],
