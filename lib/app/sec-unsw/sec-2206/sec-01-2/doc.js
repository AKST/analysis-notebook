...
const LINKS = {
  /** @type {E.Item} */
  linearRegressionInStata: doc.a({
    href: 'https://www.princeton.edu/~otorres/Regression101.pdf',
  }).t`Linear Regression in Stata`,
};

const TABLES = {
  terminology: doc.table(
    doc.thead(doc.tr(doc.th`Y`, doc.th`X`)),
    doc.tbody(
      doc.tr(doc.td`Dependent variable`, doc.td`Independent variable`),
      doc.tr(doc.td`Explained variable`, doc.td`Explanatory variable`),
      doc.tr(doc.td`Response variable`, doc.td`Control variable`),
      doc.tr(doc.td`Predicted variable`, doc.td`Predictor variable`),
      doc.tr(doc.td`Regressand variable`, doc.td`Regressor variable`),
    ),
  ),
  logFunctionalForms: twoColumns(
    doc.table(
      doc.thead(doc.tr(doc.th`Model`, doc.th`Dependent Var`)),
      doc.tbody(
        doc.tr(doc.td`Level-Level`, doc.td`y`),
        doc.tr(doc.td`Level-Log`, doc.td`y`),
        doc.tr(doc.td`Log-Level`, doc.td`log(y)`),
        doc.tr(doc.td`Log-Log`, doc.td`log(y)`),
      ),
    ),
    doc.table(
      doc.thead(doc.tr(doc.th`Independent Var`, doc.th`Interpreation of β̂₁`)),
      doc.tbody(
        doc.tr(doc.td`x`, doc.td`Δy = β₁ Δx`),
        doc.tr(doc.td`log(x)`, doc.td`Δy = (β₁/100) %Δx`),
        doc.tr(doc.td`x`, doc.td`%Δy = (100β₁) Δx`),
        doc.tr(doc.td`log(x)`, doc.td`%Δy = β₁ %Δx`),
      ),
    ),
  ),
};
...
const INFOBOX = {
  populationParamaters: infobox({ title: 'Population Parameters' }).c(
    doc.p`
  ...
  noteOnIndexSyntax: infobox({ title: 'Note on Index Syntax' }).c(
    doc.p`
      ...
      often generalised as an i, as in ${text.i`xᵢ`}
      for the x of the postion i where i is a number between
      1 and n (sometimes this can be 0 & n-1).
    `,
    mathml.indexSyntax,
  ),
  propertiesOfSummation: infobox({ title: 'Properties of Summation' }).c(
    todo({}, 'Write me later'),
  ),
  betaOneIsARandomVariable: infobox({ title: 'β̂₁ is a random variable' }).c(
    doc.p`
...
export const intro = createDoc(() => container(
  twoThree(
    container(
      doc.h1.c(
        'Simple Regression Model', doc.br(),
        doc.small({ style: 'color: #aaaaff' }).c('ECON2206, W1, Lecture 2'),
      ),
      doc.p`
    ...
    infobox({ title: 'Resources' }).c(
      text.ul(
        ...
        doc.li.c(LINKS.linearRegressionInStata),
      ),
    ),
  ),
));
...
export const models = createDoc(() => dashbox(
  ...
  twoColumns(
    ...
    container(
      ...
      doc.p`
        This can also be thought of an ${text.b`Econometric model`}
...
export const linearRegression = createDoc(() => dashbox(
  ...
  twoColumns(
    container(
      ...
      doc.p`
        ...
        is the ${text.b`slope`} of the relationship between x and y.
      `,
    ),
    container(
  ...
  container(
    ...
    doc.p`
      This says that the average ${text.b`u`} across the
      population determined by x is equal to the average ${text.b`u`}
      over the population. Put another way, this says that u is ${
      text.b`mean independent`} of x. Combining this with E(u) = 0,
  ...
  container(
    ...
    `,
    text.ul(
      doc.li`${text.b`Systematic part of y`}: E(y|x) or β₀ + β₁x`,
      doc.li`${text.b`Unsystematic part of y`}: u, or the part y not explained by x`,
    ),
...
export const ordinaryLeastSquaresEstimate = createDoc(() => dashbox(
  ...
  twoThree(
    container(
      doc.p`
        ...
        text.b`n`} each the X, Y & U for each observation is
      ...
      doc.p`
        ...
        we use ${text.b`Sum of first order conditions`}.
      `,
    ),
    container(
  ...
  container(
    ...
    twoThree(
      doc.p`
        ...
        injecting our above definition of ${text.b`β̂₀`} into the ${
        text.b`Sum of first order conditions`}, and from there we'll
    ...
    twoColumns(
      text.figure(mathml.betaOneEstimation.rearrangingAbove, doc.figcaption`
        Start by plugging β̂₀ into${doc.br()}
        the Sum of 1st Order Condition
      `),
      text.figure(mathml.betaOneEstimation.alsoNotingThat, doc.figcaption`
        Before isolating for β̂₁ note${doc.br()}
        how these can be rearranged
      `),
    ),
    doc.p`
      Given the sum of (x${doc.sub`i`}-x̄)² is greater than zero we
      compute β̂₁ as shown, which can be further simplifed to be σ̂${
      doc.sub`y`} divided by σ̂${doc.sub`x`} times ρ̂${
      doc.sub`xy`} which is the correlation (Noting that σ̂ is the
  ...
  container(
    ...
    doc.p`
      ...
      ${text.b`fitted value`} of y, and the difference between
      the observed yᵢ and its fitted value ŷᵢ is called the
      ${text.b`residual`} as ûᵢ. Note this is actually different
  ...
  container(
    ...
    doc.p`
      To measure ${text.b`goodness of fit`} we'll first define
      a few more concepts which we will use to compute ${text.b`R²`}
...
export const assumptionsOfUnbiasednessInOLS = createDoc(() => dashbox(
  container(
    ...
    doc.p`
      These are also called the ${text.b`Gauss Markov Assumptions`}
...
export const summary = createDoc(() => dashbox(
  ...
  note(
    doc.li`Population regression`,
    doc.li`Sample regression`,
    doc.li`Estimation`,
    doc.li`Fitted values`,
    doc.li`Regression risduals`,
    doc.li`Understand why estimated parameteres are (ex-ante) random variables`,
  ),
));
