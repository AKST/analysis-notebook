...
export const functionalForms = {
  log: text.figure(
    math(
      rows(
        eq(
          call.attr({ fn: mtext`log` }).of(mi`y`),
          add(
            msub(mi`β`, mn(0)),
            mul(msub(mi`β`, mn(1)), call.attr({ fn: mtext`log` }).of(msub(mi`x`, mn(1)))),
            add(
        ...
        mrow(
          annotationOver.attr({ label:  mtext`with x₁ fixed` }).of(
            eq(
              mi`%Δŷ`,
              mul(
                mn(100),
                minus.paren(
                  call.attr({ fn: mtext`exp` }).of(mul(msub(mi`β`, mn(2)), msub(mi`Δx`, mn(2)))),
                  mn(1)
                )
              )
            ),
  ...
  quadratic: text.figure(
    math(
      table.attr({ columnalign: 'right centre left' }).of(
        [
          mi`y`,
          mo`=`,
          add(
      ...
      table.attr({ columnalign: 'right center left' }).of(
        ...
        [
          ...
          add(
            ...
            mul(msub(SPECIAL.greek.beta, mn(1)), call.attr({ fn: mtext`log` }).of(msub(mi`x`, mn(1)))),
            mul(
              msub(SPECIAL.greek.beta, mn(2)),
              msup(parensA(call.attr({ fn: mtext`log` }).of(msub(mi`x`, mn(1)))), mn(2))
            )
          ),
        ],
            ...
            mrow(
              ...
              add(
                ...
                mul(
                  ...
                  mul(
                    msub(SPECIAL.greek.beta, mn(2)),
                    call.attr({ fn: mtext`log` }).of(msub(mi`x`, mn(1)))
                  ),
                )
              ),
              mo`]`,
  ...
  partialEffect: text.figure(
    mathml.math.attr({ className: 'big-math' }).of(
      mrow(
        eq(mi`y`, add(
          msub(mi`α`, mn(0)),
...
export const predictions = {
  stdErr: text.figure(
    math(
      rows.attr({ style: 'text-align: left' }).of(
        table.attr({ columnalign: 'right center left' }).of(
          [
            mi`y`,
            mo`=`,
            ...
            [
              ...
              add(
                annotationUnder.attr({
                  label: mtext`Std Err`,
                }).of(msub(SPECIAL.greek.theta, mn(0))),
                mul0(
                  msub(SPECIAL.greek.beta, mn(1)),
  ...
  varianceOfPredictionError: text.figure(
    math(
      mrow(
        eq(
          call.attr({ fn: mtext`Var` }).of(msup(mi`ê`, mn(0))),
          add(
            call.attr({ fn: mtext`Var` }).of(msup(mi`ŷ`, mn(0))),
            call.attr({ fn: mtext`Var` }).of(msup(mi`u`, mn(0)))
          ),
          add(
            call.attr({ fn: mtext`Var` }).of(msup(mi`ŷ`, mn(0))),
            msup(greek.sigma, mn(2))
  ...
  seOfPredictionError: text.figure(
    math(
      eq(
        call.attr({ fn: mtext`se` }).of(msup(mi`ê`, mn(0))),
        msup(
          parensC(
            add(
              msup(
                parensB(call.attr({ fn: mtext`se` }).of(msup(mi`ŷ`, mn(0)))),
                mn(2)
              ),
              msup(greek.sigma, mn(2))
            )
