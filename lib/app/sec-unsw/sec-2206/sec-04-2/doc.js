...
export const intro = createDoc(() => doc.header(container(
  twoThree(
    container(
      doc.h1.c('Multiple Regression Analysis: Further Issues', doc.br(),
...
export const dataScaling = createDoc(() => container(
  ...
  dashbox(
    doc.p`
      ...
      standard deviation (${text.b`z score`}). This is known as
      ${text.b`standardized coefficients`} or ${text.b`beta coefficients`}.
    `,
    doc.p`
    ...
    twoThree(
      container(
        ...
        doc.p`
          ...
          does mean ${text.b`making predictions is difficult`}.
        `,
      ),
      doc.dl(
        doc.dt`Nested Model`,
    ...
    container(
      ...
      text.ul(
        doc.li`
          ...
          note what R squared is actually estimating, ${text.b`it's estimating
        ...
        doc.li`
          ...
          ${text.b`corrected R-Square`}, however this is
...
export const predictionAndResidualAnalysis = createDoc(() => container(
  ...
  dashbox(
    twoThree(
      container(
        ...
        doc.p`
          ...
          also call ${text.b`an estimate of the expected value`}.
        `,
        doc.p`
          In order to get a 95 CI for a prediction we need a standard
          error for the esimated value ${text.i`(but not only this)`}.
          ...
          ${text.b`standard error of our estimated value`}.
        `,
      ),
      container(
        doc.h3`Definitions`,
    ...
    twoThree(
      container(
        ...
        text.ol.attr({ itemSpace: 'sparse' }).c(
          doc.li`
            ${text.b`Confidence interval for the mean response`}:
            ...
            for any explanatory variable. ${text.ul(
              ...
              doc.li`As values of ${text.b`c${doc.sub`j`}`} get further away from ${text.b`x̄${doc.sub`j`}`}, Var(ŷ) gets bigger.`,
              doc.li`You do this with hypothetical values you provide to the model`,
            )}
          `,
          doc.li`
            ${text.b`Prediction interval for an individual observation`}:
    ...
    doc.p`
      ...
      for the ${text.b`mean`} estimated indivisual. Where as a prediction
      ...
      determine y.
    `,
    doc.blockquote`${text.b`Resource`}:
      What is the difference between a ${LINKS.ciVsPi.t`
    ...
    text.ol({ itemSpace: 'sparse' }).c(
      doc.li`${text.b`Var(ŷ⁰)`}: Sampling error in ŷ⁰ (as a result of esimating βⱼ)${text.ul(
        doc.li`Var(ŷ⁰) is proportional to 1/n, as samples grow Var(ŷ⁰) will shrink.`,
      )}`,
      doc.li`${text.b`Var(u⁰) / σ²`}: Unobserved randomness, as the variance of the redisuals u⁰.`,
    ),
