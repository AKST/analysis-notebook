...
const Exp = bindOf(call({ fn: mtext`Exp` }));
  ...
  math(
    eq(
      ...
      mul0(
        ...
        annotationUnder.attr({ label: mrow()  }).of(
...
export const lpmAssumptionViolation = text.figure(
  math(
    eq(
      call.attr({ fn: mtext`Var` }).of(op.cond(mi`y`, mi`x`)),
      mul0(
        call.attr({ fn: mi`p` }).of(mi`x`),
        minus.square(
          mn(1),
          call.attr({ fn: mi`p` }).of(mi`x`)
        )
...
export const dummySlopehypothesisTest = text.figure(
  math(
    rows(
      ...
      table.attr({ columnalign: 'left left' }).of(
        [
