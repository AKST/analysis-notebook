...
export const intro = createDoc(() => container(
  twoThree(
    container(
      doc.h1.c('Multiple Regression Analysis: OLS Asymptotics', doc.br(),
        doc.small({ style: 'color: #aaaaff' }).t`ECON2206, W4`
      ),
    ),
    infobox({ title: 'Resources' }).c(
      text.ul(
        doc.li`Chapter 5`,
        doc.li.c(LINKS.asymptoticTheory.t`Asymptotic Theory`),
        doc.li.c(LINKS.asymptoticTheory.t`Probability Limits`),
        doc.li.c(LINKS.samplingDistribution.t`Sampling Distribution`),
      )
    ),
  ),
));
...
export const introduction = createDoc(() => dashbox(
  ...
  doc.p`${text.b`Finite Sample Properties`} of OLS`,
  text.ul(
    doc.li`
      Unbiasness under the first 4 Gauss-Markvo Assumptions is a
      ${text.b`finite sample property`} because ${doc.u`it holds
      for any sample size ${text.b`n`} where ${text.b`n`} is
      greater than the number of variables (${text.b`k`})`}.
    `,
    doc.li`
  ...
  doc.p`
    ...
    statistics we should know about. There are also ${text.b`asympototic
    properties`} or ${text.b`large sample properties`} of estimators
...
export const consistency = createDoc(() => container(
  ...
  dashbox(
    doc.p`
      ...
      an estimator an ${text.b`β̂ⱼ`} narrows around a true value of
      ${text.b`βⱼ`} as the size of the sample to esimate ${text.b`βⱼ`}
      increases. As the sample size increasese the inifinity
      (or the total population) the estimate ${text.b`β̂ⱼ`}
      converges to the value ${text.b`βⱼ`}.
    `,
    mathml.consistency.proofOfUnbiasness,
    mathml.consistency.probabilityLimits,
    doc.p`
      Above demostrating this via ${text.b`probability limits`},
    `,
  ...
  dashbox(
    ...
    twoThree(
      doc.p`
        ...
        Put another way ${text.b`if the error is correlated with
        any of the independent variables, then OLS is biased and
        inconsistent`}. This can be written more formually as:
      `,
      infobox({ title: 'Asymptotic Bias' }).c(doc.p`This is another name for inconsistency in an estimator such as β̂ⱼ`),
    ),
    ...
    doc.p`
      ...
      of the population variance of ${text.b`x₁`} and the
      covariance between ${text.b`x₁`} and ${text.b`x₂`}.
    ...
    doc.p`
      ...
      sample. ${text.b`It actually gets harder as you increase
      the sample size`}.
    `,
  ),
));
...
export const asymptoticNormality = createDoc(() => container(
  ...
  dashbox(
    doc.p`
      ...
      for statistical inference, we need to know the ${text.b`sampling
      distribution`} of the OLS estimators. This is like the standard
      error but over the multiple samples. When ${text.b`u`}'s are random
    ...
    `,
    doc.p`
      Apparently there's a thing called ${text.b`asymptotic normality`}, which
    ...
    twoThree(
      doc.p`
        When the sample size is small, the ${text.b`t distribution`} can be a poor
        approximation to the distribution of the ${text.b`t statitic`} when
        ${text.b`u`} is ${doc.u`${text.i`not`} normally distributed`}.
    ...
    twoThree(
      container(
        doc.p`
          ...
          sum of squares of ${text.b`x${doc.sub`j`}`} and
        ...
        doc.p`
          ...
          better. When ${text.b`u`} is not normally distributed
          sometimes the square root of the the Estimated Variance
          of β̂ⱼ is called the ${text.b`asymptotic standard error`}.
        `,
      ),
      infobox({ title: 'asymptotic standard error' }).c(
        doc.p`
          Here are some names for test statitics created
          with the ${text.b`asymptotic standard error`}.
        `,
        text.ul(
          doc.li`t statistics = ${text.b`asymptotic t statistics`}`,
          doc.li`ci = ${text.b`asymptotic confidence intervals`}`,
        ),
      ),
    ),
  ...
  twoColumns(
    doc.details.c(
      doc.summary.c(doc.h4`CTL and asymptotic normality`),
      doc.br(),
      dashbox(
    ...
    doc.details.c(
      doc.summary.c(doc.h4`Stocktake of when the CTL holds and doesnt`),
      doc.br(),
      dashbox(
        todo({}, 'TODO'),
      ),
  ...
  dashbox(
    doc.blockquote`This will not be tested`,
    doc.p`
      ...
      the ${text.b`Lagrange multiplier (LM) statistic`}. You can read more
      about this in ${LINKS.lagrange.t`Davidson and MacKinnon (1993)`}. Some
      times the name ${text.b`score statistic`} is used here. Another fun
      ...
      We also ${text.b`don't have to assume normality`}.
    `,
  ),
));
