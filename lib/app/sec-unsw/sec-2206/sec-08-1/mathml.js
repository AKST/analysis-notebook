...
const subscript = {
   ...
   * @param {'t' |  E.Item} t
   */
  tInner: t => typeof t === 'string' ? mi.c(t) : t,
  /**
   * @param {number | string} index
   */
  idx: index => typeof index === 'number' ? mn(index) : mi.c(index),
...
const $ = {
  ...
  var: (label, index) => (
    index != null
      ? msub(mi.c(label), subscript.idx(index))
      : mi.c(label)
  ),
  /**
   * @param {string} label
  ...
  varT: (label, index = undefined, t = 't') => (
     msub(mi.c(label), subscript.t(index, t))
  ),
  /**
   * @param {string} label
   * @param {number | string | undefined} index
...
export const assumption = {
  ...
  ts5: {
    def: text.figure(
      math(
        table({ columnalign: 'right right left' }).c(
          [
            mtext`Let`,
