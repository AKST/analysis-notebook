  ...
  twoThree(
    container(
      doc.h1.of(
        ...
        doc.small({ style: 'color: #aaaaff' }).t`ECON2206, W8, Lecture 1`,
      ),
    ),
    infobox({ title: 'Resources' }).of(
      text.ul(
        doc.li`Chapter 10 of the textbook.`,
        doc.li.of(LINKS.spuriousRegressions.t`Spurious regressions`),
      ),
    ),
  ),
));
    ...
    twoThree(
      ...
      container(
        ...
        doc.dl(
          ...
          doc.dd`
            ${doc.b`δⱼ`} is a function of ${doc.b`j`} which
  ...
  dashbox(
    ...
    twoThree(
      container(
        ...
        container(
          ...
          doc.quote`Require our explanator variables to be ${doc.b`strictly exogenous`}.`,
          ...
          `,
          doc.p`
            ${doc.b`The reason for this difference`} is the fact is
            ...
            ${doc.b`Hence we need strict exogenity`}. When we only do it at
            the period level we call this ${doc.b`Contemporaneously Exogenous`}.
          `,
          ...
          infobox({ title: 'u correlated with xⱼ' }).of(
            doc.p`In practise:`,
            text.ul(
              doc.li`Present ${doc.b`uₜ`} will not be correlated with past ${doc.b`xⱼₜ`}`,
              doc.li`Present ${doc.b`uₜ`} will almost correlate with future ${doc.b`xⱼₜ`}`,
            ),
          ),
          doc.quote`
            ${doc.b`TS.3`} is fairly unrealistic in practise, but we need to
      ...
      container(
        ...
        infobox({ title: 'Assumptions for unbiasness' }).of(
          text.ol(
            doc.li`Linear in Parameters`,
            doc.li`No Perfect Collinearity`,
            doc.li`Zero Conditional Mean`,
        ...
        doc.dl(
          ...
          doc.dd`
            This is also called ${doc.b`Auto-Correlation`}, it is
            when our errors are correlated across time.
          `,
        ),
        infobox({ title: "Theorum: unbiasness of OLS" }).of(
          doc.p`
            Under TS.1-3, the OLS estimators are unbiased
            conditional on ${doc.b`X`} and therefore
            unconditionally as well as when:${doc.br()}
            ${doc.br()}
            ${doc.b`E(β̂ⱼ) = βⱼ, j ∈ {0, 1, ..., k}`}
          `,
        ),
        infobox({ title: "Theorum: Unbiased Esimation of σ²" }).of(
          doc.p`
        ...
        infobox({ title: "Theorum: Guass-Markov Theorum" }).of(
          doc.p`
            Under TS.1-5, OLS estimators are the best linear
            unbiased estimators on ${doc.b`X`}.
          `,
        ),
        infobox({ title: "Normal Sampling Distribution" }).of(
          doc.p`
            ${doc.b`Theorum`}: Under TS.1-6, the CLM assumptions for time series, the OLS
    ...
    container(
      ...
      container(
        ...
        doc.quote`Variance of unobservables is constant over time`,
        twoColumns(
          container(
            doc.p`
        ...
        twoColumns(
          container(
            ...
            doc.p`
              ...
              ${doc.b`serial correlation`}, it means our errors
    ...
    container(
      ...
      twoThree(
        ...
        `,
        infobox({ title: 'TS.6 Normality' }).of(
          doc.p`
            The errors uₜ are independent of X and are independently and
            identically distributed as ${doc.b`Normal(0, σ²)`}.
          `,
        ),
      ),
    ),
      ...
      container(
        doc.dl(
          ...
          doc.dd`
            ...
            the ${doc.b`immediate propensity`} δ₀.
          `,
          doc.dt`long-run elasticity`,
          doc.dd`
            In a log log FDL model this is just
            the ${doc.b`long run propensity`}.
          `,
        ),
      ),
    ),
  ...
  dashbox(
    twoThree(
      ...
      doc.dl(
        ...
        doc.dd`
          Often annontated with ${doc.b`Umlaut`} like so ${doc.b`ÿ`}.
        `,
        doc.dt`Seasonality`,
