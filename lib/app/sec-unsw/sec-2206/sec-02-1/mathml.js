...
/** @param {...E.Item} body */
const E = (...body) => call.attr({ fn: 'E' }).c(frag(...body))
...
export const createFigure = (name, y, xs, options = {}) => text.figure(
  mathml.math(
    mrow(
      annotationUnder.attr({ label: mtext.c(y) }).c((
      ...
      ...xs.flatMap((x, index) => [
        ...
        annotationUnder.attr({ label: mtext.c(x) }).c(msub(mi`x`, mn(index + 1))),
        mspace(4), mo`+`, mspace(4)
      ]),
      mi`u`,
    )
        ...
        add(
          ...
          add(
            mul(
              ...
              }).c(parensA(minus(msub(mi`x`, mi`2i`), msub(mi`x̂`, mi`2i`)))),
            ),
