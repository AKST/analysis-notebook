 ...
 */
const INFOBOX = {
  rSquaredJ: infobox({ title: 'R²j' }).c(
    doc.p`R²j is the R² of the following regression`,
    doc.blockquote`Xj = ⍺₀ + ⍺₁X₁ + ... + αₖXₖ + V`,
    doc.p`
...
export const intro = createDoc(() => container(
  twoThree(
    container(
      doc.h1.c('Multi Linear Regression', doc.br(),
        doc.small.css({ color: '#aaaaff' }).t`ECON2206, W2-W3, Lecture 1`
      ),
    ),
    infobox({ title: 'Resources' }).c(
      text.ul(
        doc.li`Chapter 3 of the textbook`,
        doc.li.c(LINKS.gaussMarkovTheorem.t`Gauss Marko Theorem`),
        doc.li.c(LINKS.frischWaughLovell.t`Frisch–Waugh–Lovell theorem`),
      ),
    ),
  ),
));
...
const vars = {
  ...
  RJ: text.b`R${doc.sub`j`}${doc.sup`2`}`,
  R2: text.b`R${doc.sup`2`}`,
  est: {
    y: text.b`ŷ`,
...
export const motivationForMultiRegression = createDoc(() => dashbox(
  ...
  twoThree(
    container(
      doc.p`
        ...
        regression, is it is difficult to draw ${text.b`ceterix paribus`}
        conclusions about how ${text.b`x`} affects ${text.b`y`}.
    ...
    infobox({ title: 'SLR.4' }).c(
      doc.p`
        SLR.4 is the zero conditional mean assumption,
        which is roughly ${text.b`E(u|x) = 0`}.
      `,
    ),
  ),
  doc.p`
    ${doc.u`Given all Gauss Markov assumptions hold`}, as we add more
    variables to a model we can explain more of the variation of ${text.b`y`}.
  ...
  container(
    ...
    doc.p`
      ...
      guessed from the above examples. ${text.b`One neat thing`}
    ...
    twoThree(
      container(
        ...
        doc.p`
          ...
          This is fine as it is ${text.b`linear in parameters`}.
          However ${text.b`one drawback`} of this is we no longer
          have a meaningful interpretation of ${vars.b1} in isolation.
        `,
      ),
      infobox({ title: 'Key Assumption' }).c(
  ...
  container(
    twoThree(
      ...
      infobox({ title: 'K convention' }).c(doc.p`k often denotes the number independent vars.`),
    ),
    doc.p`
      ...
      ${text.b`OLS slopes`} (or OLS Slope estimates).
    `,
    twoThree(
      doc.p`
        ...
        of the conditional mean assumption for ${text.b`k`}
    ...
    doc.p`
      ...
      should be ${text.b`0`}. Which implies the error term is not
  ...
  container(
    twoThree(
      doc.p`
        ...
        will apply to a model with any number (${text.b`k`})
        of variables.
      `,
    ...
    doc.p`
      ...
      that holds the ${text.b`k`} regressors for the
    ...
    doc.p`
      ...
      ${text.b`First Order Conditions`}. This can be
  ...
  container(
    ...
    text.ul(
      doc.li`Say you make a model ${text.ul(
        doc.li.c(text.b`Y = θ0 + θ1*X1 + θ2*X2 + θ3*X3 + V`),
      )}`,
      doc.li`Say the True model is ${text.ul(
        doc.li.c(text.b`Y = B0 + B1*X1 + B2*X2 + U`),
      )}`,
...
export const expectedOLSEstimatorsOmittedVariableBias = createDoc(() => dashbox(
  ...
  twoThree(
    container(
      ...
      `,
      doc.p`
        The earlier section on ${text.b`Comparing SLR & MLR regressors
        Estimates`} explain some concepts used in this section (such as
        β̃₁ or δ̃₁).
      `,
    ),
    infobox({ title: 'Nomenclature' }).c(
      ...
      doc.p`
        ...
        ${text.b`misspecification analysis`}.
      `,
    ),
  ),
  container(
    ...
    container(
      ...
      twoColumns(
        container(
          ...
          doc.p`
            From the above equation ${text.b`a large σ²`} means ${text.b`larger sampling variances`}
            for the OLS Estimators. Which ${text.b`put another way means more noise`}.
          `,
          doc.p`
            ${text.b`Note`}, because σ² is a population feature it has nothing to do with
            the sample size. ${text.b`It is an unknown component`}.
          `,
        ),
        container(
          ...
          doc.p`
            ...
            that the larger the ${text.b`SST${text.b`j`}`} the smaller
            the variance. I mean take any fraction leave the
            nominator as is and increase the denominator and watch
            the resulting value decrease. However ${text.b`we want to
      ...
      container(
        ...
        twoThree(
          container(
            ...
            doc.p`
              Note a case where R₂² gets close to 1, ${text.b`is not a violation of MLR 1`}
            `,
          ),
          INFOBOX.rSquaredJ,
        ),
        doc.p`
          ...
          problem but it doesn't violate ${text.b`NONE`} of our assumptions.
...
export const varianceInMisspecifedModels = createDoc(() => dashbox(
  container(
    ...
    doc.p`
      ${text.b`How do you decide between omitting or including a variable?`}
