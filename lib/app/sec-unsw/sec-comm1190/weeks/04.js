...
export function render() {
  return doc.details({ name: 'week', className: 'dashbox' }).c(
    doc.summary({ className: 'h2' }).c(doc.h2`Week 4 — Predictive Analytics, Part 1`),
    container(
      ...
      doc.p`As a function`,
      ul(
        doc.p`${text.b`Input`}: Independent Variable`,
        doc.p`${text.b`Output`}: Dependant Variable`,
      ),
      twoColumns(
        container(
      ...
      ul(
        doc.p`${text.b`Quality Control`} - fraud detection of junk email`,
        doc.p`${text.b`Inventoary management`} - sales forecasting`,
        doc.p`${text.b`Risk analysis`} - churning/staff turnover/readmission`,
        doc.p`${text.b`Market segmentation`} - who are my least/most satisfied customers`,
      ),
