...
export function render() {
  return doc.details({ name: 'week', className: 'dashbox' }).c(
    doc.summary({ className: 'h2' }).c(doc.h2`Week 5 — Predictive Analytics, Part 2`),
    container(
      container(
        doc.h3`Prediction Certainty`,
        ...
        twoColumns(
          container(
            ...
            doc.p`Where:`,
            text.ul(
              doc.li`SE^2 is the variance of the resideuals from the regression model.`,
              doc.li`SE_mean is the standard error of the mean prediction.`,
            ),
          ),
          container(
            doc.h4`Prediction Intervals`,
            math.predictionInterval,
            doc.p`Where:`,
            text.ul(
              doc.li`y_new is the predicted value for the new observation.`,
              doc.li`${text.b`t_alpha/2`} is the critical value from the t
              ...
              `,
              doc.li`${text.b`SE_pred`} is the standard error of the prediction`,
            ),
          ),
        ),
        doc.h4`Prediction Accuracy`,
        ...
        `,
        text.ul(
          doc.li`${text.b`Bias in model selection`}:
          ...
          `,
          doc.li`${text.b`NOt reflective of real world perfmance or lack of generalizablity`}:
      ...
      doc.hr(),
      container(
        doc.h3`Regression trees`,
        doc.p`
          Tree-based methods involve segmenting inputs
          into mutually exlusive & exhaustive regions.
          ${doc.br()}${doc.br()}${text.ul(
            doc.li`Branches connect internal nodes to a terminal node (leaf)`,
            doc.li`Branches formed by binary splits at each node.`,
            doc.li`Each leaf assigned a prediction (e.g. mean outcome within branch)`,
          )}
        `,
      ),
      doc.hr(),
      container(
        doc.h3`Regression vs Classification`,
        twoColumns(
          container(
            doc.h4`Regression`,
            text.ul(
              doc.li`${text.b`Y`} is quatitative, continious`,
              doc.li`
          ...
          container(
            doc.h4`Classification`,
            text.ul(
              doc.li`${text.b`Y`} is qualitative, nominal`,
              doc.li`
