...
export const marginalProduct = {
  ...
  ofLabour: mathml([
    rows([
      eq(i('Y'), mul3(vars.techFixed, sup(vars.capital, powK), sup(vars.labour, powL))),
...
export const hypotheticalModel = {
  output: eq(i('Y'), over(text('H is human capital'), mul2(
    sup(i('K'), frac([n(1)], [n(3)])),
    paren([add(
      mul2(theta, sup(vars.labour, frac([n(1)], [n(2)]))),
      mul2(
        minusP(n(1), theta),
        sup(i('H'), frac([n(1)], [n(2)])),
      ),
    )]),
  ))),
  mpk: eq(i('MPK'), mul2(
