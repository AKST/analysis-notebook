 ...
 * @import { TrajectoryMap, Config, State, Event } from './type.ts';
 */
    ...
    solowSwan: {
      ...
      chart: {
        ...
        prerenderedTrajectory: {
          current: projectMap(trajectory, translate, bounds),
          initial: trajectoryInit && (
            projectMap(trajectoryInit, translate, bounds)
          ),
        },
      },
    },
 ...
 */
function projectMap(map, translate, bounds) {
  /** @param {Vec2} p */
  function projectPoint(p) {
    return math.el.div(math.el.sub(p, translate), bounds)
  }

  return {
    capital: map.capital.map(projectPoint),
    output: map.output.map(projectPoint),
  };
}
...
export function getCamera(map, mmap) {
  const vectors = Array.from(trajectoryMapToVectors(map, mmap));
