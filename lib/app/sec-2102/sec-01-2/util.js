...
function calibrateDynamics(variant, config) {
  ...
  switch (variant.kind) {
    ...
    case 'update': {
      ...
      trajectoryInit = capitalDynamics.trajectory;
      startTime = capitalDynamics.start.time;
      currentT = (now - startTime) / capitalDynamics.chart.period;
      k0 = solveAtT(currentT, fauxState, config);
      break;
    }
  ...
  const trajectory = Array.from({ length: Math.ceil(periods) }, (_, t) => (
    v2(currentT + t, solveAtT(currentT + t, fauxState, config))
  ));
  const { bounds, translate } = getCamera(
    trajectoryInit != null
