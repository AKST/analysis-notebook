...
export const modelSetup = createDoc(() => dashbox(
  ...
  note(
    ...
    'Technology is fixed just as in production model',
  ),
  ['h4', 'Resource Constraint'],
  note(
    'Output can be used for two thigns, consumption or investment',
    doc.p('economy is closed Yt = Ct + It', ulLight([
      'no imports',
      'no exports',
  ...
  ['hr'],
  ['h4', 'Capital Accumulation'],
  noteOn('How captial accumulates over time')(
    'K(t+1) = Kt + It - d̄Kt',
  ),
  note(
    doc.p('cpatial in the next period is compromised of', ulLight([
      'the stock of capital at the beginning of the current period',
      'the amoung of investment undertaken from the current years production',
      'physical depreciation on the existing capital stock',
      'd̄ is the rate of the depreciation',
    ])),
    doc.p('rewrite as', ulLight([
      'K(t+1) - Kt = ΔK(t+1) = It - d̄Kt',
    ])),
    doc.p('We must assume some inital value for the captial stock', ulLight([
      'ΔK1 = I0 - d̄K0',
    ])),
  ),
  ...
  ['h4', 'Labour Supply'],
  note(
    'Labour supply is assumed to be constant and exogenously determined, Lt = L̄',
    'One standard alternative is to allow for constant growth in labour supply (equivently population) at an exogenous growth rate (like n)',
  ),
  ['hr'],
  ['h4', 'Investment'],
  note(
    'We assume that a constant share of outptu is used for investment',
    'Such as It = s̄ * Yt',
    'Can be re-written as assumption about consumption using resource constaint',
