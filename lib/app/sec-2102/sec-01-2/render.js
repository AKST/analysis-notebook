...
export const dynamicsOfCapital = makeWidget(false, 'Captial Dynamics', projAbs, {
  height: 400,
...
export const dynamicsOfOutput = makeWidget(false, 'Captial Output', projAbs, {
  height: 400,
  gridColumn: { default: 1, s: 2 },
},
  (ctx, state) => {
    ...
    const tA = (performance.now() - time.start) / time.period;
    const tB = (performance.now() - time.branch) / time.period;
    const k = util.solveOutputAtT(model, tB);
    drawDyanmics(ctx, camera.output, v2(tA, k), points.initial?.output, points.current.output);
  },
);
