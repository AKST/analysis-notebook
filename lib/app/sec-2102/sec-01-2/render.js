...
export const dynamics = makeWidget(
  ...
  (ctx, state, config) => {
    ...
    const k = util.solveAtT(model, (performance.now() - time.branch) / periodMs);
    ...
    line.drawVector(points.initial ?? [], { width: 4, stroke: 0x0000ff });
    line.drawVector(points.current, { width: 4, stroke: 0xffffff });
