...
export const dynamics = makeWidget(
  ...
  (ctx, state, config) => {
    const { text, line, grid } = ctx;
    const { bounds, translate } = state.chart;
    const { dynamics: { periodMs } } = config;
    const t = (performance.now() - state.capitalDynamics.start) / periodMs;
    ...
    line.drawVector(state.chart.trajectory, {
      width: 4,
      stroke: 0xffffff,
    });
    line.drawVector([
