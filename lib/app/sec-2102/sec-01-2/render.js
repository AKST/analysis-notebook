...
export const dynamics = makeWidget(
  ...
  (ctx, state, config) => {
    const { line, grid } = ctx;
    const { start, chart: { bounds, translate } } = state.capitalDynamics;
    const { chart: { trajectory, trajectoryInit } } = state.capitalDynamics;
    const { dynamics: { periodMs }, exogenous: { technology } } = config;
    const t = (performance.now() - start.time) / periodMs;
    const k = util.solveAtT(t, state, config);
    const xy = math.el.div(math.el.sub(v2(t, k), translate), bounds);
    ...
    if (trajectoryInit != null) {
      line.drawVector(trajectoryInit, {
        width: 4,
        stroke: 0x0000ff,
      });
    }

    line.drawVector(trajectory, {
      width: 4,
      stroke: 0xffffff,
    });
    ...
    drawAxisTick(ctx, v2(t, k), translate, bounds);
    drawAxisTick(ctx, translate, translate, bounds);
    drawAxisTick(ctx, math.el.mul(bounds, 10/11), translate, bounds);
  },
);
...
function drawAxisTick({ text }, p, translate, bounds) {
  ...
  text.draw(v2(-0.01, yb), y.toFixed(2), {
    fontSize: 12,
    textBaseline: 'middle',
    textAlign: 'right',
  });
