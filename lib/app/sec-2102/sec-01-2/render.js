...
export const dynamics = makeWidget(
  ...
  (ctx, state, config) => {
    const { line, grid } = ctx;
    const { start, chart: { dst, bounds, translate } } = state.capitalDynamics;
    const { chart: { trajectory, trajectoryInit } } = state.capitalDynamics;
    const { dynamics: { periodMs }, exogenous: { technology } } = config;
    const tA = (performance.now() - start.timeAnimation) / periodMs;
    const tT = (performance.now() - start.timeTrajectory) / periodMs;
    const k = util.solveAtT(tT, start.capital, state, state.capitalDynamics.exogenous);
