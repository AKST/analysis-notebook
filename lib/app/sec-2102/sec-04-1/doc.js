...
const { noteOn, ulLightSm, clsRef, infobox, ulLight, dashbox, note, todo } = prelude.components;
  ...
  twoThree(
    ...
    infobox('Resources', [
      ulLight([
        ...
        LINKS.ytMonsterOfInflation`The monster of Inflation`,
        LINKS.ytMoneyInTheEconomy`Money in the Economy`,
      ]),
    ]),
  ),
));
  ...
  dashbox(
    ...
    twoColumns(
      ...
      container(
        ...
        doc.quote.of(doc.small.of('Read ', (
...
export const tutorialNotes = createDoc(() => dashbox(
  ...
  noteOn('inflation')(
    doc.span`to the central bank expected inflation is more important than actual inflation`,
    doc.span`who is Paul Volker, ${ulLight([
      doc.span`Read the wikipedia on him`,
      doc.span`He made everyone unemployed`,
      doc.span`Why did he do this? To eliminate inflation`,
    ])}`,
    doc.span`Something about stagflation${ulLight([
      doc.span`Politically hard to fix`,
      doc.span`Need to put more people out work`,
    ])}`,
    doc.span`What is worse, deflation or inflation${ulLight([
      doc.span`Is one easier to manage by a central bank?`,
    ])}`,
    doc.span`yield curve${ulLight([
      doc.span`Increase cash rate, and yield curve will go up`,
    ])}`,
    doc.span`risk free rate, government bond rate`,
  ),
));
