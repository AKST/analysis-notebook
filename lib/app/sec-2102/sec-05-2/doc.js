...
const { clsRef, infobox_2, text, dashbox, todo } = prelude.components;
...
export const derivingTheIsCurve = createDoc(() => container(
  ...
  dashbox(
    ...
    text.ul(
      doc.li.of('if ', doc.b`MPK = r̄ < Rₜ`, text.ul(
        doc.li`Firms should ${doc.b`save`}`,
        doc.li`Firms should ${doc.b`not invest`} in capital`,
        doc.li`Investment will decline`,
      )),
      doc.li.of('if ', doc.b`MPK = r̄ > Rₜ`, text.ul(
        doc.li`Firms should ${doc.b`borrow`}`,
        doc.li`Firms should ${doc.b`invest`} in capital`,
