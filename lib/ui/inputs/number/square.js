...
export class NumberSquareInput extends HTMLElement {
  ...
  render() {
    updateOn(this.#root, [
      doc.div({ className: 'square-container' }).of(
        doc.div({ className: 'square-grid' }).of(
          ...fromIndex(9, i => {
            const d = `${(1+i)*10}%`;
            const left = d, top = d;
            return [
              doc.div.css({ top }).attr({ className: 'square-grid-line horizontal' }).of(),
              doc.div.css({ left }).attr({ className: 'square-grid-line vertical' }).of(),
            ];
          }).flat(),
          doc.div.attr({ className: 'square-grid-line horizontal' }).of(),
          doc.div.attr({ className: 'square-grid-line vertical' }).of(),
        ),
        doc.div({ className: 'square-handle' }).of()
      ),
    ]);
