/**
 * @import { El } from '@base/dsl-dom/type.ts';
 */
...
export class NumberSquareInput extends HTMLElement {
  static sheet = new SharedStyleSheet(import.meta.resolve('./square.css'));
      ...
      doc.div({ className: 'square-container' }).c(
        doc.div({ className: 'square-grid' }).c(
          ...fromIndex(9, i => {
            ...
            const left = d, top = d;
            return [
              doc.div.css({ top }).void({ className: 'square-grid-line horizontal' }),
              doc.div.css({ left }).void({ className: 'square-grid-line vertical' }),
            ];
          }).flat(),
          doc.div.void({ className: 'square-grid-line horizontal' }),
          doc.div.void({ className: 'square-grid-line vertical' }),
        ),
        doc.div.void({ className: 'square-handle' })
      ),
    ]);
