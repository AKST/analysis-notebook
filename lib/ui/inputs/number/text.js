...
export class NumberTextInput extends HTMLElement {
  static sheet = new SharedStyleSheet(import.meta.resolve('./text.css'));
  ...
  connectedCallback() {
    NumberTextInput.sheet.install(this.#shadow);
   ...
   */
  #render() {
    const input = this.#input = render(doc.input({
      type: 'text',
      value: this.#text,
