...
import { generateCSS, renderOnto } from '../../../base/render_ui/index.js';
import { insertAtStart } from '../../../util/dom.js';
import { renderDocument as appRender } from '../../../base/render_app/index.js';
...
export function createMultiWidgetApplicationFactory() {
  return {
    ...
    create(module, {
      ...
      const containerId = `app-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
...
class MultiWidgetApplicationEngine {
  ...
  installWidget(childModule, index, viewportWidth) {
    ...
    if (widget.constructor.scalingStrategy === 'fixed' && !size?.height) {
    ...
    this.#widgets.set(id, {
      ...
      scalingStrategy: widget.constructor.scalingStrategy,
      size,
    });
