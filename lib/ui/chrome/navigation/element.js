/**
 * @import { AppExplorerMenu } from './app-explorer/element.js';
 * @import { TableOfContentsMenu } from './table-of-contents/element.js';
 * @import { TabLayout } from '@ui/layout/tabs/element.js';
 */
import { Toggleable } from '@ui/mixins/toggleable.js';
import { render, renderOnto, meta, insert } from '@base/dsl-dom/render.js';
import * as doc from '@base/dsl-dom/helper/html.js';
import * as Meta from '@base/dsl-dom/node-meta.js';
...
export class NavigationMenu extends Toggleable(HTMLElement) {
  ...
  render() {
    const nav = render(doc.nav({ className: 'navigation' }).css({ opacity: '0' }).c(
      insert(this.#tabLayout, Meta.data(meta(), 'active', 'apps'), [
        doc.slot({ name: 'tab', title: 'Apps' }).data({ tabId: 'apps' }).c(
          doc.div({ className: 'tab-container' }).c(this.#appExplorer),
        ),
        doc.slot({ name: 'tab', title: 'Table Of Contents' }).data({ tabId: 'toc' }).c(
          doc.div({ className: 'tab-container' }).c(this.#tocMenu),
        ),
      ]),
    ));
