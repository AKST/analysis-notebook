...
import { updateOn, render } from '@base/dsl_dom/render.js';
...
export class TableOfContentsMenu extends HTMLElement {
  ...
  #ul = render(doc.ul.css({ opacity: '0' }).of());
  ...
  render() {
    ...
    updateOn(this.#root, [
      ...
      doc.div({ className: 'layout' }).of(this.#ul),
    ]);
  }
  ...
  set items (items) {
    ...
    for (const item of items) {
      ...
      const listItem = doc.li({ className: 'list-item' }).css({
        ...
        doc.a({ href: `#${item.titleId}` }).of(item.title),
        doc.span({ className: 'dots' }).of(),
      );
