...
export class TabLayout extends HTMLElement {
  ...
  list = render(doc.div({ className: 'tablist', role: 'tablist' }).of());
  content = render(doc.div({ classList: ['content', 'ms-scroll'] }).of());
  ...
  render() {
    renderOnto(this.shadow, [
      doc.div({ className: 'layout' }).of(
        doc.nav({ className: 'ms-scroll' }).of(this.list),
        doc.hr(),
        this.content,
      ),
    ]);
  ...
  installTab(id, title, slot) {
    ...
    const content = render(doc.div({ id: contentId, style: 'display: none' }).of(slot));
