...
import { render, renderOnto } from '@base/dsl-dom/render.js';
import { SharedStyleSheet } from '@base/platform/styles/shared-style-sheet.js';

export class TabLayout extends HTMLElement {
  static sheetTabs = new SharedStyleSheet(import.meta.resolve('./style.css'));
  ...
  installTab(id, title, slot) {
    ...
    const content = render(doc.div({ id: contentId }).css({ display: 'none' }).c(slot));
