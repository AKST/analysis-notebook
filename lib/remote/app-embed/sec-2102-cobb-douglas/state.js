...
export class SceneState {
  ...
  updateRenderUniforms(device, { meshRender, text, gridRender }) {
    ...
    const max = cb.computeMaxOutput(mesh.x, mesh.z);
    const maxScale = 10 ** Math.ceil(Math.log10(max));
    const maxToScale = maxScale * (500/max);

    gridRender.update('world', world.scale(1, maxToScale, 1));
    gridRender.update('viewProj', viewProjection);
    gridRender.update('cameraAngle', camera.cameraAngle);
