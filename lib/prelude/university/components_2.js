...
const makeListTag = (tagName, cls) => makeHelper({
  of: ['html', tagName],
  create: (attrs, items) => {
    const { className: classIn, itemSpace = 'dense', blockMargin = 0 } = attrs.element;
    let className = joinClass(classIn, cls) ?? '';
    if (itemSpace === 'dense') className = joinClass(className, 'no-item-padding');

    const newAttrs = /** @type {E.Attrs<any>} */ (attrs)
      .elem('className', className)
      .style({ marginBlock: `${blockMargin * 8}px` });

    return /** @type {any} */ (node('html', tagName, newAttrs, items));
  },
});
...
const makeClsTagB = (tagName, cls) => makeTemplateHelper({
  of: ['html', tagName],
  create: (attr, items) => {
    const { className: classIn } = attr.element;
    const classOut = joinClass(classIn, cls);
    const attrsOut = /** @type {any} */ (attr).elem('className', classOut);
    return /** @type {any} */ (node('html', tagName, attrsOut, items));
  },
});
...
export const text = {
  a: makeTemplateHelper({
    of: ['html', 'a'],
    create: (m, ls) => node('html', 'a', m.elem('target', 'blank'), ls),
  }),
  b: doc.strong,
  figure: makeHelper({
    of: ['html', 'figure'],
    create (m, items) {
      const { className: classIn } = m.element;
      const cls = 'pre--horizontal-shadow';
      return node('html', 'figure', m.elem('className', joinClass(classIn, cls)), items);
    },
  }),
  i: doc.em,
  p: {
...
export const infobox = makeHelper({
  ...
  /**
   * @param {E.Attrs<{ title: string } & DefaultHelperAttrs<'html', 'aside'>>} m
   */
  create: (m, body) => {
    const { className: interClassName, title, ...attrs } = m.element;
    const className = joinClass(interClassName, 'infobox');
    return doc.aside.attr({ ...attrs, className: joinClass(className, 'infobox') }).of(
