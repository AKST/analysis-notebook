...
export const annotationUnder = makeHelper({
   ...
   */
  create(meta, body) {
    const { label = 'unset', labelSize, marginTop } = meta.element;
    let style = '';
    if (labelSize != null) style += `; font-size: ${labelSize}px`;
    if (marginTop != null) style += `; margin-top: ${marginTop}px`;
    return munder(meta).of(mrow(...body), munder(mo`⏟`, mrow.attr({ style }).of(label)))
  },
});
...
export const annotationOver = makeHelper({
   ...
   */
  create(meta, body) {
    const { label = 'unset', labelSize } = meta.element;
    const overAttr = labelSize == null ? ({}) : ({ style: `font-size: ${labelSize}px` });
    return mover(meta).of(mrow(...body), mover(mo`⏞`, mrow.attr(overAttr).of(label)))
  },
});
...
export const multiscripts = makeHelper({
  ...
  create(meta, body) {
    ...
    return mathml.mmultiscripts(meta).of(
      base,
      br ?? mrow(),
      tr ?? mrow(),
      mathml.mprescripts(),
...
const makeOperator = (o, space = 4) => {
  const helper = makeHelper({
     ...
     */
    create(meta, items) {
      const { paren, subsup = false } = meta.element;
      let wrapper, sep;
  ...
  const operatorHelper = Object.assign(helper, {
    ...
    mtr: makeHelper({
      of: ['mathml', 'mtr'],
      create: (meta, items) => {
        const { pos: ca = [] } = meta.element;
        return mtr(...seperateItems(items.map((it, i) => mtd({ columnalign: ca[i] ?? 'center' }).of(it)), [mtd(o)]));
      },
    }),
