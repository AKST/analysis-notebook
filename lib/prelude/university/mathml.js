 ...
 * @import { TemplateHelper, Helper, DefaultHelperAttrs } from '@base/dsl-dom/helper-type.ts';
 */

import { makeHelper } from '@base/dsl-dom/helper.js';
...
export const annotationUnder = makeHelper({
  ...
  create(meta, body) {
    ...
    return munder(newMeta).c(mrow(...body), munder(mo`⏟`, mrow({ style }).c(label)))
  },
});
...
export const annotationOver = makeHelper({
  ...
  create(meta, body) {
    ...
    const newMeta = Meta.build(meta).setAttr(attr).meta;
    return mover(newMeta).c(mrow(...body), mover(mo`⏞`, mrow(overAttr).c(label)))
  },
});
  ...
  create(meta, conds) {
    return mrow(meta).c(
      ...
      mtable(...conds.map(({ cond, then }) =>
        mtr(
          mtd({ columnalign: 'left' }).c(then),
          mtd({ columnalign: 'left' }).c(cond == null ? mrow() : mtext`if`),
          mtd({ columnalign: 'left' }).c(cond == null ? mtext`otherwise` : cond),
        ),
      )),
    );
  }
