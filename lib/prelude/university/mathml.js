...
export const parensD = makeHelper((attrs, body) => mrow(attrs).of(mo`⟨`, ...body, mo`⟩`));
...
export const set = makeHelper((attrs, body) => parensC(attrs).of(
...
export const annotationUnder = makeHelper(({
  ...
  return munder(attr).of(mrow(...body), munder(mo`⏟`, mrow.attr({ style }).of(label)))
}, /** @type {{
  label: E.Item,
  labelSize?: number,
  marginTop?: number,
...
export const annotationOver = makeHelper(({ label, labelSize, ...attr }, body) => {
  ...
  return mover(attr).of(mrow(...body), mover(mo`⏞`, mrow.attr(overAttr).of(label)))
}, /** @type {{
  label: E.Item,
  labelSize?: number
}} */ ({ label: mrow() }));
...
export const rows = makeHelper((attr, body) => (
  table(attr).of(...body.map(row => [mtd(row)]))
));
 ...
 */
export const index = makeHelper((attr, [body, index]) => mrow(attr).of(
  mrow(body),
  mrow(mo`[`, index, mo`]`),
));
...
export const inv = makeHelper((attr, items) => {
  ...
  return msup.attr(attr).of(item, mrow(mo`-`, mn(1)));
});
...
export const neg = makeHelper((attr, items) => {
  if (items.length === 0) throw new Error();
  const item = items.length > 1 ? parensA(...items) : items[0];
  return mrow.attr(attr).of(mo`-`, item);
});
...
const makeOperator = (o, space = 4) => {
  ...
  return Object.assign(helper, {
    sup: makeHelper((attr, items) => {
      return helper.attr({ ...attr, subsup: true }).of(...items);
    }),
    sub: makeHelper((attr, items) => {
      return helper.attr({ ...attr, subsup: true }).of(...items);
    }),
    paren: makeHelper((attr, items) => {
      return helper.attr({ ...attr, paren: '()' }).of(...items);
    }),
