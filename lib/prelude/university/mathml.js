...
export const parensA = makeHelper({
  of: ['mathml', 'mrow'],
  create: (meta, body) => mrow(meta).of(mo`(`, ...body, mo`)`),
});
...
export const set = makeHelper({
  of: ['mathml', 'mrow'],
  create(meta, body) {
    return parensC(meta).of(...seperateItems(body, [mo`,`, mspace(4)]));
  },
});
...
export const annotationUnder = makeHelper({
  of: ['mathml', 'munder'],
  /**
   * @param {E.Attrs<DefaultHelperAttrs<'mathml', 'munder'> & {
   *   label: E.Item,
   *   labelSize?: number,
   *   marginTop?: number,
   * }>} meta
   */
  create(meta, body) {
    const { label, labelSize, marginTop, ...attr } = meta.element;
    let style = '';
    ...
    return munder(newMeta).of(mrow(...body), munder(mo`⏟`, mrow.attr({ style }).of(label)))
  },
});
...
export const annotationOver = makeHelper({
  of: ['mathml', 'mover'],
  /**
   * @param {E.Attrs<DefaultHelperAttrs<'mathml', 'mover'> & {
   *   label: E.Item,
   *   labelSize?: number,
   *   marginTop?: number,
   * }>} meta
   */
  create(meta, body) {
    const { label, labelSize, ...attr } = meta.element;
    const overAttr = labelSize == null ? ({}) : ({ style: `font-size: ${labelSize}px` });
    const newMeta = Meta.build(meta).setAttr(attr).meta;
    return mover(newMeta).of(mrow(...body), mover(mo`⏞`, mrow.attr(overAttr).of(label)))
  },
});
...
export const table = makeHelper({
   ...
   */
  create(meta, rows) {
    const { columnalign = '' } = meta.element;
    return mtable(meta).of(...rows.map(row => {
      const split = columnalign != null
        ? columnalign.split(' ')
        : Array(row.length).fill(undefined);

      const cell = row.map((col, idx) => mtd({ columnalign: split[idx] }).of(col));
...
export const rows = makeHelper({
  of: ['mathml', 'mtable'],
  create(meta, body) {
    return table(meta).of(...body.map(row => [mtd(row)]));
  },
});
...
export const abs = makeHelper({
  of: ['mathml', 'mrow'],
  create: (meta, body) => mrow(meta).of(mo`|`, mrow(...body), mo`|`)
});
...
export const index = makeHelper({
  of: ['mathml', 'mrow'],
  create: (meta, [body, index]) => mrow(meta).of(mrow(body), mrow(mo`[`, index, mo`]`))
});
...
export const sum = makeHelper({
   ...
   * }} SumAttrs
   *
   * @param {E.Attrs<DefaultHelperAttrs<'mathml', 'mrow'> & SumAttrs>} meta
   */
  create(meta, items) {
    const { inc, max, ...attr } = meta.element;
    const newMeta = Meta.build(meta).setAttr(attr).meta;
    return mrow(newMeta).of(
  ...
  create(meta, conds) {
    return mrow(meta).of(
      ...
      mtable(...conds.map(({ cond, then }) =>
        mtr(
          mtd.attr({ columnalign: 'left' }).of(then),
          mtd.attr({ columnalign: 'left' }).of(cond == null ? mrow() : mtext`if`),
          mtd.attr({ columnalign: 'left' }).of(cond == null ? mtext`otherwise` : cond),
        ),
      )),
    );
  }
...
export const iv = makeHelper({
   ...
   */
  create: (meta, items) => (
    mi(meta).of(`${items[0]} ⃗`.replace(/\s+/g, ''))
  ),
});
...
export const call = makeHelper({
   ...
   *
   * @param {E.Attrs<CallAttrs & DefaultHelperAttrs<'mathml', 'mrow'>>} meta
   */
  create(meta, items) {
    const { paren = '()', fn = mrow(), space = true, ...attr } = meta.element;
    const [po, pc] = paren === '()' ? [mo`(`, mo`)`] : [mo`[`, mo`]`];
    const inner = seperateItems(items, [mo`,`, mspace(4)]);
    const leadingSpace = space ? [mspace(2)] : [];
    const newMeta = Meta.build(meta).setAttr(attr).meta;
    return mrow(newMeta).of(fn, ...leadingSpace, mrow(po, ...inner, pc));
  },
});
...
export const inv = makeHelper({
  ...
  create(meta, items) {
    ...
    return msup(meta).of(item, mrow(mo`-`, mn(1)));
  },
});
...
export const multiscripts = makeHelper({
   ...
   * }} MultiscriptsAttrs
   *
   * @param {E.Attrs<MultiscriptsAttrs & DefaultHelperAttrs<'mathml', 'mrow'>>} meta
   */
  create(meta, body) {
    const {
    ...
    return mathml.mmultiscripts(newMeta).of(
      base,
      br ?? mrow(),
      tr ?? mrow(),
      mathml.mprescripts(),
...
const makeOperator = (o, space = 4) => {
  const helper = makeHelper({
    of: ['mathml', 'mrow'],
    /**
     * @param {E.Attrs<OperatorAttr & DefaultHelperAttrs<'mathml', 'mrow'>>} meta
     */
    create(meta, items) {
      const { paren, subsup = false, ...attrs } = meta.element;
  ...
  const operatorHelper = Object.assign(helper, {
    sup: makeHelper({
      of: ['mathml', 'mrow'],
      create: (meta, items) => helper(meta).attr({ subsup: true }).of(...items),
    }),
    sub: makeHelper({
      of: ['mathml', 'mrow'],
      create: (meta, items) => helper(meta).attr({ subsup: true }).of(...items),
    ...
    paren: makeHelper({
      of: ['mathml', 'mrow'],
      create: (meta, items) => helper(meta).attr({ paren: '()' }).of(...items),
    }),
    brace: makeHelper({
      of: ['mathml', 'mrow'],
      create: (meta, items) => helper(meta).attr({ paren: '{}' }).of(...items),
    ...
    mtr: makeHelper({
      of: ['mathml', 'mtr'],
      create: (meta, items) => {
        const { pos: ca = [], ...attrs } = meta.element;
        const newMeta = Meta.build(meta).setAttr(attrs).meta;
        return mtr(newMeta).of(...seperateItems(items.map((it, i) => mtd({ columnalign: ca[i] ?? 'center' }).of(it)), [mtd(o)]));
      },
    }),
