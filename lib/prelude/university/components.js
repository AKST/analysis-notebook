...
const makeListTag = (tagName, cls) => makeHelper({
  of: ['html', tagName],
  create: (m, items) => {
    const { className: classIn, itemSpace = 'dense', blockMargin = 0, ...attr } = m.element;
...
const makeClsTagB = (tagName, cls) => makeTemplateHelper({
  of: ['html', tagName],
  create: (m, items) => {
    const { className: classIn, ...attr } = m.element;
    const newMeta = Meta.build(m)
      .setAttr(/** @type {any} */ (attr))
...
export const text = {
  a: makeTemplateHelper({
    of: ['html', 'a'],
    create: (m, ls) => node('html', 'a', Meta.attr(m, 'target', 'blank'), ls),
  }),
  b: doc.strong,
  figure: makeHelper({
    of: ['html', 'figure'],
...
export const infobox = makeHelper({
  ...
  create: (m, body) => {
    ...
    /** @type {E.Attrs<DefaultHelperAttrs<'html', 'aside'>>} */
    const newMeta = Meta.build(m)
